/*! \file **********************************************************************
  ...
  MODULNAME:              sim_hla_visu_menu_vis.cpp
...
#include <sstream>
#include "sim_hla_visu_menu_vis.h"
...
, m_general     ("Menu",      xPos, yPos)
{
  ...
  m_general.setCell(5,1,"Debug Errors [OFF]", color_yellow);
  m_general.setCell(5,2,"Debug Errors [ON]", color_yellow);
  m_general.setCell(5,3,"", color_yellow);
  ...
  m_general.SetRowActivation(5, SIM_ROW_ACTIVATED);
}
...
void CSimHlaMenuVisOnly::updateOverlay( SimuOverlayActivation& overlayActivation, boolean ETH_KAFASInputs, boolean CAN_bKafas2_ObjMabAsst_Valid )
{
  ...
  if(tmpActivatedRows[0] == SIM_ROW_ACTIVATED)
  {
    ...
    {
      overlayActivation.measfreezeVisu = 3; /* VISU_ENET */
    }
    else if(   (tmpColActive[1] == 3)
            && (CAN_bKafas2_ObjMabAsst_Valid)
            )
    {
      overlayActivation.measfreezeVisu = 4; /* VISU_CAN */
    }
    else
    {
      overlayActivation.measfreezeVisu        = 1; /* VISU_ECU */
    }
    overlayActivation.activeInputInterface  = (tmpActivatedRows[2]) ? true:false;
    overlayActivation.activeOutputInterface = (tmpActivatedRows[3]) ? true:false;
  ...
  else
  {
    ...
    overlayActivation.activeOutputInterface = 1;
    overlayActivation.activeBrightObjects   = 0;
    overlayActivation.activeErrorList       = 1;
