...
class enum(object):
    ...
    def __getattr__(self, name):
        try:
            ...
            raise StandardError("Type '%s' is not registered" % (name))
    ...
    def GetAttributeName(self, id):
        if id is not None:
            ...
            raise StandardError("Type with id '%i' is not registered" % (id))
        # else:
...
Revision 1.3 2013/11/13 16:20:41CET Hospes, Gerd-Joachim (uidv8815) 
...
- Added comments -  uidv8815 [Nov 13, 2013 4:20:41 PM CET]
...
Added 2 new generic functions for report generation (summary header, testcase header)
...
CYCLE_TIME_SHORT_MS = 56.0
CYCLE_TIME_LONG_MS  = 76.0
These cycle time limits are used by the Validation GUI to check how many cycles of a file are bad.
...
Increased accuracy of cycle time from 66 ms to 66.67 ms. The intention is to replace this value
...
Changed "TPECUCycleCounter" tag name to "CycleCounter" as tag name was misleading.
Replaced MAX_OBJECT_NR define with NUMBER_OF_OBJECTS_PORT_NAME. The value is located in the config file.
