 ...
 */
#define DEM_E_DET_REPORT                    0x03U

#if (defined  DEM_EVENTENTRY_UPDATED)
#error  DEM_EVENTENTRY_UPDATED already defined
#endif
/** \brief Indicates that an event memory entry was stored/updated
 **
 ** Dem-specific internal ::Std_ReturnType value used by
 ** Dem_StoreFF(), Dem_StoreDevFFs() , Dem_ProcessEventEntryFFData(),
 ** Dem_StoreED(), Dem_ProcessEventEntryEData(),
 ** Dem_StoreJ1939FF() and Dem_ProcessEventEntryJ1939FFData().
 */
#define  DEM_EVENTENTRY_UPDATED             0x32U /* 50 */

#if (defined  DEM_EVENTENTRY_NO_UPDATE_REQUIRED)
#error  DEM_EVENTENTRY_NO_UPDATE_REQUIRED already defined
#endif
/** \brief Indicates that the processing was successful but the storage/update
 **        of the event memory entry was not required
 **
 ** Dem-specific internal ::Std_ReturnType value used by
 ** Dem_StoreFF(), Dem_StoreDevFFs() , Dem_ProcessEventEntryFFData(),
 ** Dem_StoreED(), Dem_ProcessEventEntryEData(),
 ** Dem_StoreJ1939FF() and Dem_ProcessEventEntryJ1939FFData().
 */
#define  DEM_EVENTENTRY_NO_UPDATE_REQUIRED  0x33U /* 51 */
...
#define DEM_IS_OVFIND_SET(OriginIdx) \
  (DEM_IS_ANY_BIT_SET(DEM_OVFIND_FLAGS, (uint8) (1U << (OriginIdx)) ))
...
#define DEM_IS_ST_BIT_TFTOC_SET(StatusByte) \
  DEM_IS_ANY_BIT_SET((StatusByte), DEM_UDS_STATUS_TFTOC)
...
#define DEM_IS_ST_BIT_TF_SET(StatusByte) \
  DEM_IS_ANY_BIT_SET((StatusByte), DEM_UDS_STATUS_TF)
...
#define DEM_IS_ST_BIT_TNCTOC_SET(StatusByte) \
  DEM_IS_ANY_BIT_SET((StatusByte), DEM_UDS_STATUS_TNCTOC)
...
#define DEM_IS_ST_BIT_WIR_SET(StatusByte) \
  DEM_IS_ANY_BIT_SET((StatusByte), DEM_UDS_STATUS_WIR)
...
#define DEM_IS_ST_BIT_CDTC_SET(StatusByte) \
  DEM_IS_ANY_BIT_SET((StatusByte), DEM_UDS_STATUS_CDTC)
...
#define DEM_IS_ST_BIT_PDTC_SET(StatusByte) \
  DEM_IS_ANY_BIT_SET((StatusByte), DEM_UDS_STATUS_PDTC)
...
#define DEM_ERRORQUEUE_GET_FFTOC(ErrorQueueEntry) \
  (DEM_IS_ANY_BIT_SET((ErrorQueueEntry), DEM_EVENT_STATUS_FFTOC_MASK) ?\
   TRUE : FALSE)

#if (defined DEM_IS_ANY_EV_ST_BIT_SET)
#error DEM_IS_ANY_EV_ST_BIT_SET already defined
#endif
/** \brief Is any bit set from mask in DTC status of event */
#define DEM_IS_ANY_EV_ST_BIT_SET(EventId, Mask) \
  DEM_IS_ANY_BIT_SET(DEM_DTC_STATUS_MASKS[(EventId)], (Mask))
...
#define DEM_IS_EV_ST_BIT_TNCTOC_SET(EventId) \
  DEM_IS_ANY_EV_ST_BIT_SET((EventId), DEM_UDS_STATUS_TNCTOC)
...
#define DEM_IS_EV_ST_BIT_TFTOC_SET(EventId) \
  DEM_IS_ANY_EV_ST_BIT_SET((EventId), DEM_UDS_STATUS_TFTOC)
...
#define DEM_IS_EV_ST_BIT_WIR_SET(EventId) \
  DEM_IS_ANY_EV_ST_BIT_SET((EventId), DEM_UDS_STATUS_WIR)
...
#define DEM_IS_EV_ST_BIT_TF_SET(EventId) \
  DEM_IS_ANY_EV_ST_BIT_SET((EventId), DEM_UDS_STATUS_TF)
...
#define DEM_IS_EV_ST_BIT_TNCSLC_SET(EventId) \
  DEM_IS_ANY_EV_ST_BIT_SET((EventId), DEM_UDS_STATUS_TNCSLC)
...
#define DEM_IS_EV_ST_BIT_PDTC_SET(EventId) \
  DEM_IS_ANY_EV_ST_BIT_SET((EventId), DEM_UDS_STATUS_PDTC)
...
#define DEM_IS_CMB_ST_BIT_SET(EventId, Mask) \
  DEM_IS_ANY_BIT_SET((Dem_CmbDTCStatus[Dem_GbiDTCConfIdx(EventId)]), (Mask))
...
#define DEM_IS_SR_ENDIANNESS_CONVERSION_REQUIRED(DataEndianness) \
  ((CPU_BYTE_ORDER != (DataEndianness)) && (DEM_OPAQUE != (DataEndianness)))
     ...
     ((DEM_J1939_READING_DTC_SUPPORT == STD_ON) || (DEM_J1939_READINESS1_SUPPORT == STD_ON)))
#if (defined DEM_J1939_MAX_DTC_COUNT_VALUE)
#error DEM_J1939_MAX_DTC_COUNT_VALUE already defined
#endif
/** \brief Maximum number of J1939 DTCs matching the filter */
#define DEM_J1939_MAX_DTC_COUNT_VALUE 250U
#endif
...
/*------------------[J1939 event configuration macros]----------------------*/

/** \brief Macro holds offset value to retrieve J1939 node IDs */
#if (defined DEM_J1939_NODE_OFFSET)
#error DEM_J1939_NODE_OFFSET already defined
#endif
#define DEM_J1939_NODE_OFFSET 24U

/** \brief Macro holds mask value to retrieve J1939 DTC value */
#if (defined DEM_J1939_DTC_MASK)
#error DEM_J1939_DTC_MASK already defined
#endif
#define DEM_J1939_DTC_MASK 0x00FFFFFFU
...
#if (defined DEM_NUMBER_OF_J1939_FEATURES)
#error DEM_NUMBER_OF_J1939_FEATURES already defined
#endif
/** \brief Number of J1939 features initialized with Dem_InitJ1939FncPtr[] */
#if ((DEM_J1939_FREEZEFRAME_SUPPORT == STD_ON) && (DEM_J1939_READING_DTC_SUPPORT == STD_ON))
#define DEM_NUMBER_OF_J1939_FEATURES     2U
#elif ((DEM_J1939_FREEZEFRAME_SUPPORT == STD_ON) || (DEM_J1939_READING_DTC_SUPPORT == STD_ON))
#define DEM_NUMBER_OF_J1939_FEATURES     1U
#endif
#endif
...
#define DEM_GET_PERMANENT_MEMORY_PFC_CYCLE_COND(Entry)                   \
  DEM_IS_ANY_BIT_SET((Entry), DEM_PERMANENT_MEMORY_PFC_CYCLE_COND_MASK)
...
#define DEM_GET_PERMANENT_MEMORY_PASSED_CYCLE_COND(Entry)                \
  DEM_IS_ANY_BIT_SET((Entry), DEM_PERMANENT_MEMORY_PASSED_CYCLE_COND_MASK)

#endif /* DEM_MAX_NUMBER_EVENT_ENTRY_PER != 0U */

#if (DEM_AVAILABILITY_SUPPORT == DEM_EVENT_AVAILABILITY)
/** \brief Macro to generalize access to the bit-field arrays holding the event
 ** available status or the DTC enabled/disabled flags */
#define DEM_EVENT_AVAILABILITY_BIT_ARRAY Dem_EventAvailableStatus
#else
#define DEM_EVENT_AVAILABILITY_BIT_ARRAY Dem_EventEnabled
#endif
...
/*------------------[gate-entry type for intermediate DTC status storage]---*/

#if (DEM_INTERMEDIATE_STORAGE_DTC_STATUS == STD_ON)
/** \brief StorageOrderIdAndDTCStatus base type */
typedef uint8 StorageOrderIdAndDTCStatusType;

/** \brief Pointer to ::StorageOrderIdAndDTCStatusType */
typedef CONSTP2VAR(StorageOrderIdAndDTCStatusType, TYPEDEF, DEM_VAR_CLEARED)
  Dem_GateEntryStorageOrderIdAndDTCStatusPtrType;
...
typedef struct
{
  ...
  Dem_DTCKindType                 DTCKind;              /**< Kind of DTC */
  uint8                           Node;                 /**< NM Node */
  boolean                         IsFilterValid;        /**< Flag used to indicate filter validity */
...
typedef struct
{
  ...
  /** \brief Notification data "J1939Dcm_DemTriggerOnDTCStatus" */
  Dem_EventIdType Invoke_J1939Dcm_DemTriggerOnDTCStatus_Data;
#endif
 ...
 **
 ** Type to store the J1939 DTC value and node ID for each configured event.
 ** Three lower bytes are used for DTC value and highest byte is used for node ID.
 */
typedef uint32 Dem_J1939DtcAndNodeBitfieldType;
#endif
...
#if (DEM_INTERMEDIATE_STORAGE_DTC_STATUS == STD_ON)
/** \brief Function to fill StorageOrderId
 **
 ** This function generates and provides the StorageOrderId value to be stored
 ** in the non-volatile blocks.
 **
 ** \param[in]     BlockType       The type of the non-volatile block for
 **                                which the Largest BlockId is reported.
 ** \param[out]    StorageOrderId  Pointer to the array that holds the
 **                                StorageOrderId of the non-volatile block.
 **
 ** \Reentrancy{Non reentrant}
 ** \Synchronicity{Synchronous}
 **/
extern FUNC(void, DEM_CODE) Dem_GetNextStorageOrderId(
 ...
 ** \pre This function shall be called from within a critical section only.
 */
/* !LINKSTO Dem_OBD_0040,2 */
extern FUNC(void, DEM_CODE) Dem_PermanentMemoryRemoveEntry(
  Dem_EventIdType EventId);
 ...
 ** \Synchronicity{Synchronous}
 */
extern FUNC(Std_ReturnType, DEM_CODE) Dem_ProcessEventEntryJ1939FFData(
  Dem_EventIdType             EventId,
  Dem_EventMemoryEntryPtrType EvMemEntryPtr);

  /** \brief Function to copy J1939 freeze frame data to destination buffer
 **
 ** \param[in] EventId
 ** \param[in] EvMemEntry
 ** \param[in] BufSize
...
/** \brief Function which queues event status to be written in the NVM
 **
 ** It is used by Dem_TriggerNvmWrite().
 **
 ** \param[in] none
 **
 ** \return WriteBlockResult value indicating if the data was stored in the NVM
 **
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous}
 */
...
#if ((DEM_J1939_SUPPORT == STD_ON) && (DEM_J1939_READING_DTC_SUPPORT == STD_ON))
/** \brief Trigger J1939 Dcm callback in case the given event references a DTC
 ** with J1939 value.
 **
 ** \param[in] EventId  0 < EventId <= DEM_MAX_EVENTID
 **
 ...
 **
 ** \param[in] ServiceId Identifies the calling context
 ** \param[in] EventInfo Structure containing:
 **              0 < EventId <= DEM_MAX_EVENTID, always
 **              0 < RootId <= DEM_MAX_EVENTID, if multi event triggering
 **              is enabled
...
#if (DEM_J1939_SUPPORT == STD_ON)
/** \brief Get J1939 DTC value from Dem_J1939Config
 **
 ** Internal function to read the assigned J1939 DTC regardless of the event availability
 ** and the DTC suppression status.
 ** This function shall be used instead of the wrapper Dem_GetJ1939DTC().
 **
 ** \param[in] EventId  0 < EventId < DEM_NUMBER_OF_EVENTS
 **
 ** \return DEM_NO_DTC if no J1939 DTC is assigned to the event, otherwise the assigned
...
/** \brief Initializes the J1939 Freeze Frame filter attributes.
 **
 ** \params None
 **
 ** \return void
 ...
 */
#if (DEM_J1939_FREEZEFRAME_SUPPORT == STD_ON)
extern FUNC(void, DEM_CODE) Dem_InitJ1939FreezeFrameFilter(void);
#endif

/** \brief Initializes the J1939 DTC filter attributes.
 **
 ** \params None
 **
 ** \return void
...
extern FUNC(void, DEM_CODE) Dem_InitJ1939DtcFilter(void);
#endif
#endif
...
extern Dem_GateEntryStorageOrderIdAndDTCStatusPtrType
  Dem_GateEntryStorageOrderIdAndDTCStatus[DEM_MAXNUM_ORIGINS];
#endif
...
#if (DEM_J1939_SUPPORT == STD_ON)
#if (DEM_EVENT_COMBINATION_SUPPORT == DEM_EVCOMB_DISABLED)
/** \brief J1939 specific configuration description tables for Events
 **
 ** Number of elements: ::DEM_NUMBER_OF_EVENTS
 **
 ** These arrays contain all J1939 specific event-related data,
 ** which partially are packed by using the 'bit packing scheme'.
 */
extern CONST(Dem_J1939DtcAndNodeBitfieldType, DEM_CONST)
    Dem_EventJ1939DtcAndNode[DEM_NUMBER_OF_EVENTS];
extern CONST(Dem_J1939FFClassIdxType, DEM_CONST)
    Dem_EventJ1939FFClassIdx[DEM_NUMBER_OF_EVENTS];
#else /* (DEM_EVENT_COMBINATION_SUPPORT != DEM_EVCOMB_DISABLED) */
/** \brief J1939 specific configuration description table for DTCs
 **
 ** Number of elements: ::DEM_NUMBER_OF_DTCS
 **
 ** These arrays contain all J1939 specific event-related data,
 ...
 */
extern CONST(Dem_J1939DtcAndNodeBitfieldType, DEM_CONST)
    Dem_EventJ1939DtcAndNode[DEM_NUMBER_OF_DTCS];
extern CONST(Dem_J1939FFClassIdxType, DEM_CONST)
    Dem_EventJ1939FFClassIdx[DEM_NUMBER_OF_DTCS];
#endif/* (DEM_EVENT_COMBINATION_SUPPORT == DEM_EVCOMB_DISABLED) */
