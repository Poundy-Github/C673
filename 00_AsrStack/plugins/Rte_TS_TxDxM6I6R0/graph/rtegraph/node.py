...
class Node:
    ...
    def generate(self, subgraph):
        ...
        """
        subgraph.attr('node', fillcolor=self._color, shape=self._shape,
                      label=self._label, style='filled', fontcolor=self._fontcolor)
        subgraph.node(self.id)

    @staticmethod
    def get_or_create_node(nodes, cluster, xmlelement, text, style):
        """
        ...
        return Node.get_or_create_node_with_customized_id(element_id, nodes, cluster, xmlelement, text, style)
    ...
    @staticmethod
    def get_or_create_node_with_customized_id(customizedid, nodes, cluster, xmlelement=None, text="", style=None):
        """
        ...
        if node is None:
            ...
            node = Node(element_id, text, style.color,
                        style.shape, cluster.id, style.fontcolor)
