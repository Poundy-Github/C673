...
#endif /* (LINIF_MASTER_SUPPORTED == STD_ON) && (LINIF_DEV_ERROR_DETECT == STD_ON) */
        {
            ...
            if(pIfChannel->cFSM == LINIF_CHANNEL_OPERATIONAL)
            {
                ...
                SchM_Enter_LinIf_SCHM_LINIF_EXCLUSIVE_AREA_0();
                {
                    ...
                    const uint8 newNAD = (LINIF_CH_IS_MASTER == (LINIF_CFG_GET_CHANNEL[Channel].CfgFlags & LINIF_NODETYPE_MASK)) ? pTpTxConfig[LinTpTxSduId].LinTpTxNSduNad : LinTp_GetConfNad(Channel);
#else
