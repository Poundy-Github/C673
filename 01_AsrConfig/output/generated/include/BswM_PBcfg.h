...
typedef struct /* BswM_ConfigLayoutType */ {
    VAR( BswM_ConfigType, TYPEDEF ) BswM_RootConfig[4];
    VAR( BswMRuleType, TYPEDEF ) RuleTable[45];
    VAR( uint16, TYPEDEF ) RuleDependencyTable[46];
    VAR( BswMRangeType, TYPEDEF ) RuleDependencyListRangeTable[14];
    VAR( BswMActionListType, TYPEDEF ) ActionListTable[47];
    VAR( BswMActionListItemType, TYPEDEF ) ActionListItemTable[73];
    VAR( BswMPduGroupSwitchActionType, TYPEDEF ) PduGroupSwitchActionTable[12];
    VAR( uint16, TYPEDEF ) UInt16InitialValuesTable[4];
    VAR( UInt8Symbol, TYPEDEF ) UInt8InitialValuesTable[8];
