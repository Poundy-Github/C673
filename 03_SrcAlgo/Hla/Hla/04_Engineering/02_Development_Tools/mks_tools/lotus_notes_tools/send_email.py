...
class lotus_notes_send_email_obj(object):
    """
    Lotus notes send email class
    """
    def __init__(self):
        pass
    ...
    ##
    def sendEmail(self, recipients=[], subject='', body='', attachments=[], **kwargs):
...
#        self.sendEmail(recipients=["jochen.spruck@contiautomotive.com","jochen.spruck@contiautomotive.com"],subject='Test email from Python!', body='This has been a test, and only a test.',attachments= ["H:\BuildFarm\ARS301Report.xml"])
        ...
        doc.CopyTo = kwargs.get('cc', [])
        doc.BlindCopyTo = kwargs.get('bc', [])
        #doc.ReplaceItemValue("CopyTo", copyto)
        doc.Subject = subject
        
#        text = """file:\\\\lil75071\BuildFarm\ARS301Report.xml"""
...
#        rti.AppendDocLink(doc,"H:\BuildFarm\ARS301Report.xml","H:\BuildFarm\ARS301Report.xml") 
#        rti.AppendDocLink(db, db.Title)#"Link to referring document") 
