 ...
 ** containing node ID and J1939 DTC value */
CONST(Dem_J1939DtcAndNodeBitfieldType, DEM_CONST) Dem_EventJ1939DtcAndNode[DEM_NUMBER_OF_EVENTS] =
{
  [!INDENT "2"!][!//
    ((uint32)(DEM_J1939_INVALID_NODE_ID) << DEM_J1939_NODE_OFFSET) | 0x000000U, /* invalid event id and node entry */
    [!LOOP "node:order(DemConfigSet/*/DemEventParameter/*, 'DemEventId')"!][!//
      /* [!"name(.)"!] */
      [!IF "node:exists(DemDTCClassRef) and node:exists(as:ref(DemDTCClassRef)/DemJ1939DTCValue)"!][!//
        ((uint32)([!"translate(num:inttohex(as:ref(as:ref(./DemDTCClassRef)/DemJ1939NodeRef)/J1939NmNodeId, 2), 'abcdef', 'ABCDEF')"!]U) << DEM_J1939_NODE_OFFSET) |
        [!"translate(num:inttohex(as:ref(DemDTCClassRef)/DemJ1939DTCValue, 6), 'abcdef', 'ABCDEF')"!]U, /* J1939-DTC value and Node ID */
      [!ELSE!][!/* No DTC class configured and no node is referenced */!][!//
        (Dem_J1939DtcAndNodeBitfieldType)(((uint32)(DEM_J1939_INVALID_NODE_ID) << DEM_J1939_NODE_OFFSET) |
        (uint32)(DEM_NO_DTC)), /* No J1939-DTC value and no node referenced */
      [!ENDIF!][!//
    [!//
    [!ENDLOOP!][!//
  [!ENDINDENT!]
    ...
    [!FOR "DTCCtr" = "0" TO "num:integer(count(text:split($DemDTCClassList, '#')))"!][!//
      ...
      [!LOOP "as:modconf('Dem')[1]/DemConfigSet/*/DemEventParameter/*"!][!//
        ...
        [!IF "$DTCClassName = $EventDTCClassName"!][!//
          ...
          [!IF "not(node:exists(DemDTCClassRef)) or not(node:exists(DemEvtCmbCommonParamMaster)) or (DemEvtCmbCommonParamMaster = 'true')"!][!//
            [!IF "(DemEvtCmbCommonParamMaster = 'true')"!][!//
              /* J1939-DTC of Master Configuration Event: [!"name(.)"!] */
            [!ELSE!][!//
              /* J1939-DTC of Event: [!"name(.)"!] */
            [!ENDIF!][!//
            [!IF "node:exists(DemDTCClassRef) and node:exists(as:ref(DemDTCClassRef)/DemJ1939DTCValue)"!][!//
              ((uint32)([!"translate(num:inttohex(as:ref(as:ref(./DemDTCClassRef)/DemJ1939NodeRef)/J1939NmNodeId, 2), 'abcdef', 'ABCDEF')"!]U) << DEM_J1939_NODE_OFFSET) |
              [!"translate(num:inttohex(as:ref(DemDTCClassRef)/DemJ1939DTCValue, 6), 'abcdef', 'ABCDEF')"!]U, /* J1939-DTC value and Node ID */
...
[!INDENT "0"!]
  [!IF "count(DemGeneral/DemSPNClass/*) > 0"!]
    ...
    [!LOOP "DemGeneral/DemJ1939FreezeFrameClass/*"!]
      /*---[J1939 Freeze frame class [!"name(.)"!]]---*/
      [!VAR "NumJ1939FFSegs" = "count(DemSPNClassRef/*)"!]
      /** \brief J1939 Freeze frame data element class indexes linked
       ** to the configured SPNs */
      STATIC CONST(Dem_DataElementIdxType, DEM_CONST)
      Dem_J1939FFDataElementClassIdx[!"name(.)"!][[!"num:integer($NumJ1939FFSegs)"!]] =
      {
        [!LOOP "DemSPNClassRef/*"!]
          [!WS "2"!]/* [!"name(as:ref(.))"!] */
          [!VAR "DEClassName" = "name(as:ref(as:ref(.)/DemSPNDataElementClassRef))"!]
          [!VAR "IdxCtr" = "0"!]
          [!LOOP "as:modconf('Dem')[1]/DemGeneral/DemDataElementClass/*[. = 'DemExternalCSDataElementClass']"!]
            [!IF "$DEClassName = name(.)"!]
    ...
    Dem_J1939FFClass[DEM_NUM_J1939FFCLS] =
    {
      [!INDENT "2"!]
        [!LOOP "DemGeneral/DemJ1939FreezeFrameClass/*"!]
          {
            [!VAR "NumJ1939FFSegs" = "count(DemSPNClassRef/*)"!]
            [!WS "2"!]Dem_J1939FFDataElementClassIdx[!"name(.)"!],
