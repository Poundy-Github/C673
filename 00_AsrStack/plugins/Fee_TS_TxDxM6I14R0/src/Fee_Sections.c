...
)
{
  ...
  TS_MemSet(Fee_Gv.FeeBuffPtr, FEE_SECTION_STATUS_ACTIVE, FEE_SECTION_MARKER_SIZE_AL);

  if(E_OK == Fls_Write(TargetAddress, Fee_Gv.FeeBuffPtr, FEE_SECTION_MARKER_SIZE_AL))
  {
    /* Reset retry counter */
...
)
{
  ...
  TS_MemSet(Fee_Gv.FeeBuffPtr, FEE_SECTION_STATUS_ERASABLE, FEE_SECTION_MARKER_SIZE_AL);
