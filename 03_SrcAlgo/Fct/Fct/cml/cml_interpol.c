/*! \file **********************************************************************
  ...
  VERSION:                 $Revision: 1.9.1.3 $


  ---*/ /*---
  CHANGES:                 $Log: cml_interpol.c  $
  CHANGES:                 Revision 1.9.1.3 2020/01/08 11:30:45CET Thomas, Jyothy (uid22555) 
  CHANGES:                 corrected alignment
  CHANGES:                 Revision 1.9.1.2 2019/11/11 11:40:21CET Thomas, Jyothy (uid22555) 
  CHANGES:                 CML_f_BoundedLinInterpol2, CML_f_CalculatePolygonValue and CML_f_LinearInterpolation
  CHANGES:                 are rewritten to have one computation per line. This is to have ECU-SIL bit exactness on A15.
  CHANGES:                 Revision 1.9.1.1 2019/06/21 15:15:08CEST Thomas, Jyothy (uid22555) 
  CHANGES:                 QAF Level3 fixes for MFC431TA19
  CHANGES:                 Revision 1.9 2018/02/26 08:50:04CET Arulkumar-EXT, Nandu (uidr5428) 
  CHANGES:                 Updated the source code with respect to AL_CML_01.09.18_INT-1
  CHANGES:                 Revision 1.8 2017/11/09 11:36:17CET Mutschler, Matthias (uidv7867) 
...
float32 CML_f_linearInterpolation (float32 f_X1, float32 f_Y1, float32 f_X2, float32 f_Y2, float32 f_XPos)
{
  ...
  f_temp1 = f_X2 - f_X1;
  if (!CML_f_IsZero(f_temp1))
  {
    /* slope */
    f_temp1 = f_Y2 - f_Y1;
    f_temp2 = f_X2 - f_X1;
    f_Slope  = f_temp1 /f_temp2;

    /* offset */
    f_temp1 = f_Slope * f_X1;
    f_Offset = f_Y1 - f_temp1;
...
float32 CML_f_boundedLinInterpol2(float32 f_IVal, float32 f_Imin, float32 f_Imax, float32 f_Omin, float32 f_Omax)
{
  ...
  {
    f_temp1 = f_Omin + f_Omax;
    f_OVal = f_temp1 * 0.5f;
  }
  else
  {
      f_temp1 = f_Imax - f_Imin;
      f_temp2 = f_Omax - f_Omin;
      f_temp3 = f_temp2/f_temp1;

      f_temp1 = f_IVal - f_Imin;
      f_temp2 = f_temp1 * f_temp3;
      f_OVal = f_Omin + f_temp2;
  }
...
/* Deactivate QAC warning 3350, Date: 2019-06-18; Reviewer: uid22555;
    ...
    if( f_dx > C_F32_DELTA )
    {
      f_temp1 = f_InputValue - a_Table[s_Right-1].f0;
      f_WeightRight =  f_temp1 / f_dx;
    }
    else
    {
      f_WeightRight = 0.0f;
    }
    f_temp1 = 1.0f - f_WeightRight;
    f_temp2 = (a_Table[s_Right-1].f1) * f_temp1;
    f_temp3 = (a_Table[s_Right].f1) * f_WeightRight;
    f_Result = f_temp2 + f_temp3;
  }
  return f_Result;
                                        ...
                                        float32 a_ParaA[], float32 a_ParaB[], float32 a_ParaC[], float32 a_ParaD[], boolean a_SegmentSplineValid[])
{
  ...
  for (i = 0u; i < u_NrOfShapePoints; i++)
  {
      ...
      )
    {
      ...
      {
/* PRQA S 2986 5 */
/* Deactivate QAC warning 2986, Date: 2019-06-18; Reviewer: uid22555;
  ...
  {
/* PRQA S 4440 5 */
/* Deactivate QAC warning 4440, Date: 2019-06-17; Reviewer: uid22555;
   Reason: There is no risk in violating this rule as there is no loss of data. */
...
{ 
 uint32 u_FirstIndex = 0u;
 uint32 u_SecondIndex = 0u;
 float32 f_ReturnValue = 0.0f; 
 boolean b_flag = b_FALSE;

/* PRQA S 1842 10 */
/* Deactivate QAC warning 1842, Date: 2019-06-18; Reviewer: uid22555;
   Reason: There is no risk in converting from signed to unsigned here as both are positve values. */
 if(u_NBPoints > 1)
 {
     for (u_SecondIndex = 1u; (u_SecondIndex < u_NBPoints) && (b_flag == b_FALSE); ++u_SecondIndex) 
     {  
        if (f_InputValue < f_XValues[u_SecondIndex]) 
        { 
            b_flag = b_TRUE;
        }
     } 
     --u_SecondIndex;   /*u_SecondIndex was incremented one more than the actual exit condition*/
/* PRQA S 1842 , 1840 10 */
/* Deactivate QAC warning 1842,1840,  Date: 2019-06-18; Reviewer: uid22555;
   Reason: There is no risk in converting from signed to unsigned here as both are positve values. */
     u_FirstIndex = u_SecondIndex - 1;

     /*compute the linear interpolation*/
     f_ReturnValue = CML_f_linearInterpolation (f_XValues[u_FirstIndex], f_YValues[u_FirstIndex], f_XValues[u_SecondIndex], f_YValues[u_SecondIndex], f_InputValue);
 }
  
 return f_ReturnValue;
}/* CML_f_piecewiseLinearInterpolation() */
