"""
Filename                  : scons_common_scripts\algo\lib_env_ti_cortex_a8\SConscript.py
Description               : SConscript to set up the build environment for the build of ARM Cortex A8 Libraries. 
...
#
# MODULE NAME: lib_env_ti_cortex_a8\SConscript.py
#
# DESCRIPTION: SConscript to set up the build environment for the build of ARM Cortex A8 Libraries. 
#              It also generates IDE'S for ECU Libraries.
#
...
#
# LAST CHANGE:      $Date: 2021/12/13 19:04:45CET $
#                   $Author: Wang, David (Wangd3) $
#
# CURRENT VERSION:  $Revision: 1.1 $
#############################################################################
# CHANGES:                   $Log: SConscript.py  $
# CHANGES:                   Revision 1.1 2021/12/13 19:04:45CET Wang, David (Wangd3) 
# CHANGES:                   Initial revision
# CHANGES:                   Member added to project /ADAS/Projects/MFC5xx/MFC5J3/06_Algorithm/DPU/04_Engineering/02_Development_Tools/scons_tools/scons_common_scripts/algo/lib_env_ti_cortex_a8/project.pj
# CHANGES:                   Revision 1.49 2017/05/30 11:50:45CEST Bhagawati, Tridip (uidr2134) 
# CHANGES:                   GSCons update for Mainstream
# CHANGES:                   Revision 1.48 2017/04/27 16:19:11CEST Bhagawati, Tridip (uidr2134) 
# CHANGES:                   Support Mainstream adaptation
# CHANGES:                   Revision 1.47 2016/06/30 10:31:46CEST Annashamachar, Shwetha (uidj8746) 
# CHANGES:                   File header added
# CHANGES:                   Revision 1.46 2016/06/30 08:36:17CEST Annashamachar, Shwetha (uidj8746) 
# CHANGES:                   File header added
# CHANGES:                   Revision 1.45 2015/09/23 16:11:05CEST Fischer-EXT, Andre (uidg5297) 
# CHANGES:                   add dependency between ccs project generation and ti compiler-directory
# CHANGES:                   - Added comments -  uidg5297 [Sep 23, 2015 4:11:06 PM CEST]
# CHANGES:                   Change Package : 379758:1 http://mks-psad:7002/im/viewissue?selection=379758
# CHANGES:                   Revision 1.44 2015/06/26 07:06:18CEST Palanisamy-EXT, Lenin (uidr0826) 
# CHANGES:                   Add revision label with module description for all GSCons files (Issue 320651)
# CHANGES:                   --- Added comments ---  uidr0826 [Jun 26, 2015 7:06:18 AM CEST]
# CHANGES:                   Change Package : 350940:1 http://mks-psad:7002/im/viewissue?selection=350940

#***************************************************************************************************

# SConscript to build ARM Cortex A8 library and CCS 5.3 project
...
execfile(sconscript_dir + "/ti_arm_cortex_a8_lib_config.scfg")
...
if not verbose:
    arm_env['CCCOMSTR'] = "\nCompiling ecu cortex a8 $variant \n[$SOURCE]"
    arm_env['CXXCOMSTR'] = "\nCompiling ecu cortex a8 $variant \n[$SOURCE]"
    arm_env['LINKCOMSTR'] = "\nLinking ecu cortex a8 $variant \n[$TARGET]"
...
for variant in variant_list:
    ...
    if variant[0] == "release":
        if gcc_used:
            for flag in cortex_a8_gcc_rel_common_ccflags.split():
                new_variant.PrependUnique(CCFLAGS = flag)
        else:
            for flag in cortex_a8_rel_common_ccflags.split():
                new_variant.PrependUnique(CCFLAGS = flag)
    elif variant[0] == "debug":
        if gcc_used:
            for flag in cortex_a8_gcc_dbg_common_ccflags.split():
                new_variant.PrependUnique(CCFLAGS = flag)
        else:
            for flag in cortex_a8_dbg_common_ccflags.split():
                new_variant.PrependUnique(CCFLAGS = flag)
    else:
        if gcc_used:
            for flag in cortex_a8_gcc_common_ccflags.split():
                new_variant.PrependUnique(CCFLAGS = flag)
        else:
            pass
...
for e in arm_env_list:
    ...
    arm_algo_build_dir = (build_dir + "/algo/" + algo_name +
                         "/ti_cortex_a8/" + env["variant"])
    arm_algo_deliverables_dir = (deliverables_dir + "/lib/ti_cortex_a8/algo/" +
                         algo_name + "/")
    ...
    if install_to_deliverables_dir in env["variant"]:
        algo_ti_arm_cortex_a8_lib = env.Install([arm_algo_build_dir, arm_algo_deliverables_dir],
                                    result[0][0])
    else:
        algo_ti_arm_cortex_a8_lib = env.Install([arm_algo_build_dir],
                                    result[0][0])

    if gcc_used:
      # GCC requires a library name format "lib<name>.a" at command line interface
      gcc_format_lib_name = "lib" + target_name + "_" + env["variant"] + ".a"
      algo_ti_arm_cortex_a8_lib += env.InstallAs([arm_algo_build_dir + "/" + gcc_format_lib_name], result[0][0])

    # generate aliases containing the variant's name
    generate_alias(component_name + "_algo_ti_cortex_a8_" + env["variant"], algo_ti_arm_cortex_a8_lib)
    generate_alias(component_name + "_algo_ecu_" + env["variant"], algo_ti_arm_cortex_a8_lib)
    generate_alias(component_name + "_algo_" + env["variant"], algo_ti_arm_cortex_a8_lib)
    if algo_name != component_name:
        generate_alias(algo_name + "_algo_ti_cortex_a8_" + env["variant"], algo_ti_arm_cortex_a8_lib)
        generate_alias(algo_name + "_algo_ecu_" + env["variant"], algo_ti_arm_cortex_a8_lib)
        generate_alias(algo_name + "_algo_" + env["variant"], algo_ti_arm_cortex_a8_lib)
    generate_alias(env["variant"], algo_ti_arm_cortex_a8_lib)
    generate_alias("all_" + env["variant"], algo_ti_arm_cortex_a8_lib)
    # generate aliases NOT containing the variant's name
    generate_alias(component_name + "_algo_ti_cortex_a8", algo_ti_arm_cortex_a8_lib, component_name + ':algo:ecu:ti_cortex_a8', ['algo lib: Cortex A8'])
    generate_alias(component_name + "_algo_ecu", algo_ti_arm_cortex_a8_lib, component_name + ':algo:ecu')
    generate_alias(component_name + "_algo", algo_ti_arm_cortex_a8_lib, component_name + ':algo')
    if algo_name != component_name:
        generate_alias(algo_name + "_algo_ti_cortex_a8", algo_ti_arm_cortex_a8_lib, component_name + ':' + algo_name + '_algo:ecu:ti_cortex_a8', ['algo lib: Cortex A8'])
        generate_alias(algo_name + "_algo_ecu", algo_ti_arm_cortex_a8_lib, component_name + ':' + algo_name + '_algo:ecu')
        generate_alias(algo_name + "_algo", algo_ti_arm_cortex_a8_lib, component_name + ':' + algo_name + '_algo')
    generate_alias("all", algo_ti_arm_cortex_a8_lib, component_name)
    ...
    if COMMAND_LINE_TARGETS:
        for command in COMMAND_LINE_TARGETS:
            if((re.search(algo_name + "_sim", command)) or (re.search(component_name + "_sim", command))):    
                NoClean([algo_ti_arm_cortex_a8_lib, dirsToClean])
            else:
			    Clean(algo_ti_arm_cortex_a8_lib, dirsToClean)
    else:
        Clean(algo_ti_arm_cortex_a8_lib, dirsToClean)
...
if not skip_ide_generation:
    ...
    ide_ccs_algo_ti_cortex_a8 = arm_env.EclipseProject(
                        # specify project name
                        target_name = algo_name.upper() + "_TI_CortexA8_Lib",
                        # specify folder for storing the project
                        target_dir = build_dir + "/algo/" + algo_name + "/ti_cortex_a8/",
                        ...
                        scons_target = [algo_name + "_algo_ti_cortex_a8_" + variant[0] for variant in variant_list],
    ...
    Requires(ide_ccs_algo_ti_cortex_a8, Dir(tms470_code_gen_tool_dir).abspath)

    generate_alias(component_name + "_ide_ccs_algo_ti_cortex_a8", ide_ccs_algo_ti_cortex_a8, component_name + ':ide:ccs:ti_cortex_a8', ['CCS 5: CortexA8 algo lib'])
    generate_alias(component_name + "_ide_ccs", ide_ccs_algo_ti_cortex_a8, component_name + ':ide:ccs')
    generate_alias(component_name + "_ide", ide_ccs_algo_ti_cortex_a8, component_name + ':ide')
    if algo_name != component_name:
        generate_alias(algo_name + "_ide_ccs_algo_ti_cortex_a8", ide_ccs_algo_ti_cortex_a8, component_name + ':' + algo_name + '_ide:ccs:ti_cortex_a8', ['CCS 5: CortexA8 algo lib'])
        generate_alias(algo_name + "_ide_ccs", ide_ccs_algo_ti_cortex_a8, component_name + ':' + algo_name + '_ide:ccs')
        generate_alias(algo_name + "_ide", ide_ccs_algo_ti_cortex_a8, component_name + ':' + algo_name + '_ide')
    generate_alias("all", ide_ccs_algo_ti_cortex_a8, component_name)
...
if not skip_ide_generation:
    ...
    if 'arm_gcc' not in arm_env.Clone()['TOOLS']:
        for index, variant in enumerate(variant_list):
            map(lambda x : ecu_cppdefs[variant[0]].append(x), cortex_a8_built_in_defines_list)
            if "--abi=eabi" in arm_env_list[index].Clone(variant=variant[0])['CCFLAGS']:
		        map(lambda x : ecu_cppdefs[variant[0]].append(x), cortex_a8_built_in_eabi_defines_list)		
    ...
    talias = [algo_name + "_algo_ti_cortex_a8_" + variant[0] for variant in variant_list]
    talias += [algo_name + "_qac_ti_cortex_a8"]
    tcppdefs.append([])
    if profile:
