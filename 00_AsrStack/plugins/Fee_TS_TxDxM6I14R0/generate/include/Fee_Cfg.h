...
[!IF "(((node:exists(FeeUseBlankCheck)) and (FeeUseBlankCheck = 'true')) and (FeeConsistencyPattern = 'true'))"!][!//
...
[!IF "((node:exists(FeeUseBlankCheck)) and (FeeUseBlankCheck = 'true'))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(((node:exists(FeeUseBlankCheck)) and (FeeUseBlankCheck = 'true')) and (FeeConsistencyPattern = 'true'))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]

[!IF "(not(node:exists(as:modconf('Fls')[1]/CommonPublishedInformation/VendorApiInfix))) or (as:modconf('Fls')[1]/CommonPublishedInformation/VendorApiInfix = '')"!][!//
/** \brief Fls_Read API used */
#if (defined Fls_Read)
#error Fls_Read is already defined
#endif
#define Fls_Read              [!"$FeeFlsReadApi"!]
[!ENDIF!][!//
...
[!SELECT "FeeGeneral"!]
[!VAR "StateNumber" = "20"!][!//
[!IF "((node:exists(FeeUseBlankCheck)) and (FeeUseBlankCheck = 'true'))"!]
