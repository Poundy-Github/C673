/*! \file **********************************************************************
  ...
  CHANGES:                   Revision 1.19 2017/11/27 06:18:19CET A, Nandu (uidr5428) (uidr5428) 
  CHANGES:                   Replaced TRUE and FALSE with b_TRUE and b_FALSE to match the Mainstream update
  CHANGES:                   Revision 1.18 2017/11/16 13:26:19CET Kotre-EXT, Jeevan (uidk9586) 
  CHANGES:                   Updated cml_mat module.
  CHANGES:                   Revision 1.17 2017/06/06 07:43:32CEST Thomas, Jyothy (uid22555) 
  CHANGES:                   Changed function name to CML_GetTrafoMatByDisplacementandAngle to fix QAc warnings
  CHANGES:                   Revision 1.16 2017/05/26 11:06:03CEST Thomas, Jyothy (uid22555) 
  CHANGES:                   merged from branch
...
void CML_v_TransformPosition2D(CML_t_TrafoMatrix2D const *p_Matrix , float32 *p_DistX , float32 *p_DistY)   
{
  ...
  *p_DistY = ( ((*p_DistX * p_Matrix->f10) + (f_DistanceY * p_Matrix->f00)) + p_Matrix->f12 );
  *p_DistX = ( ( (*p_DistX * p_Matrix->f00) - (f_DistanceY * p_Matrix->f10) ) + p_Matrix->f02 );
}
...
float32 CML_f_TransformXPosition2D(CML_t_TrafoMatrix2D const *p_Matrix , float32 f_DistX , float32 f_DistY)   
{
  ...
  {
    f_ret  = ( ( (f_DistX * p_Matrix->f00) - (f_DistY * p_Matrix->f10) ) + p_Matrix->f02 );    
  }
  return f_ret;
}
...
float32 CML_f_TransformYPosition2D(CML_t_TrafoMatrix2D const *p_Matrix , float32 f_DistX , float32 f_DistY)   
{
  ...
  {    
    f_ret = ( ((f_DistX * p_Matrix->f10) + (f_DistY * p_Matrix->f00)) + p_Matrix->f12 );    
  }
  return f_ret;
}
...
*****************************************************************************/
uint8 CML_u_MatrixCopy(float32 const a_MatrixA[], float32 a_MatrixCpy[], uint8 u_dim_Ax, uint8 u_dim_Ay)
{
  uint16 n;
  uint16 p;
...
*****************************************************************************/
uint8 CML_u_MatrixTranspose(float32 const a_MatrixA[], float32 a_ATranspose[], uint8 u_dim_Ax, uint8 u_dim_Ay)
{
  uint8   u_ret;
  uint8   u_Idx2,u_Idx1;
...
*****************************************************************************/
uint8 CML_u_MatrixMultiplication(float32 const a_MatrixA[], float32 const a_MatrixB[], float32 a_AxB[], uint8 u_dim_Ax, uint8 u_dim_Ay, uint8 u_dim_Bx)
{
  uint8    u_ret;
  uint8    i, j, k;  
  float32  f_save;
      ...
      if(max_val >= f_e)
      {
          ...
          for (i = 0u; i < u_dim_xy; i++) 
          {
              if (i != u_row) 
              {
                  ...
                  for (j = 0u; j < u_row; j++)    
                  {  
                      a_InverseA[(i*u_dim_xy)+j] += fak*a_InverseA[(u_row*u_dim_xy)+j];      
                  }

                  for (j = u_row; j < u_dim_xy; j++)    
                  {  
                      a_tempMatrix[(i*u_dim_xy)+j] += fak*a_tempMatrix[(u_row*u_dim_xy)+j];                  
                      a_InverseA[(i*u_dim_xy)+j]   += fak*a_InverseA[(u_row*u_dim_xy)+j];      
                  }
              }
          }
...
*****************************************************************************/
uint8 CML_u_MatrixAddition(float32 const a_MatrixA[], float32 const a_MatrixB[], float32 a_AplusB[], uint8 u_dim_x, uint8 u_dim_y)
{
  uint8  u_Index      = 0u;
...
*****************************************************************************/
uint8 CML_u_MatrixSubtraction(float32 const a_MatrixA[], float32 const a_MatrixB[], float32 a_AminusB[], uint8 u_dim_x, uint8 u_dim_y)
{
  uint16 u_Index      = 0u;
  uint16 u_NumEntries = (uint16)u_dim_x * (uint16)u_dim_y;
  while(u_NumEntries > 0UL)
