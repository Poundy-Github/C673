/*! \file **********************************************************************
   ...
   File: vdy_matrix_inversion_2x2.c
...
/*
 * Output and update for atomic system: '<S27>/Matrix_Inversion_2x2'
 * Block description for: '<S27>/Matrix_Inversion_2x2'
 *   Compute an Inverse Matrix for a 2x2 Matrix
 */
void vdy_matrix_inversion_2x2(const real32_T rtu_HPHt_R[4],
  rtB_vdy_matrix_inversion_2x2 *localB)
{
  real32_T determinant;
  int32_T i;

  /* Embedded MATLAB: '<S27>/Matrix_Inversion_2x2' */
  /* Embedded MATLAB Function 'Matrix_Inversion_2x2': '<S35>:1' */
  ...
  /* '<S35>:1:7' */
  determinant = (rtu_HPHt_R[0] * rtu_HPHt_R[3]) - (rtu_HPHt_R[2] * rtu_HPHt_R[1]);
  ...
  if (!localB->det_flag) {
    ...
    /* '<S35>:1:12' */
    localB->HPHt_R_inv[0] = rtu_HPHt_R[3] / determinant;

    /* '<S35>:1:13' */
    localB->HPHt_R_inv[2] = (-rtu_HPHt_R[2]) / determinant;

    /* '<S35>:1:14' */
    localB->HPHt_R_inv[1] = (-rtu_HPHt_R[1]) / determinant;

    /* '<S35>:1:15' */
    localB->HPHt_R_inv[3] = rtu_HPHt_R[0] / determinant;
  }
}
