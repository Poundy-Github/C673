...
{
  const Fls_AddressType TargetAddressInFlash = Fee_Gv.FeeInfoAddress;
  Fls_AddressType TargetAddressInCache;
...
)
{
  ...
  else
  {
    ...
    Fee_Gv.FeeDataAddress += ((FEE_BIGGEST_BLOCK_SIZE - (Fls_AddressType)BlockAlignedSize) >
                             ((Fee_Gv.FeeInfoAddress - ((FEE_DATA_INFO_MARGIN - 1U)*FEE_BLOCKINFO_SIZE)) - Fee_Gv.FeeDataAddress)) ?
                             ((Fee_Gv.FeeInfoAddress - ((FEE_DATA_INFO_MARGIN - 1U)*FEE_BLOCKINFO_SIZE)) - Fee_Gv.FeeDataAddress) :
                             (FEE_BIGGEST_BLOCK_SIZE - (Fls_AddressType)BlockAlignedSize);
    /* !LINKSTO Fee.Dns.FailureHandling.Job,1 */
    Fee_JobError();
  }
}
