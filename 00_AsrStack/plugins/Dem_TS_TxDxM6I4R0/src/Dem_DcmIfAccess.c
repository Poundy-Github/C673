 ...
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_ClearEvents(void);
 ...
 ** \return Updated result of clear event process
 */
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_TriggerNvmWrite(
...
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_ClearEvents(void)
{
  ...
  Dem_EventIdType EventId = Dem_ClearDtcAsyncStatus.NextEventId;
  ...
  else
  {
    ...
    if (DEM_PENDING == Result)
    {
      ...
      Dem_ClearDtcAsyncStatus.NextEventId = EventId;
    }
  }
  ...
  DBG_DEM_CLEAREVENTS_EXIT(Result);
  ...
  P2VAR(Dem_EventModifiedType, AUTOMATIC, AUTOMATIC) AnyEventModified)
{
  ...
  /* !LINKSTO Dem.J1939.Dem_J1939DcmClearDTC.ClearStatus,1 */
#if ( (DEM_EVENT_DISABLING_SUPPORT == STD_ON) && \
  ...
  if (DEM_IS_ANY_BIT_SET(OldDTCStatusMask, DEM_UDS_STATUS_TFSLC))
#endif
  {
    ...
    if (Dem_SearchForEntry(EventId, &EvMemEntry, &EvMemEntryIdx) == TRUE)
    {
      ...
      /* !LINKSTO Dem.J1939.Dem_J1939DcmClearDTC.ClearEventEntry,1 */
...
#endif /* (DEM_NUM_CMB_DTCS > 0U) */
  {
    if (TRUE == Dem_J1939DcmClearFilterCheck(
                  Dem_ClearDtcAsyncStatus.Dem_J1939DTCTypeFilter,
  ...
  P2VAR(uint8, AUTOMATIC, AUTOMATIC) GroupIdx)
{
  ...
  if(Dem_ClearDtcAsyncStatus.ClientId == DEM_VIRTUAL_CLIENT_J1939)
  {
    /* J1939 client can request DTC clearing only for the DTC groups
     * DEM_J1939DTC_CLEAR_ALL and DEM_J1939DTC_CLEAR_PREVIOUSLY_ACTIVE */
    Result = TRUE;
  }
  else
#endif
