...
/* minor version check */
#if (WDGM_SW_MINOR_VERSION < 2U)
#error WDGM_SW_MINOR_VERSION wrong (< 2U)
#endif
...
/* patch version check */
#if (WDGM_SW_PATCH_VERSION < 3U)
#error WDGM_SW_PATCH_VERSION wrong (< 3U)
#endif

#if (defined WDGM_EB_CHECK_CALLERID)
#error WDGM_EB_CHECK_CALLERID already defined
#endif
...
#if (defined WDGM_EB_MAINFUNCTION_VIOLATION)
#error WDGM_EB_MAINFUNCTION_VIOLATION already defined
#endif
#if (WDGM_EB_EXT_MAINFUNCTION_VIOLATION_ENABLED == STD_ON)
#define WDGM_EB_MAINFUNCTION_VIOLATION() WDGM_EB_EXT_MAINFUNCTION_VIOLATION()
#else
#define WDGM_EB_MAINFUNCTION_VIOLATION()
#endif
...
#if (defined WDGM_EB_SET_BIT)
#error WDGM_EB_SET_BIT already defined
#endif
/* Internal macro to set one bit */
#define WDGM_EB_SET_BIT(x,y) ((x) = ((x) | (1U << (y))))

#if (defined WDGM_EB_CLR_BIT)
#error WDGM_EB_CLR_BIT already defined
#endif
/* Internal macro to clear one bit */
#define WDGM_EB_CLR_BIT(x,y) ((x) = (x) & (~(1U << (y))))

/*------------------[EB single/multi core specific macros]-------------------*/
#if (defined WDGM_EB_GET_GLOBAL_INIT_STATUS)
#error WDGM_EB_GET_GLOBAL_INIT_STATUS already defined
#endif
/* Internal macro to get the Global Initialization status */
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
#define WDGM_EB_GET_GLOBAL_INIT_STATUS() WdgM_EB_GetGlobalInitStatus()
#else
#define WDGM_EB_GET_GLOBAL_INIT_STATUS() WdgM_EB_InitStatus
#endif

#if (defined WDGM_EB_GET_SE_LOCAL_STATUS)
#error WDGM_EB_GET_SE_LOCAL_STATUS already defined
#endif
/* Internal macro to get the SE local status */
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
#define WDGM_EB_GET_SE_LOCAL_STATUS(IntSEId, ApiId) WdgM_EB_GetSELocalStatus(IntSEId, ApiId)
#else
#define WDGM_EB_GET_SE_LOCAL_STATUS(IntSEId, ApiId) WdgM_EB_SELocalStatus[IntSEId]
#endif

#if (WDGM_PARTITION_RESET == STD_ON)
/* Internal macro to trigger a partition reset */
#if (defined WDGM_EB_REQUEST_PARTITION_RESET)
#error WDGM_EB_REQUEST_PARTITION_RESET already defined
#endif
#if (WDGM_EB_EXT_REQUEST_PARTITION_RESET_ENABLED == STD_ON)
#define WDGM_EB_REQUEST_PARTITION_RESET(Application) WDGM_EB_EXT_REQUEST_PARTITION_RESET(Application)
#else
#define WDGM_EB_REQUEST_PARTITION_RESET(Application)
#endif

/* Internal macro to get the partition state */
#if (defined WDGM_EB_GET_APPLICATION_STATE)
#error WDGM_EB_GET_APPLICATION_STATE already defined
#endif
#if (WDGM_EB_EXT_GET_APPLICATION_STATE_ENABLED == STD_ON)
#define WDGM_EB_GET_APPLICATION_STATE(Application, Value) WDGM_EB_EXT_GET_APPLICATION_STATE(Application, Value)
#else
#define WDGM_EB_GET_APPLICATION_STATE(Application, Value)
#endif
#endif /* (WDGM_PARTITION_RESET == STD_ON) */
 ...
 *
 * \param[in] CoreInstance  Core instance specific data */
static FUNC(WdgM_GlobalStatusType, WDGM_CODE) WdgM_EB_GetNewGlobalStatus
(
  P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance
);
...
(
  ...
  VAR(uint8,AUTOMATIC) IndexInGraphDataSeq,
...
#if (WDGM_PARTITION_RESET == STD_ON)
/** \brief resets the Supervision Data for the mapped partition.
 *
 * \param[in] SECfg Pointer to Supervised Entity Configuration
 * \param[in] CoreInstance  Core instance specific data
 *
 * \retval void */
static FUNC(void, WDGM_CODE) WdgM_EB_SEPartitionResetData
(
  P2CONST(WdgM_EB_SECfgType,AUTOMATIC,WDGM_CONST) SECfg,
  P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance
);
...
#if (WDGM_PARTITION_RESET == STD_ON)
/** \brief Perform restart of a WdgM partition
 *
 * \param[in] IntSEId internal SE Id
 * \param[in] CoreInstance  Core instance specific data
 * \retval void
 */
static FUNC(void, WDGM_CODE) WdgM_EB_PartitionRestart
(
  WdgM_EB_SEIdType InternalSEId,
  P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance
);

/** \brief Synchronize alive supervision with the WdgM_MainFunction()
 *
 * \param[in] CoreInstance  Core instance specific data
 * \retval void
 */
static FUNC(void, WDGM_CODE) WdgM_EB_PartitionResSyncAliveSpv
(
  P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance
);
#endif
...
/* !LINKSTO WDGM.EB.DesignDecision.InternalVariable33,1 */
...
#if(WDGM_EB_ENABLE_SETMODE_SYNCHRON == STD_ON)
/* !LINKSTO WDGM.EB.DesignDecision.InternalVariable32,1 */
...
STATIC VAR(boolean, WDGM_VAR) WdgM_EB_SatteliteWdgIfSetModeFailed = FALSE;
#endif
...
#endif /* (WDGM_EB_MULTICORE_ENABLED == STD_ON) || (WDGM_EB_ENABLE_SETMODE_SYNCHRON == STD_ON) */
...
/* !LINKSTO WDGM.EB.DesignDecision.InternalVariable20,1 */
...
)
{
  ...
  WdgM_EB_ExpectedInitState = WDGM_EB_INIT_STATUS_INIT;
  WdgM_EB_ExpectedWdgMModeId = WdgM_EB_InitialModeId;
...
)
{
  ...
  if ((WDGM_EB_INIT_STATUS_DEINIT == WDGM_EB_GET_GLOBAL_INIT_STATUS())
        && (WDGM_EB_INIT_STATUS_DEINIT == WdgM_EB_ExpectedInitState))
  {
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
  ...
  else
  {
    ...
    WdgM_EB_ExpectedInitState = WDGM_EB_INIT_STATUS_DEINIT;
  }
...
#endif
{
  ...
  WdgM_EB_InitStatusType InitStatus = WDGM_EB_GET_GLOBAL_INIT_STATUS();
...
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
#if (WDGM_EB_MULTICORE_ENABLED == TRUE)
  if (WDGM_EB_MASTER_INSTANCE != WDGM_EB_GET_CORE_ID())
  {
    /* !LINKSTO WDGM.EB.SatelliteCall.DetError,1 */
    WDGM_EB_DET(WDGM_SID_SET_MODE, WDGM_E_PARAM_WRONG_CORE_ID);
    RetVal = E_NOT_OK;
  }
  else
#endif /* WDGM_EB_MULTICORE_ENABLED == TRUE */
  ...
  if ((WDGM_EB_INIT_STATUS_INIT == WDGM_EB_GET_GLOBAL_INIT_STATUS())
      && (WDGM_EB_INIT_STATUS_DEINIT == WdgM_EB_ExpectedInitState))
  {
...
#endif
  {
     ...
     * WdgM_EB_InitStatus != WDGM_EB_INIT_STATUS_DEINIT)*/
    if (WDGM_EB_INIT_STATUS_DEINIT == WDGM_EB_GET_GLOBAL_INIT_STATUS())
    ...
    else if (((WDGM_GLOBAL_STATUS_OK != WdgM_EB_GlobalStatus) &&
              (WDGM_GLOBAL_STATUS_FAILED != WdgM_EB_GlobalStatus)) &&
              (WDGM_EB_INIT_STATUS_INIT == InitStatus))
    {
      RetVal = E_NOT_OK;
    }
    else
    {
      ...
      WdgM_EB_ExpectedWdgMModeId = Mode;
...
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
#if (WDGM_EB_MULTICORE_ENABLED == TRUE)
  if (WDGM_EB_MASTER_INSTANCE != CoreInstance->CoreID)
  {
    /* !LINKSTO WDGM.EB.SatelliteCall.DetError,1 */
    WDGM_EB_DET(WDGM_SID_GET_MODE, WDGM_E_PARAM_WRONG_CORE_ID);
    RetVal = E_NOT_OK;
  }
  else
#endif /* WDGM_EB_MULTICORE_ENABLED == TRUE */
  /* !LINKSTO WDGM.ASR40.WDGM253,1,WDGM.ASR40.WDGM004.1,1 */
  if (*(CoreInstance->WdgM_EB_InitStatus) == WDGM_EB_INIT_STATUS_DEINIT)
  {
...
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
    /* read the current WdgM mode for each core */
    *Mode = *(CoreInstance->WdgM_EB_CoreSwitchMode);
#else
      /* !LINKSTO WDGM.ASR40.WDGM170,1 */
    *Mode = CoreInstance->WdgM_EB_CurrentModeId;
#endif
    RetVal = E_OK;
  }
...
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
    if (WDGM_LOCAL_STATUS_DEACTIVATED == CoreInstance->WdgM_EB_SELocalStatus[WdgM_EB_SELocalCoreIdx[IntSEId]])
#else
   if (WDGM_LOCAL_STATUS_DEACTIVATED == CoreInstance->WdgM_EB_SELocalStatus[IntSEId])
#endif
    {
...
#elif (WDGM_PARTITION_RESET == STD_ON)
          CONST(uint8, WDGM_CONST) PartitionIndex = WdgM_EB_GetPartitionIndex(CoreInstance, SECfg->OsPartitionRef);
          /* !LINKSTO WDGM.EB.DesignDecision.PartitionSupervisionActive,1 */
          if ((SECfg->PartitionStatus == NULL_PTR) ||
              (SECfg->PartitionStatus[PartitionIndex] == WDGM_EB_PARTITION_ACTIVE))
#endif
          {
...
)
{
...
#endif
  {
    ...
    else
    {
      ...
      *Status = WDGM_EB_GET_SE_LOCAL_STATUS(IntSEId, WDGM_SID_GET_LOCAL_STATUS);
      RetVal = E_OK;
    }
  }
...
)
{
  ...
  {
    /* !LINKSTO WDGM.EB.SatelliteCall.DetError,1 */
    WDGM_EB_DET(WDGM_SID_GET_GLOBAL_STATUS, WDGM_E_PARAM_WRONG_CORE_ID);
    RetVal = E_NOT_OK;
  }
  else
#endif /* WDGM_EB_MULTICORE_ENABLED == TRUE */
...
FUNC(void, WDGM_CODE) WdgM_PerformReset(void)
{
  ...
  {
    /* !LINKSTO WDGM.EB.SatelliteCall.DetError,1 */
    WDGM_EB_DET(WDGM_SID_PERFORM_RESET, WDGM_E_PARAM_WRONG_CORE_ID);
  }
  else
#endif /* WDGM_EB_MULTICORE_ENABLED == TRUE */
...
#endif
  {
...
#endif
    {
      ...
      WdgM_EB_GlobalStatus = WDGM_GLOBAL_STATUS_STOPPED;
...
)
{
  ...
  {
    /* !LINKSTO WDGM.EB.SatelliteCall.DetError,1 */
    WDGM_EB_DET(WDGM_SID_GET_FIRST_EXPIRED_SEID, WDGM_E_PARAM_WRONG_CORE_ID);
    RetVal = E_NOT_OK;
  }
  else
#endif /* WDGM_EB_MULTICORE_ENABLED == TRUE */
...
#endif
  {
    ...
    RetVal = WdgM_EB_GetCheckedExpiredSEID (WdgM_EB_ExpiredSEID, &LocalSEId);
    *SEID = LocalSEId;
  }
...
{
#if (WDGM_PARTITION_RESET == STD_ON)
  ApplicationType CorePartitionIdx;
#endif
  ...
  else
  {
...
#if(WDGM_TRIGGER_WDG_DRIVER_MULTICORE_ENABLED == STD_ON)
    if((CoreInstance->CoreID == WDGM_EB_MASTER_INSTANCE) && (WdgM_EB_SatteliteWdgIfSetModeFailed == TRUE))
    {
      /* A satellite has signaled an error when trying to change the mode of the wdg drivers which are triggered
         by the satellite. If a mode switch has failed this is treated as a fatal error and a reset is called. */
      /* !LINKSTO WDGM.EB.MulticoreSetWatchdog4,1 */
      WdgM_PerformReset();
    }
#endif

    {
      ...
      const uint8 DoNotCheckInitRequest = *(CoreInstance->WdgM_EB_InitStatus);

      /* check whether WdgM shall be deinitialized */
      if (*(CoreInstance->WdgM_EB_InitStatus) == WDGM_EB_INIT_STATUS_INIT)
      {
...
#if (WDGM_PARTITION_RESET == STD_ON)
      WdgM_EB_PartitionResSyncAliveSpv(CoreInstance);
#endif
       ...
       * Note: *(CoreInstance->WdgM_EB_InitStatus) != WDGM_EB_INIT_STATUS_DEINIT implies that
      ...
      if (*(CoreInstance->WdgM_EB_InitStatus) == WDGM_EB_INIT_STATUS_INIT)
      {
...
#endif
        {
          ...
          /* !LINKSTO WDGM.EB.MulticoreGlobalStatus,2 */
      ...
      if (DoNotCheckInitRequest == WDGM_EB_INIT_STATUS_DEINIT)
      {
...
#if (WDGM_PARTITION_RESET == STD_ON)
          for (CorePartitionIdx = 0U; CorePartitionIdx < CoreInstance->NumOsPartitions; CorePartitionIdx++)
          {
            /* At initialization, reset data for the partition restart feature */
            CoreInstance->WdgM_EB_PartitionSupervisionCycleCntPtr[CorePartitionIdx] = 0U;

            /* !LINKSTO WDGM.EB.DesignDecision.PartitionInitial,1 */
            WdgM_EB_SetPartitionStatus(CoreInstance->CoreID, CorePartitionIdx, WDGM_EB_PARTITION_ACTIVE);
          }
#endif
        }
      }
      ...
      for (CorePartitionIdx = 0U; CorePartitionIdx < CoreInstance->NumOsPartitions; CorePartitionIdx++)
      {
        if (WdgM_EB_GetPartitionStatus(CoreInstance->CoreID, CorePartitionIdx) == WDGM_EB_PARTITION_POST_RESTARTED)
        {
          /* Reset counters for all partitions in state POST_RESTARTED */
          CoreInstance->WdgM_EB_PartitionSupervisionCycleCntPtr[CorePartitionIdx] = 0U;

          /* !LINKSTO WDGM.EB.DesignDecision.PartitionActive,1 */
          WdgM_EB_SetPartitionStatus(CoreInstance->CoreID, CorePartitionIdx, WDGM_EB_PARTITION_ACTIVE);
        }
      }
#endif
...
)
{
  ...
  /* !LINKSTO WDGM.EB.TIMEPM.WDGM020450,2 */
   ...
   *(CoreInstance->WdgM_EB_InitStatus) = WDGM_EB_INIT_STATUS_INIT;
  ...
  if (RetVal == E_OK)
  {
...
#endif
    {
      ...
      /* !LINKSTO WDGM.ASR40.WDGM223,1,WDGM.EB.MulticoreTriggerWatchdog,2 */
      WdgM_EB_UpdateTriggerConditions(CoreInstance);
    }
#if(WDGM_TRIGGER_WDG_DRIVER_MULTICORE_ENABLED == STD_ON)
    else
...
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
    *(CoreInstance->WdgM_EB_CoreSwitchMode) = WdgM_EB_InitialModeId;
#endif
...
)
{
  ...
  if (E_OK == RetVal)
  {
#if((WDGM_EB_MULTICORE_ENABLED == STD_ON) && (WDGM_TRIGGER_WDG_DRIVER_MULTICORE_ENABLED == STD_OFF))
    if (CoreInstance->CoreID == WDGM_EB_MASTER_INSTANCE)
#endif
    {
...
#endif
  {
    const WdgM_GlobalStatusType OldGlobalStatus = WdgM_EB_GlobalStatus;

    /* Reset global status and flags independent of actual global state */
    /* !LINKSTO WDGM.EB.TIMEPM.WDGM020435,1 */
    WdgM_EB_GlobalStatus = WDGM_GLOBAL_STATUS_DEACTIVATED;
   ...
   * instance has been performed */
  *(CoreInstance->WdgM_EB_InitStatus) = WDGM_EB_INIT_STATUS_DEINIT;
...
)
{
...
#endif
  {
    ...
    if (((WDGM_GLOBAL_STATUS_OK != WdgM_EB_GlobalStatus) &&
        (WDGM_GLOBAL_STATUS_FAILED != WdgM_EB_GlobalStatus)) &&
...
#endif
      {
...
#endif
        {
          ...
          CONSTP2VAR(WdgM_LocalStatusType,AUTOMATIC,AUTOMATIC) SEWData_SELocalStatus =
              &(CoreInstance->WdgM_EB_SELocalStatus[WdgM_EB_SELocalCoreIdx[IntSEId]]);
#else
          CONSTP2VAR(WdgM_LocalStatusType,AUTOMATIC,AUTOMATIC) SEWData_SELocalStatus =
              &WdgM_EB_SELocalStatus[IntSEId];
#endif
          ...
          if (SEIsActive)
          {
            ...
            if ((WDGM_LOCAL_STATUS_DEACTIVATED == *SEWData_SELocalStatus)
                  || (FALSE == IsNormalOperation))
              {
                *SEWData_SELocalStatus = WDGM_LOCAL_STATUS_OK;
          ...
          else
          {
            ...
            /* Note: In case IsNormalOperation is set to FALSE, then old Local Status is ignored. */
            if (((WDGM_LOCAL_STATUS_OK == (*SEWData_SELocalStatus)) ||
                  (WDGM_LOCAL_STATUS_FAILED == *SEWData_SELocalStatus)) ||
                  (FALSE == IsNormalOperation))
            {
              const WdgM_LocalStatusType OldLocalStatus = *SEWData_SELocalStatus;
              /* !LINKSTO WDGM.EB.TIMEPM.WDGM020427,1 */
              *SEWData_SELocalStatus = WDGM_LOCAL_STATUS_DEACTIVATED;
              /* !LINKSTO WDGM.ASR40.WDGM197,1,WdgM.ASR43.SWS_WdgM_00197,1 */
              /* status changes because of mode change, signal via callout and/or RTE */
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
...
#endif
      {
        /* !LINKSTO WDGM.EB.TIMEPM.WDGM020424,1,WDGM.EB.MulticoreTriggerWatchdog,2,WDGM.ASR40.WDGM186,1 */
        /* !LINKSTO WDGM.EB.MulticoreSetWatchdog,2 */
...
)
{
  ...
  /* if there are no watchdog for this mode consider retVal as OK */
  if(0U == ModeCfg->NumTriggers)
  {
    RetVal = E_OK;
  }
  ...
  {
    /* !LINKSTO WDGM.EB.MulticoreSetWatchdog2,1, WDGM.EB.MulticoreSetWatchdog3,1 */
#if(WDGM_TRIGGER_WDG_DRIVER_MULTICORE_ENABLED == STD_ON)
    if ((TriggerCfg[TriggerId].TriggerFromAllCores == TRUE) || (TriggerCfg[TriggerId].WdgCoreId == CoreInstance->CoreID))
#endif
    {
      ...
      if (RetVal != E_OK)
      {
        ...
        if(CoreInstance->CoreID == WDGM_EB_MASTER_INSTANCE)
        {
          ...
          /* !LINKSTO WDGM.EB.TIMEPM.WDGM020424,1,WDGM.EB.TIMEPM.WDGM020450,2,WDGM.ASR40.WDGM186,1 */
          WdgM_PerformReset();
#if(WDGM_TRIGGER_WDG_DRIVER_MULTICORE_ENABLED == STD_ON)
        }
        else
        {
          WdgM_EB_SatteliteWdgIfSetModeFailed = TRUE;
        }
#endif
        break;
      }
...
)
{
...
#if (WDGM_PARTITION_RESET == STD_OFF)
   CPRefCycleCnt = (uint32)
  ...
  {
    CPRefCycleCnt = (uint32)
       (CoreInstance->WdgM_EB_PartitionSupervisionCycleCntPtr[WdgM_EB_GetPartitionIndex(CoreInstance, SECfg->OsPartitionRef)] % (CPAliveCfg->SupervisionReferenceCycle));
  }
#endif
...
)
{
...
#endif
  {
    ...
    if (WDGM_LOCAL_STATUS_DEACTIVATED != CoreInstance->WdgM_EB_SELocalStatus[WdgM_EB_SELocalCoreIdx[IntSEId]])
#else
    /* !LINKSTO WDGM.EB.DesignDecision.PartitionSupervisionActive,1,WDGM.EB.DesignDecision.PartitionSupervisionRestart,1 */
    if ((WDGM_LOCAL_STATUS_DEACTIVATED != CoreInstance->WdgM_EB_SELocalStatus[WdgM_EB_SELocalCoreIdx[IntSEId]]) &&
        ((SECfg->PartitionStatus == NULL_PTR) ||
         (SECfg->PartitionStatus[PartitionIndex] != WDGM_EB_PARTITION_RESTARTING)))
#endif
#else
#if (WDGM_PARTITION_RESET == STD_OFF)
   if (WDGM_LOCAL_STATUS_DEACTIVATED != CoreInstance->WdgM_EB_SELocalStatus[IntSEId])
      ...
      for (CPId = 0U; CPId < SECfg->NumOfCPs; CPId++)
      {
...

        {
          ...
          if (CPAliveCfg != NULL_PTR)
          {
            ...
            if (WdgM_EB_TmpDisableAliveSupv == FALSE)
#elif (WDGM_PARTITION_RESET == STD_ON)
            /* !LINKSTO WDGM.EB.DesignDecision.PartitionSupervisionPost,1 */
            if ((SECfg->PartitionStatus == NULL_PTR) ||
                (SECfg->PartitionStatus[PartitionIndex] != WDGM_EB_PARTITION_POST_RESTARTED))
#endif
            {
...
)
{
  ...
  if (WdgM_EB_SECfg[InternalSEId].OsPartitionRefEnabled == TRUE)
  {
    ApplicationType FailedOsApp = WdgM_EB_SECfg[InternalSEId].OsPartitionRef;
    WdgM_EB_SEIdType IntSEId = 0U;
    Std_ReturnType ExtRetVal = E_NOT_OK;

    /* !LINKSTO WDGM.EB.TIMEPM.WDGM020225,1 */
    ExtRetVal = WDGM_EB_REQUEST_PARTITION_RESET(FailedOsApp);
    WDGM_PRECONDITION_ASSERT((ExtRetVal == E_OK) || (ExtRetVal == E_NOT_OK),WDGM_SID_MAIN_FUNCTION);

    if(E_OK == ExtRetVal)
    {
      /* !LINKSTO WDGM.EB.DesignDecision.PartitionRestart,1 */
      WdgM_EB_SetPartitionStatus(CoreInstance->CoreID, WdgM_EB_GetPartitionIndex(CoreInstance, FailedOsApp), WDGM_EB_PARTITION_RESTARTING);

      /* Reset data for all SEs mapped to the failed partition */
#if (WDGM_EB_SE_NUM != 1U)
      for (IntSEId = 0U; IntSEId < WDGM_EB_SE_NUM; IntSEId++)
#endif
      {
...
#endif
        {
          if ((WdgM_EB_SECfg[IntSEId].OsPartitionRefEnabled == TRUE) && (WdgM_EB_SECfg[IntSEId].OsPartitionRef == FailedOsApp))
          {
            CONSTP2VAR(uint16,AUTOMATIC,AUTOMATIC) SEWData_FailedAliveSupervisionRefCycleCnt =
                &(CoreInstance->WdgM_EB_FailedAliveSupervisionRefCycleCnt[IntCoreSEId]);

            /* !LINKSTO WDGM.EB.DesignDecision.PartitionResetData,1 */
            WdgM_EB_SEPartitionResetData(SECfg, CoreInstance);

            /* Reset all local alive supervision related data of the SE.*/
            WdgM_EB_SEResetCPAliveData(IntSEId, IntRamIdxBase, TRUE, CoreInstance);
...
/*------------------[WdgM_EB_PartitionResSyncAliveSpv]---------------------------------------------*/
static FUNC(void, WDGM_CODE) WdgM_EB_PartitionResSyncAliveSpv
(
  P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance
)
{
  ApplicationType CorePartitionIdx;
  ApplicationStateType OsPartitionState = APPLICATION_TERMINATED;
  uint8 SEidx;
  StatusType ExtRetVal = E_NOT_OK;

  /* Synchronize alive supervision with the WdgM_MainFunction(). */
  for (CorePartitionIdx = 0U; CorePartitionIdx < CoreInstance->NumOsPartitions; CorePartitionIdx++)
  {
    if (WdgM_EB_GetPartitionStatus(CoreInstance->CoreID, CorePartitionIdx) == WDGM_EB_PARTITION_RESTARTING)
    {
      /* !LINKSTO WDGM.EB.TIMEPM.WDGM020226,1 */
      ExtRetVal = WDGM_EB_GET_APPLICATION_STATE(WdgM_EB_GetPartitionId(CoreInstance->CoreID, CorePartitionIdx), &OsPartitionState);
      WDGM_PRECONDITION_ASSERT((ExtRetVal == E_OK) || (ExtRetVal == E_NOT_OK),WDGM_SID_MAIN_FUNCTION);
      WDGM_PRECONDITION_ASSERT((OsPartitionState == APPLICATION_ACCESSIBLE) ||
          (OsPartitionState == APPLICATION_RESTARTING) ||
          (OsPartitionState == APPLICATION_TERMINATED),
          WDGM_SID_MAIN_FUNCTION);

      if ((ExtRetVal == E_OK) && (OsPartitionState == APPLICATION_ACCESSIBLE))
      {
        uint8 LocalSize;
        CONSTP2VAR(WdgM_EB_SEIdType, AUTOMATIC, WDGM_APPL_CONST) WdgM_EB_SE_AppPtr = WdgM_EB_GetSEfromOsApp(WdgM_EB_GetPartitionId(CoreInstance->CoreID, CorePartitionIdx), &LocalSize);

        VAR(WdgM_EB_SEIdType, WDGM_VAR) SEIndex;

        for (SEidx = 0U; SEidx < LocalSize; SEidx++)
        {
#if(WDGM_EB_MULTICORE_ENABLED == STD_OFF)
          SEIndex = WdgM_EB_SE_AppPtr[SEidx];
#else
          SEIndex = WdgM_EB_SELocalCoreIdx[WdgM_EB_SE_AppPtr[SEidx]];
#endif
          CoreInstance->WdgM_EB_SELocalStatus[SEIndex] = (uint8)WDGM_LOCAL_STATUS_OK;
        }

        /* !LINKSTO WDGM.EB.DesignDecision.PartitionPost,1 */
        WdgM_EB_SetPartitionStatus(CoreInstance->CoreID, CorePartitionIdx, WDGM_EB_PARTITION_POST_RESTARTED);
      }
    }
  }
}
...
)
{
  ...
  WdgM_LocalStatusType NewLocalStatus = *SEWData_SELocalStatus;
  ...
  switch (*SEWData_SELocalStatus)
  {
    ...
    case WDGM_LOCAL_STATUS_FAILED:
    {
      ...
      {
        WdgM_EB_ExpiredSEIDType ExpiredSEID = WdgM_EB_ExpiredSEID;

#if (WDGM_EB_GET_ALL_EXPIRED_SE == STD_ON)

        /* !LINKSTO WDGM.EB.GetAllExpiredSEID.MainFunction.Write, 1 */
        if (*CoreInstance->WdgM_EB_ExpiredSEIDsCounter == 0U)
        {
          *CoreInstance->WdgM_EB_ExpiredSEIDsCounter = 1U;
          CoreInstance->WdgM_EB_ExpiredSEIDs[0U] = IntSEId;
        }
        else
        {
          uint8 LocalIndex;
          boolean IsSEIDFound = FALSE;

          for (LocalIndex = 0U; LocalIndex < CoreInstance->WdgM_EB_ExpiredSEIDArraySize; LocalIndex++)
          {
            if (IntSEId == CoreInstance->WdgM_EB_ExpiredSEIDs[LocalIndex])
            {
              IsSEIDFound = TRUE;
              break;
            }
          ...
          if (IsSEIDFound == FALSE)
          {
            CoreInstance->WdgM_EB_ExpiredSEIDs[*CoreInstance->WdgM_EB_ExpiredSEIDsCounter] = IntSEId;
            *CoreInstance->WdgM_EB_ExpiredSEIDsCounter += 1U;
          }
        }
#endif /* #if (WDGM_EB_GET_ALL_EXPIRED_SE == STD_ON) */
        ...
        WdgM_EB_ExpiredSEID = ExpiredSEID;
      }

#if (WDGM_PARTITION_RESET == STD_ON)
      /* !LINKSTO WDGM.EB.WDGM225.3,1 */
      if ((NewLocalStatus == WDGM_LOCAL_STATUS_FAILED) && (PartitionReset == TRUE))
      {
        WdgM_EB_PartitionRestart(IntSEId, CoreInstance);
      }
#endif
  ...
  if (*SEWData_SELocalStatus != NewLocalStatus)
  {
    WdgM_LocalStatusType OldLocalStatus = *SEWData_SELocalStatus;
    ...
    *SEWData_SELocalStatus = NewLocalStatus;
    ...
    /* signal via callout and/or RTE */
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
    WDGM_EB_INDIVIDUAL_MODE_SWITCH(SECfg->SEId, OldLocalStatus, NewLocalStatus, SECfg->CoreID);
#else
...
{
  const WdgM_GlobalStatusType OldGlobalStatus = WdgM_EB_GlobalStatus;
  const WdgM_GlobalStatusType NewGlobalStatus = WdgM_EB_GetNewGlobalStatus(CoreInstance);
...
)
{
  ...
  WdgM_GlobalStatusType NewGlobalStatus = WdgM_EB_GlobalStatus;
...
#endif
  {
    ...
    if (WdgM_EB_GetCoreSwitchMode(SECfg->CoreID) == WdgM_EB_GetCoreSwitchMode(WDGM_EB_MASTER_INSTANCE))
#endif
    {
      switch (WDGM_EB_GET_SE_LOCAL_STATUS(IntSEId, WDGM_SID_MAIN_FUNCTION))
      {
        case WDGM_LOCAL_STATUS_FAILED:
          NumSEFailed++;
          break;
...
)
{
  ...
  for (CorePartitionIdx = 0U; CorePartitionIdx < CoreInstance->NumOsPartitions; CorePartitionIdx++)
  {
    CoreInstance->WdgM_EB_PartitionSupervisionCycleCntPtr[CorePartitionIdx] += 1U;
    CoreInstance->WdgM_EB_PartitionSupervisionCycleCntPtr[CorePartitionIdx] %= ModeCfg->MaxSupervisionCycleCounter;
  }
#endif
...
#if ((WDGM_EB_EXT_GET_TIME_ENABLED == STD_ON) && (WDGM_EB_EXT_MAINFUNCTION_VIOLATION_ENABLED == STD_ON))
      /* !LINKSTO WDGM.EB.TIMEPM.WDGM020485,1 */
      WDGM_EB_MAINFUNCTION_VIOLATION();
#endif
    }
  }
...
{
  Std_ReturnType RetVal;
  const uint16 UncheckedIsActiveData = (uint16)GraphData->IsActiveData;
  const uint32 UncheckedLastCPData = (uint32)GraphData->LastCPData;
...
{
  Std_ReturnType RetVal;
  const uint16 UncheckedLastSeqNumber = IntExtGraphDataSeqPtr[IndexInGraphDataSeq];
...
)
{
  ...
  IntExtGraphDataSeqPtr[IndexInGraphDataSeq] = LastSeqNumber;
...
)
{
  ...
  /* !LINKSTO WDGM.EB.Dbg4,1 */
  DBG_WDGM_EB_SEISACTIVEPARTITIONRESETDATA_ENTRY(SECfg,CoreInstance);
...
#if (WDGM_EB_LOGICAL_SUPERVISION_ENABLED == STD_ON)
    {
      CONSTP2CONST(WdgM_EB_CPLogicalCfgType,AUTOMATIC,WDGM_CONST) CPIntLogicalCfgMode =
          CPCfg->IntLogicalCfgPtr[CoreInstance->WdgM_EB_CurrentModeId];

      if (CPIntLogicalCfgMode != NULL_PTR)
      {
          const WdgM_EB_GraphIdType GraphId = CPIntLogicalCfgMode->GraphId;
          CONSTP2VAR(WdgM_EB_GraphDataType,AUTOMATIC,WDGM_VAR) GraphData =
              &(WdgM_EB_InternalGraphData[GraphId]);
          /* !LINKSTO WDGM.EB.ResetActiveFlags3,1,WDGM.EB.ResetActiveFlags2,1 */
          WdgM_EB_StoreGraphData (GraphData, 0U, 0U, FALSE);
...
#if (WDGM_EB_EXTERNAL_GRAPH_NUM > 0U)
    {
      CONSTP2CONST(WdgM_EB_CPLogicalCfgType,AUTOMATIC,WDGM_CONST) CPExtLogicalCfgMode =
          CPCfg->ExtLogicalCfgPtr[CoreInstance->WdgM_EB_CurrentModeId];

      if (CPExtLogicalCfgMode != NULL_PTR)
      {
        const WdgM_EB_GraphIdType GraphId = CPExtLogicalCfgMode->GraphId;
        CONSTP2VAR(WdgM_EB_GraphDataType,AUTOMATIC,WDGM_VAR) GraphData =
        &(WdgM_EB_ExternalGraphData[GraphId]);
...
#if (WDGM_EB_DEADLINE_SUPERVISION_ENABLED == STD_ON)
    {
      CONSTP2CONST(WdgM_EB_CPDeadlineCfgType,AUTOMATIC,WDGM_CONST) CPDeadlineCfgMode =
          CPCfg->DeadlineCfgPtr[CoreInstance->WdgM_EB_CurrentModeId];

      /* !LINKSTO WDGM.ASR40.WDGM314,1 */
      if (CPDeadlineCfgMode != NULL_PTR)
      {
        const WdgM_EB_DMIdType DMId = CPDeadlineCfgMode->DMId;
        CONSTP2VAR(WdgM_EB_DMDataType,AUTOMATIC,WDGM_VAR) DMData =
            &(WdgM_EB_DMData[DMId]);
        const WdgM_EB_TimestampType TimeStamp = 0U;
  ...
  /* !LINKSTO WDGM.EB.Dbg4,1 */
  DBG_WDGM_EB_SEISACTIVEPARTITIONRESETDATA_EXIT(IsActive,SECfg,CoreInstance);
}
#endif
...
)
{
  ...
  for (TriggerId = 0U; TriggerId < ModeCfg->NumTriggers; TriggerId++)
  {
    /* !LINKSTO WDGM.EB.MulticoreTriggerWatchdog2,1, WDGM.EB.MulticoreTriggerWatchdog3,1, WDGM.EB.MulticoreSetWatchdog2,1*/
#if(WDGM_TRIGGER_WDG_DRIVER_MULTICORE_ENABLED == STD_ON)
    if ((TriggerCfg[TriggerId].TriggerFromAllCores == TRUE) || (TriggerCfg[TriggerId].WdgCoreId == CoreInstance->CoreID))
#endif
    {
      ...
      if (TriggerCfg[TriggerId].WatchdogMode != WDGIF_OFF_MODE)
      {
        ...
        if (WdgM_EB_GlobalStatus == WDGM_GLOBAL_STATUS_STOPPED)
        {
...
)
{
...

  {
...
#endif
    {
      ...
      WdgM_EB_ExpectedInitState = ExpectedInitState;
    }
    else
    {
      /* Load the expected init state from the shared memory. */
      ExpectedInitState = WdgM_EB_ExpectedInitState;
...
#else
    if (WDGM_EB_INIT_STATUS_INIT == WdgM_EB_ExpectedInitState)
#endif
    {
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
      if (CoreInstance->CoreID == WDGM_EB_MASTER_INSTANCE)
      ...
      {
        if(WDGM_EB_GET_MASTER_STATUS == WDGM_EB_INIT_STATUS_INIT)
          {
...
#else
    if (WDGM_EB_INIT_STATUS_DEINIT == WdgM_EB_ExpectedInitState)
#endif
    {
#if (WDGM_EB_DEINIT_MODE_ENABLED == STD_ON)
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
...
)
{
...

  {
...
#if (WDGM_EB_EXT_GET_EXPECTED_WDGM_MODE_ENABLED == STD_ON)
      ExpectedWdgMMode = CoreInstance->WdgM_EB_CurrentModeId;
...
#else
      ExpectedWdgMMode = WdgM_EB_ExpectedWdgMModeId;
#endif
    }
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
    else
    {
      ...
      ExpectedWdgMMode = WdgM_EB_GetCoreSwitchMode(WDGM_EB_MASTER_INSTANCE);
#if (WDGM_EB_EXT_GET_EXPECTED_WDGM_MODE_ENABLED == STD_ON)
...
#endif
    {
      ...
      {
        /* For satellite in case the E_NOT_OK value is returned then nothing to be done */
        if (CoreInstance->CoreID == WDGM_EB_MASTER_INSTANCE)
        {
#if (WDGM_EB_EXT_GET_EXPECTED_WDGM_MODE_ENABLED == STD_OFF)
          /* if callouts are not used,
           * reset request to old state (Never-give-up strategy makes no sense here */
          WdgM_EB_ExpectedWdgMModeId = CoreInstance->WdgM_EB_CurrentModeId;
#endif
      ...
      else
      {
        ...
        * instance has been performed */
        *(CoreInstance->WdgM_EB_CoreSwitchMode) = ExpectedWdgMMode;
#endif
      }
    }
  }
...
static FUNC(void, WDGM_CODE) WdgM_EB_CheckModeSwitchTimeFrame (void)
{
  ...
  if((WDGM_EB_INIT_STATUS_INIT == WDGM_EB_GET_MASTER_STATUS)
    && (WDGM_EB_INIT_STATUS_DEINIT == InitStatus))
  {
  ...
  else
  {
    ...
    AllCoresSameMode = WdgM_EB_AllCoresSameMode();
  }
...
/* !LINKSTO WDGM.EB.GetAllExpiredSEID.Availability, 1 */
#if (WDGM_EB_GET_ALL_EXPIRED_SE == STD_ON)
/* !LINKSTO WDGM.EB.GetAllExpiredSEID.API, 1 */
FUNC(Std_ReturnType, WDGM_CODE) WdgM_GetAllExpiredSEID
(
    P2VAR(uint8, AUTOMATIC, WDGM_APPL_DATA) ExpiredSEID,
    P2VAR(uint8, AUTOMATIC, WDGM_APPL_DATA) NoOfExpiredSEID
)
{
...
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
  /* !LINKSTO WDGM.EB.GetAllExpiredSEID.Params.NULL_PTR, 1 */
  if ( (ExpiredSEID == NULL_PTR) || (NoOfExpiredSEID == NULL_PTR) )
  {
    /* !LINKSTO WDGM.EB.GetAllExpiredSEID.Params.DetReport, 1 */
    WDGM_EB_DET(WDGM_SID_GET_ALL_EXPIRED_SEID, WDGM_E_INV_POINTER);
  }
  else
#endif /* #if (WDGM_DEV_ERROR_DETECT == STD_ON) */
  {
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
    /* !LINKSTO WDGM.EB.GetAllExpiredSEID.MasterInstanceOnly, 1 */
    /* !LINKSTO WDGM.EB.GetAllExpiredSEID.SatelliteCall.Return, 1 */
    if (CoreInstance->CoreID == WDGM_EB_MASTER_INSTANCE)
#endif
    {
      WdgM_EB_GetAllExpiredSEIDsHelper(NoOfExpiredSEID, ExpiredSEID);

      /* !LINKSTO WDGM.EB.GetAllExpiredSEID.CoreSpecific.Reset, 1 */
      if (*NoOfExpiredSEID >= 1U)
      {
        WdgM_EB_ResetAllExpiredSEIDs();
      }
...
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
      /* !LINKSTO WDGM.EB.GetAllExpiredSEID.SatelliteCall.DetError, 1 */
      WDGM_EB_DET(WDGM_SID_GET_ALL_EXPIRED_SEID, WDGM_E_PARAM_WRONG_CORE_ID);
#endif /* #if (WDGM_DEV_ERROR_DETECT == STD_ON) */
    }
#endif /* #if(WDGM_EB_MULTICORE_ENABLED == STD_OFF) */
  }
