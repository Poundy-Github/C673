...
[!IF "CanIfMultiCoreSupport = 'true'"!]
  [!IF "CanIfPublicReadTxPduNotifyStatusApi = 'true' or CanIfPublicReadRxPduNotifyStatusApi = 'true'"!]
    [!FOR "i" = "0" TO "num:i(count(../CanIfCtrlDrvCfg/*/CanIfCtrlCfg/*)) - 1"!]
      [!IF "CanIfPublicReadTxPduNotifyStatusApi = 'true'"!]

/** \brief Bit-array of Tx notification flags (if enabled) */
STATIC VAR( uint8, CANIF_VAR )
  CanIf_TxLPduNotifyFlags_[!"$i"!][([!IF "node:exists(../CanIfCtrlDrvCfg/CanIfCtrlMaxTxNotifyPdus)"!][!"../CanIfCtrlDrvCfg/CanIfCtrlMaxTxNotifyPdus"!][!ELSE!]1[!ENDIF!]U+7U)/8U];
      [!ENDIF!]

      [!IF "CanIfPublicReadRxPduNotifyStatusApi = 'true'"!]
/** \brief Bit-array of Rx notification flags (if enabled) */
STATIC VAR( uint8, CANIF_VAR )
  CanIf_RxLPduNotifyFlags_[!"$i"!][([!IF "node:exists(../CanIfCtrlDrvCfg/CanIfCtrlMaxRxNotifyPdus)"!][!"../CanIfCtrlDrvCfg/CanIfCtrlMaxRxNotifyPdus"!][!ELSE!]1[!ENDIF!]U+7U)/8U];
      [!ENDIF!]
    [!ENDFOR!]

    [!IF "CanIfPublicReadTxPduNotifyStatusApi = 'true'"!]
VAR(CanIf_TxLPduNotifyFlagType, CANIF_VAR) CanIf_ControllerTxLPduNotifyFlag[CANIF_CTRL_SIZE] =
{
      [!FOR "i" = "0" TO "num:i(count(../CanIfCtrlDrvCfg/*/CanIfCtrlCfg/*)) - 1"!]
  {
    &CanIf_TxLPduNotifyFlags_[!"$i"!][0U]
  },
      [!ENDFOR!]
};
    [!ENDIF!]
    ...
    [!IF "CanIfPublicReadRxPduNotifyStatusApi = 'true'"!]
VAR(CanIf_RxLPduNotifyFlagType, CANIF_VAR) CanIf_ControllerRxLPduNotifyFlag[CANIF_CTRL_SIZE] =
{
      [!FOR "i" = "0" TO "num:i(count(../CanIfCtrlDrvCfg/*/CanIfCtrlCfg/*)) - 1"!]
  {
    &CanIf_RxLPduNotifyFlags_[!"$i"!][0U]
  },
      [!ENDFOR!]
};
    [!ENDIF!]
