 ...
 *
 * Reason:
 * In the context of "for(SrcIterator = XCPR_NO_OF_RX_SRC_PDU_IDS;
 *                        SrcIterator < XCPR_NO_OF_SRC_PDU_IDS; SrcIterator++)"
 * when "XCPR_NO_OF_RX_SRC_PDU_IDS = SrcIterator" the message condition is always
 * false is expected;
 * Expression "SrcIterator < XCPR_NO_OF_SRC_PDU_IDS" is always evaluated as false
 * if "SrcIterator = XCPR_NO_OF_SRC_PDU_IDS".
 *
 ...
 * Expression "Iterator < XCPR_NO_OF_TCPIP_SOCKET_CON" is always evaluated as true
 * if "XCPR_NO_OF_TCPIP_SOCKET_CON = 1".
 */
