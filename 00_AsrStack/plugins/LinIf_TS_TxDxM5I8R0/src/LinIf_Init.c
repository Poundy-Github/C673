   ...
   /* check whether parameter LocalConfigPtr is a NULL_PTR */
...
#endif /* (LINIF_DEV_ERROR_DETECT == STD_ON) */
    {
        ...
        /* now initialize master channels */
            if (LINIF_CH_IS_MASTER == (LINIF_CFG_GET_CHANNEL[Channel].CfgFlags & LINIF_NODETYPE_MASK))
            {
                CONSTP2VAR(LinIf_MasterChannelInfoType, AUTOMATIC, LINIF_VAR) pChannelMaster = &LinIf_MasterChannelInfo[Channel];

                const uint16 NullSchedule = LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[Channel].LinIfChIndex)->SchTableInd;
        ...
        /* now initialize slave channels */
            if (LINIF_CH_IS_SLAVE == (LINIF_CFG_GET_CHANNEL[Channel].CfgFlags & LINIF_NODETYPE_MASK))
            {
                ...
                Atomics_Init(&LinIf_SleepHandlingInProgress[Channel], 0U);
            }
