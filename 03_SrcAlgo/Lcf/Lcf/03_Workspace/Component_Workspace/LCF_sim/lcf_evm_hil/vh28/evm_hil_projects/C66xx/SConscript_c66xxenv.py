...
#
# LAST CHANGE:      $Date: 2021/12/14 01:45:39CET $
#                   $Author: Wang, David (Wangd3) $
#
# CURRENT VERSION:  $Revision: 1.1 $
#############################################################################
# CHANGES:                   $Log: SConscript_c66xxenv.py  $
# CHANGES:                   Revision 1.1 2021/12/14 01:45:39CET Wang, David (Wangd3) 
# CHANGES:                   Initial revision
# CHANGES:                   Member added to project /ADAS/Projects/MFC5xx/MFC5J3/06_Algorithm/DPU/04_Engineering/03_Workspace/Component_Workspace/LCF_sim/lcf_evm_hil/vh28/evm_hil_projects/C66xx/project.pj
# CHANGES:                   Revision 1.2 2017/06/27 14:00:18CEST Asong, Wedndah Asaha (uidj2187) 
# CHANGES:                   Modified scons config scripts for Mainstream SIL-Build
# CHANGES:                   Revision 1.1 2017/06/13 16:03:35CEST Berendes, Ignacius (uid41806) 
# CHANGES:                   Initial revision
# CHANGES:                   Member added to project /ADAS/SW/Components/LCF_LateralControlFunctions/04_Engineering/03_Workspace/Component_Workspace/LCF_sim/lcf_evm_hil/vh28/evm_hil_projects/C66xx/project.pj
# CHANGES:                   Revision 1.0 2016/07/01 12:17:08CEST Prajescu, Claudia (uidl9533) 
# CHANGES:                   Initial revision
# CHANGES:                   Member added to project /nfs/projekte1/REPOSITORY/Base_Development/05_Algorithm/LCF_LateralControlFunctions/04_Engineering/03_Workspace/algo/lcf_sim/lcf_evm_hil/vh28/evm_hil_projects/C66xx/project.pj
...
for e in c66xx_env_list:
    ...
    result = SConscript((c66xx_sim_build_dir + "/03_Workspace/algo/" + component_name + 
    ...
    if env["variant"] == "release":
      ...
      # generate aliases containing the variant's name 
      generate_alias(component_name + "_sim_hil_out_ti_c66xx_" + env["variant"], c66xx_libsize_out)
      generate_alias(component_name + "_sim_hil_ecu_out_" + env["variant"], c66xx_libsize_out)
      generate_alias(component_name + "_sim_" + env["variant"], c66xx_libsize_out)
      if algo_name != component_name:
        generate_alias(component_name + "_sim_hil_out_ti_c66xx_" + env["variant"], c66xx_libsize_out)
        generate_alias(algo_name + "_sim_hil_ecu_out_" + env["variant"], c66xx_libsize_out)
      ...
      # generate aliases NOT containing the variant's name 
      generate_alias(component_name + "_sim_hil_out_ti_c66xx_libsize", c66xx_libsize_out, component_name + ':sim:hil:ti_c66xx:libsize', ['VH28-HIL: ram/rom for C66xx'])
      generate_alias(component_name + "_sim_hil_out_ti_c66xx", c66xx_libsize_out, component_name + ':sim:hil:ti_c66xx')
      generate_alias(component_name + "_sim_hil_ecu_out", c66xx_libsize_out, component_name + ':sim:hil')
      generate_alias(component_name + "_sim", c66xx_libsize_out, component_name + ':sim')
      if algo_name != component_name:
        generate_alias(algo_name + "_sim_hil_out_ti_c66xx_libsize", c66xx_libsize_out, component_name + ':' + algo_name + '_sim:hil:ti_c66xx:libsize', ['VH28-HIL: ram/rom for C66xx'])
        generate_alias(algo_name + "_sim_hil_out_ti_c66xx", c66xx_libsize_out, component_name + ':' + algo_name + '_sim:hil:ti_c66xx')
        generate_alias(algo_name + "_sim_hil_ecu_out", c66xx_libsize_out, component_name + ':' + algo_name + '_sim:hil')
...
if not skip_ide_generation:
    Return(["c66xx_outfile", "ide_ccs_ti_c66xx"])
else:
    Return(["c66xx_outfile"])
