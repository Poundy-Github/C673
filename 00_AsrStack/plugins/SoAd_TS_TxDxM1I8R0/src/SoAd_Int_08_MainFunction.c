...
STATIC FUNC(void, SOAD_CODE) SoAd_Init_BufferStructures
(
  void
);
...
/** \brief Helper function to enable Tx Data Structures especially PduRouteVirtDestinations
 **
...
/** \brief Helper function to enable Rx Data Structures especially SocketRouteDestinations
 **
...
/** \brief Helper function to disable Tx Data Structures especially PduRouteVirtDestinations
 **
 ** \param[in] InternalRoutingGroupId - Internal Id of RG to be disabled.
 */
...
/** \brief Helper function to disable Rx Data Structures especially SocketRouteDestinations
 **
 ** \param[in] SoConId - SoConId of specific RG to be disabled.
 ** \param[in] ExternalRoutingGroupId - Id of RG to be disabled.
 ** \param[in] InternalRoutingGroupId - Internal Id of RG to be disabled.
 */
...
/** \brief Helper function to check if any other associated specific RG is still enabled.
 **
 ...
 */
STATIC FUNC(boolean, SOAD_CODE) SoAd_OneOtherInternalSpecificRGActiveForSocketRouteDest
(
...
/** \brief Helper function to check if any other associated global RG is still enabled.
 **
 ** \param[in] SocketRouteDestPtr - SocketRouteDestination that shall be checked.
 ** \param[in] InternalRoutingGroupId - Id of RG to be checked.
 */
STATIC FUNC(boolean, SOAD_CODE) SoAd_OneOtherInternalGlobalRGActiveForSocketRouteDest
(
  P2CONST(SoAd_SocketRouteDestType, AUTOMATIC, SOAD_APPL_DATA) SocketRouteDestPtr,
  SoAd_RoutingGroupIdType InternalRoutingGroupId
);
...
)
{
  ...
  TS_MemCpy(
              &SOAD_PBRAM_REF_ID(uint32, SoAd_PBcfg->SocketRouteDestVirtualEnabledBitMapBase, 0U),
              &SOAD_CFG_TYPE(uint32, SocketRouteDestVirtualEnableBitMapInit, 0U),
              (uint32)(4*(((uint32)SoAd_PBcfg->SocketRouteDestVirtualCnt / 32U) + 1U))
            );
...
{
  SoAd_BufLengthType BufferPoolId;
  const SoAd_BufLengthType BufferPoolCnt = SoAd_PBcfg->BufferPoolCnt;
  if (SoAd_PBcfg->FixedBufferCnt > 0U) {
    uint32 CurrentFixedBufferId = 0U;
    for (;CurrentFixedBufferId < SoAd_PBcfg->FixedBufferCnt;CurrentFixedBufferId++) {
      CONSTP2CONST(SoAd_FixTxBufferIdToSoConIdInitMapCfgType, AUTOMATIC, SOAD_APPL_DATA)
      FixedBufferMapPtr = &SOAD_CFG(FixTxBufferIdToSoConIdInitMapCfg, CurrentFixedBufferId);
      CONSTP2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr =
      &SOAD_CFG(SoCon, FixedBufferMapPtr->SoConId);
      P2VAR(SoAd_SoConTxUdpDataType, AUTOMATIC, SOAD_APPL_DATA) SoConTxUdpDataPtr = NULL_PTR;
      SoAd_GetSoConTxUdpData(SoConPtr, SoConTxUdpDataPtr);
      SoConTxUdpDataPtr->TxBufferId = FixedBufferMapPtr->BufferId;
    }
  }
  if (SoAd_PBcfg->BufferCnt > 0U) {
    uint32 CurrentBufferId = 0U;
    for (;CurrentBufferId < SoAd_PBcfg->BufferCnt;CurrentBufferId++) {
      P2VAR(SoAd_SoConTxBufferStateType, AUTOMATIC, SOAD_APPL_DATA) TxBufferStatePtr = NULL_PTR;
      SoAd_GetSoConTxBufferState(CurrentBufferId, TxBufferStatePtr);
  ...
  /* for the BF algorithm we need a 1 at Cnt + 1U */
  for(BufferPoolId = 0U; BufferPoolId < BufferPoolCnt + 1U; BufferPoolId++)
  {
    SoAd_SetBitIn32BitMap(BufferPoolId, &SoAd_PBcfg->SoAdBufferPoolBitMapBase);
  }
}
...
)
{
  ...
  for(SoConId = 0U; SoConId < SoConCnt; SoConId++)
  {
    ...
    if(SoConPtr->SoConTxUdpDataId != SOAD_INVALID_PDUTXUDPDATA_ID)
    {
      P2VAR(SoAd_SoConTxUdpDataType, AUTOMATIC, SOAD_APPL_DATA) SoConTxUdpDataPtr = NULL_PTR;
      SoAd_GetSoConTxUdpData(SoConPtr, SoConTxUdpDataPtr);
      SoConTxUdpDataPtr->TxBufferId = SOAD_INVALID_SOCON_ID;
    }
...
#endif /* SOAD_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    if(SOAD_INVALID_ROUTINGGROUP_ID != InternalRoutingGroupId)
    {
      ...
      switch(Action)
      {
        ...
        {
          SoAd_DisableRoutingGroup_RxDataStructures
            (SoConId, ExternalRoutingGroupId, InternalRoutingGroupId);
...
)
{
  ...
  SoAd_SocketRouteDestIdType RgDestMapId;
  ...
  if(RoutingGroupSocketRouteDestMapMapPtr->SocketRouteDestMapFirstId == MaxDestMapId)
  {
    /* This SoAdRoutingGroup is not referenced by any SoAdSocketRouteDest.  */
    RetVal = E_OK;
  }
  else if(SoConId != SOAD_INVALID_SOCON_ID)
  {
    CONSTP2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr =
      &SOAD_CFG(SoCon, SoConId);
    CONSTP2CONST(SoAd_SoConGroupType, AUTOMATIC, SOAD_APPL_DATA) SoConGroupPtr =
    ...
    const SoAd_SoConIdType SoConOffSet = SoConId - SoConGroupPtr->SoConFirstId;

#if(SOAD_DEV_ERROR_DETECT == STD_ON)
    boolean ErrorTwoSoConSamePduOccurred = FALSE, ErrorTwoPdusSameSoConOccurred = FALSE;
#endif
    for(
         RgDestMapId = RoutingGroupSocketRouteDestMapMapPtr->SocketRouteDestMapFirstId;
         RgDestMapId < MaxDestMapId;
         RgDestMapId++
       )
    {
      const SoAd_SocketRouteDestIdType SocketRouteDestId =
          SOAD_CFG_TYPE(SoAd_SocketRouteDestIdType,RoutingGroupSocketRouteDestMap, RgDestMapId);
      P2CONST(SoAd_SocketRouteDestType, AUTOMATIC, SOAD_APPL_DATA) SocketRouteDestPtr =
          &SOAD_CFG(SocketRouteDest, SocketRouteDestId);
      ...
      {
        const SoAd_SocketRouteDestIdType VirtualDestinationId =
            SocketRouteDestPtr->VirtualDestinationStartId + (SoAd_SocketRouteDestIdType)SoConOffSet;

        SoAd_SetBitIn32BitMap
        (
          VirtualDestinationId,
          &SoAd_PBcfg->SocketRouteDestVirtualEnabledBitMapBase
        );
        RetVal = E_OK;
      }
      else
      {
        ...
        if((*ExpectedSocketRouteDestIdPtr) == SOAD_INVALID_SOCKET_ROUTE_DEST_ID)
        {
           ...
           * Now we have to additionally check if the same destination was designated in
           * any other MapEntry */
             ...
             )
          {
            ...
            if((*CurrentSocketRouteDestIdPtr) == SocketRouteDestPtr->SocketRouteDestId)
            {
#if(SOAD_DEV_ERROR_DETECT == STD_ON)
              /* Same Pdu can not be configured to two different SocketConnections */
          ...
          if(DestIdUniqueInSocketRoute)
          {
            ...
            *ExpectedSocketRouteDestIdPtr = SocketRouteDestPtr->SocketRouteDestId;
            RetVal = E_OK;
          }
        }
        /* same destination allowed */
        else if((*ExpectedSocketRouteDestIdPtr) == SocketRouteDestPtr->SocketRouteDestId)
        {
        ...
        {
#if(SOAD_DEV_ERROR_DETECT == STD_ON)
          /* Two Pdus can't be used with the same SocketConnection */
...
/* !LINKSTO SoAd.dsn.Func0130.Rx.Udp.If.TwoSoConSamePdu, 1 */
  ...
  else /* Global Routing Group */
  {
       ...
       )
    {
      ...
      if(SocketRouteDestPtr->VirtualDestinationCount != 1U)
      {
        /* Mixture of global and specific RG detected which violates lim.SoAd.EB_INTREQ_SoAd_0013.
         * In case of such a not allowed mixture the global RG is not enabled to ensure that it
         * does not interfere with the specific routing groups.
         */
        continue;
      }
      else
      {
        SoAd_SetBitIn32BitMap
        (
          SocketRouteDestPtr->VirtualDestinationStartId,
          &SoAd_PBcfg->SocketRouteDestVirtualEnabledBitMapBase
        );
      }
      RetVal = E_OK;
...
)
{
  ...
  for(RgId = RoutingGroupMapFirstId; RgId < MaxRoutingGroupMap; RgId++)
  {
    ...
    if(SoConPtr->SoConGroupId == RoutingGroupExtToIntIdMapEntryPtr->SoConGroupId)
    {
      CONSTP2CONST(SoAd_SoConGroupType, AUTOMATIC, SOAD_APPL_DATA) SoConGroupPtr =
...
)
{
  ...
  if(SoConId != SOAD_INVALID_SOCON_ID)
  {
       ...
       )
    {
      ...
      {
        boolean OtherRoutingGroupEnabled = SoAd_OneOtherInternalSpecificRGActiveForSocketRouteDest
        (
          SocketRouteDestPtr,
          SoConPtr,
          InternalRoutingGroupId
      ...
      else
      {
        ...
        const SoAd_SocketRouteDestIdType SoConToSocketRouteDestMapId =
          SocketRoutePtr->SoConToSocketRouteDestMapFirstId + (SoAd_SocketRouteDestIdType)SoConOffSet;
        CONSTP2VAR(SoAd_SocketRouteDestIdType, AUTOMATIC, SOAD_APPL_DATA) SocketRouteDestIdPtr =
          &SOAD_PBRAM_REF_ID(
                              SoAd_SocketRouteDestIdType,
                              SoAd_PBcfg->SoConToSocketRouteDestMapBase,
        ...
        if(CurrentDestId == SocketRouteDestPtr->SocketRouteDestId)
        {
          boolean OtherRoutingGroupEnabled = SoAd_OneOtherInternalSpecificRGActiveForSocketRouteDest
          (
            SocketRouteDestPtr,
  ...
  else
  {
       ...
       )
    {
      ...
      {
        boolean OtherRoutingGroupEnabled = SoAd_OneOtherInternalGlobalRGActiveForSocketRouteDest
        (
          SocketRouteDestPtr,
          InternalRoutingGroupId
        );
...
)
{
  ...
  CONSTP2CONST(SoAd_SoConGroupType, AUTOMATIC, SOAD_APPL_DATA) SoConGroupPtr =
    &SOAD_CFG(SoConGroup, SoConPtr->SoConGroupId);
  const SoAd_SoConIdType SoConOffSet = SoConPtr->SoConId - SoConGroupPtr->SoConFirstId;
  for(RGMapId = SocketRouteDestPtr->RoutingGroupMapFirstId;
      RGMapId < MaxRGMapId;
      RGMapId++
     )
  {
    const SoAd_RoutingGroupIdType TmpRoutingGroupExtId =
      SOAD_CFG_TYPE(SoAd_RoutingGroupIdType, SocketRouteRoutingGroupMap, RGMapId);

    CONSTP2CONST(SoAd_RoutingGroupExtToIntIdMapType, AUTOMATIC, SOAD_APPL_DATA)
      RoutingGroupExtToIntIdMapEntryPtr
      = &SOAD_CFG(RoutingGroupExtToIntIdMap, TmpRoutingGroupExtId);
    SoAd_RoutingGroupIdType TmpInternalRoutingGroupId
      = RoutingGroupExtToIntIdMapEntryPtr->RoutingGroupId;

    TmpInternalRoutingGroupId += (SoAd_RoutingGroupIdType)SoConOffSet;

    /* Check if any other than the currently to turn off routing group is active. */
    if(
        (TmpInternalRoutingGroupId != InternalRoutingGroupId)
        && (SoConPtr->SoConGroupId == RoutingGroupExtToIntIdMapEntryPtr->SoConGroupId)
        && (SOAD_IS_ROUTINGGROUP_ENABLED(TmpInternalRoutingGroupId) == TRUE)
      )
    {
      /* Another routing group belonging to this destination is still enabled. */
      IsEnabled = TRUE;
      break;
    }
  }
...
/* patch version check */
#if (SOAD_SW_PATCH_VERSION < 18U)
#error SOAD_SW_PATCH_VERSION wrong (< 18U)
#endif
/*==================[end of file]===========================================*/
