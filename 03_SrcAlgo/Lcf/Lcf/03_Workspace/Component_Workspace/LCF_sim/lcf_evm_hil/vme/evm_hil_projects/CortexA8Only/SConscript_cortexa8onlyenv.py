...
#
# MODULE NAME: evm_hil\evm_hil_ti_cortex_a8\SConscript_cortexa8onlyenv.py
#
...
#
# LAST CHANGE:      $Date: 2021/12/14 03:42:57CET $
#                   $Author: Wang, David (Wangd3) $
#
# CURRENT VERSION:  $Revision: 1.1 $
#############################################################################
# CHANGES:                   $Log: SConscript_cortexa8onlyenv.py  $
# CHANGES:                   Revision 1.1 2021/12/14 03:42:57CET Wang, David (Wangd3) 
...
# CHANGES:                   Revision 1.2 2017/06/27 14:01:34CEST Asong, Wedndah Asaha (uidj2187) 
# CHANGES:                   Modified scons config scripts for Mainstream SIL-Build
# CHANGES:                   Revision 1.1 2017/06/13 16:06:33CEST Berendes, Ignacius (uid41806) 
# CHANGES:                   Initial revision
...
# CHANGES:                   Revision 1.23 2015/09/23 16:18:35CEST Fischer-EXT, Andre (uidg5297) 
# CHANGES:                   add dependency between ccs project and relevant ti-tools
# CHANGES:                   - Added comments -  uidg5297 [Sep 23, 2015 4:18:35 PM CEST]
# CHANGES:                   Change Package : 379758:1 http://mks-psad:7002/im/viewissue?selection=379758
# CHANGES:                   Revision 1.22 2015/07/13 15:29:56CEST Fischer-EXT, Andre (uidg5297) 
# CHANGES:                   add lcf-files to ccs-projects
# CHANGES:                   --- Added comments ---  uidg5297 [Jul 13, 2015 3:29:56 PM CEST]
# CHANGES:                   Change Package : 356572:1 http://mks-psad:7002/im/viewissue?selection=356572
# CHANGES:                   Revision 1.21 2015/06/26 12:21:45CEST Palanisamy-EXT, Lenin (uidr0826) 
# CHANGES:                   Add revision label with module description for all GSCons files (Issue 320651)
# CHANGES:                   --- Added comments ---  uidr0826 [Jun 26, 2015 12:21:45 PM CEST]
...
# import
#-------------------------
Import("skip_ide_generation", "scons_arguments", "component_name", "eve_used", "thread_number", "define_list", "algo_name", "gcc_used", "verbose", "help_dict", "profile", "SconscriptTimesTable")
import os
...
if not verbose:
    arm_env['CCCOMSTR'] = "\nCompiling hil vme cortex a8-only $variant\n[$SOURCE]"
    arm_env['CXXCOMSTR'] = "\nCompiling hil vme cortex a8-only $variant\n[$SOURCE]"
    arm_env['LINKCOMSTR'] = "\nLinking hil vme cortex a8-only $variant\n[$TARGET]"                        
#-------------------------
# setup environment
#-------------------------
...
for e in ca8x_env_list:
    ...
    if algo_name != component_name:
        generate_alias(algo_name + "_sim_hil_out_ti_cortex_a8_only_" + env["variant"], ti_cortexA8_sim_out)
    ...
    if env["variant"] == "release":
      ...
      # generate aliases containing the variant's name
      generate_alias(component_name + "_sim_hil_out_ti_cortex_a8_only_" + env["variant"], ca8_libsize_out)
      generate_alias(component_name + "_sim_hil_ecu_out_" + env["variant"], ca8_libsize_out)
      generate_alias(component_name + "_sim_" + env["variant"], ca8_libsize_out)
      if algo_name != component_name:
          generate_alias(algo_name + "_sim_hil_out_ti_cortex_a8_only_" + env["variant"], ca8_libsize_out)
          generate_alias(algo_name + "_sim_hil_ecu_out_" + env["variant"], ca8_libsize_out)
      ...
      # generate aliases NOT containing the variant's name
      generate_alias(component_name + "_sim_hil_out_ti_cortex_a8_only_libsize", ca8_libsize_out, component_name + ':sim:hil:ti_cortex_a8_only:libsize', ['VME-HIL: ram/rom for Cortex A8-only'])
      generate_alias(component_name + "_sim_hil_out_ti_cortex_a8_only", ca8_libsize_out, component_name + ':sim:hil:ti_cortex_a8_only')
      generate_alias(component_name + "_sim_hil_ecu_out", ca8_libsize_out, component_name + ':sim:hil')
      generate_alias(component_name + "_sim", ca8_libsize_out, component_name + ':sim')
      if algo_name != component_name:
          generate_alias(algo_name + "_sim_hil_out_ti_cortex_a8_only_libsize", ca8_libsize_out, component_name + ':' + algo_name + '_sim:hil:ti_cortex_a8_only:libsize', ['VME-HIL: ram/rom for Cortex A8-only'])
          generate_alias(algo_name + "_sim_hil_out_ti_cortex_a8_only", ca8_libsize_out, component_name + ':' + algo_name + '_sim:hil:ti_cortex_a8_only')
          generate_alias(algo_name + "_sim_hil_ecu_out", ca8_libsize_out, component_name + ':' + algo_name + '_sim:hil')
...
if not skip_ide_generation:
    ...
    generate_alias("all", ide_ccs_ti_cortex_a8)
