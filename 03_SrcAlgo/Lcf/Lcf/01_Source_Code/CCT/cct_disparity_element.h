...
namespace cct
{
  ...
  public:
    ...
    explicit CDisparityElement(const DisparityElement_t& de) : m_data(de) {}
    ...
    bool isValidNotZero() const
    {
      return isValid() && ((m_data & DISPARITY_MASK) != 0);
    }

    /// Returns true when the disparity is flagged valid and
    /// not negative.
    /// @return true if isValid() and >= 0
    bool isValidPositive() const
    {
