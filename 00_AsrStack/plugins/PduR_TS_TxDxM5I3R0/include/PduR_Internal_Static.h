 ...
 **
 ** Function called in a multicast gateway scenario by TpRxIndication or TpTxConfirmation (TP gateway queueing enabled)
 ** for each individual destination PDU configured.
 **
 ** \param[in]  GMPduId          Identification of the TP gateway (multicast) PDU.
 ** \param[in]  SessionIndex     Gateway session the GMPduId is handled with (avoid search).
 ** \param[in]  TargetAPIRef     Set of Target APIs the destination proceeds with.
 ** \param[in]  PduInfoPtr       Provides PduInfo for the Target API the destination proceeds with.
 ** \param[out] RemainingEntries Number of remaining destination PDUs to be handled, i.e. in a musticast gateway
 **                              scenario, the number of destination PDUs that remain to be called after that call.
 **
 ** \return Function execution success status
 ** \retval E_OK        Transmit request has been accepted by the destination
 ** \retval E_NOT_OK    Transmit requests was rejected
 **
 **/
TS_MOD_PRIV_DECL FUNC(Std_ReturnType, PDUR_CODE) PduR_GateTpTransmit
(
