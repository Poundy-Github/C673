  ...
  switch (ReadContext->ProcessingState)
  {
    ...
    case DCM_SIGNALREAD_STATE_DATALENGTH:
    {
          ...
          (DataCfg->FixedLength == FALSE))
      {
        ...
        /* Variable used for casting the SignalLength to uint16, ReadContext->SignalLength
           cannot be cast in the Dcm_Dsp_DidSignals_ReadDataLength() call.
           The cast of ReadContext->SignalLength to an uint16 is safe due to limitation
           of configuration parameter DcmDspDataSize range from 1 - 65535 from Tresos. */
        uint16 TempSignalLength = (uint16)ReadContext->SignalLength;
        ...
        /* Save the signal length returned from Dcm_Dsp_DidSignals_ReadDataLength() in the context */
        ReadContext->SignalLength = TempSignalLength;
  ...
  P2VAR(uint16, AUTOMATIC, DCM_VAR)                       SignalLength)
{
  ...
  uint16 ReadSignalDataLengthInBits    = 0U;
  ...
  *Nrc = E_OK;
  ...
  else
  {
    ...
    ReadSignalDataLengthInBits = DCM_GET_BITS(ReadSignalDataLength);
    *SignalLength = ReadSignalDataLengthInBits;

    OperationRetVal = DCM_E_OK;

    if( (ReadSignalDataLength >= DCM_GET_BYTES(DCM_UINT16_MAX)) )
    {
      /*Signal length greater than maximum configurable length 65535 bits */
      /* !LINKSTO Dcm.EB.ReadDid.VariableSignal.OverFlowUint16.ReportDETError,1 */
#if (DCM_DEV_ERROR_DETECT == STD_ON)
      if ((DataCfg->AccessInterface == DCM_USE_DATA_ASYNCH_FNC) ||
          (DataCfg->AccessInterface == DCM_USE_DATA_ASYNCH_CLIENT_SERVER))
      {
      ...
      /* !LINKSTO Dcm.EB.ReadDid.VariableSignal.OverFlowUint16.TransmitGR,1 */
      *Nrc = DCM_E_GENERALREJECT;
      OperationRetVal = DCM_E_NOT_OK;
      *SignalLength = 0U;
    }
            ...
            (DataCfg->AccessInterface != DCM_USE_DATA_SENDER_RECEIVER))
        {
          ...
          else
          {
            ...
            Dcm_Dsp_EndiannessInterpretation_CopyData(ReadContext->SignalBuffer,
                                             ...
                                             DataCfg->DataEndianness,
                                             DataCfg->DataType,
                                             (uint16)ReadContext->SignalLength,
                                             ReadContext->SignalOffset);
          }
#endif
