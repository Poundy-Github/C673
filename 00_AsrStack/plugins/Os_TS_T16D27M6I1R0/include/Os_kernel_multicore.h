 ...
 *
 * $Id: Os_kernel_multicore.h 34931 2021-08-16 09:54:42Z beoe851 $
 */
 ...
 * lastTimeStampTime
 * timeStampTimer
 *     Data for the generic time-stamp if the architecture doesn't provide a special time-stamp timer.
 *     Which hardware timer touse is defined by the pointer timeStampTimer.
 *
...
#define OS_MEMBER_SIZEOF_IDLEMODE		OS_U(1)
#define OS_MEMBER_SIZEOF_INFUNCTION		OS_U(1)
#else
...
#define OS_MEMBER_SIZEOF_IDLEMODE		OS_U(0)
#define OS_MEMBER_SIZEOF_INFUNCTION		OS_U(0)
#endif
...
#define OS_T_HOOKAPP					(OS_T_ISPINITIAL + OS_MEMBER_SIZEOF_ISPINITIAL)
...
#define OS_T_IDLEMODE					(OS_T_INKERNEL + OS_MEMBER_SIZEOF_INKERNEL)
#define OS_T_INFUNCTION					(OS_T_IDLEMODE + OS_MEMBER_SIZEOF_IDLEMODE)
#define OS_T_TFCALLEDFROM				(OS_T_INFUNCTION + OS_MEMBER_SIZEOF_INFUNCTION) 
/*!
...
#ifndef OS_ASM
extern os_result_t OS_DoControlIdle(os_idlemode_t);
#endif
 ...
 *
*/
#ifndef OS_PushTrustedFunction
#define OS_PushTrustedFunction(fid,tf,p)				\
	do {												\
		os_uint8_t inFunction;							\
		os_uint8_t tfCalledFrom;						\
		inFunction = OS_GetKernelData()->inFunction;	\
		tfCalledFrom = OS_GetKernelData()->tfCalledFrom;\
		OS_GetKernelData()->inFunction = OS_INTRUSTED;	\
		OS_GetKernelData()->tfCalledFrom = inFunction;	\
		(*(tf))((fid), (p));							\
		OS_GetKernelData()->inFunction = inFunction;	\
		OS_GetKernelData()->tfCalledFrom = tfCalledFrom;\
	} while (0)
#endif
