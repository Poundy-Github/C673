...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    else
    {
           ...
           (DTCStatusFilter == DEM_J1939DTC_PERMANENT) )
      {
        ...
        * setting NextItem to DEM_MAX_EVENTID avoids unnecessary event looping
        * in subsequent API calls */
        /* !LINKSTO dsn.Dem.J1939.J1939DTCFilter.J1939DcmSetDTCFilter.NoOBDSupport,1 */
        Dem_J1939DTCFilter.NextItem = DEM_MAX_EVENTID;
      }
      else
#endif
      {
              ...
              (FilterResult == FALSE) )
      {
        ...
        if (GetFilter_J1939Dtc != DEM_NO_DTC)
        {
...
#endif
          {
            ...
            {
              if (TRUE == Dem_ApplyJ1939DTCFilter(NextEventId,
                            Dem_J1939DTCFilter.J1939DTCStatusFilter))
              {
                FilterResult = TRUE;
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    DataValue->PreviouslyActiveDiagnosticTroubleCodes =
      Dem_GetNumberJ1939DTCs(
        ...
        DEM_DTC_KIND_ALL_DTCS,
        node);
    /* !LINKSTO Dem.J1939.ReadDiagnosticReadiness1.OBDCompliance,1 */
    ...
    for (loopIndex = 0U; loopIndex < 8U; loopIndex++)
    {
      if (Dem_J1939ReadinessGroupCompressionMap[loopIndex] != DEM_OBD_J1939_RDY_NONE)
      {
        ...
        PID01_map[0U] = PID01_map[0U] | ((uint16)(((PID01[1U] & mask)) <<
          (Dem_J1939ReadinessGroupCompressionMap[loopIndex]-loopIndex)));
  ...
  Dem_J1939DcmLampStatusType* LampStatus)
{
  ...
  switch (Dem_J1939InternalGetLampStatus(DEM_MIL_ID))
  {
  ...
  uint8                            Node)
{
  ...
  while ((EventId <= DEM_MAX_EVENTID) && (EventCnt < MaxDTCCount))
  {
    J1939Dtc = Dem_GetVisibleJ1939DTC(EventId);
