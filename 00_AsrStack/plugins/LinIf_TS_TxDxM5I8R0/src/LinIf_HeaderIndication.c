...
#endif /* (LINIF_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    if(pIfChannel->cFSM == LINIF_CHANNEL_OPERATIONAL)
    {
      CONSTP2CONST(LinIfSlaveChannelType, AUTOMATIC, LINIF_APPL_DATA) pIfChConfSlave =
        LINIF_CFG_GET_CHANNEL_SLAVE(LINIF_CFG_GET_CHANNEL[Channel].LinIfChIndex);
...
)
{
  ...
  switch(pIfChannelSlave->RSID)
  {
    case LINIF_ASSIGN_NAD_RSID:
    {
       ...
       */
      LinIf_BldFrame_PosRespGeneric(dataPtr, LINIF_CFG_GET_CHANNEL_SLAVE(LINIF_CFG_GET_CHANNEL[Channel].LinIfChIndex)->NCInitNad, LINIF_ASSIGN_NAD_RSID);
      Result = E_OK;
      break;
    }
...
)
{
  ...
  TS_AtomicAssign8(dataPtr[LINIF_PDU_BYTE_NAD], pIfChannelSlave->ConfNad);
...
)
{
  ...
  for(i = LINIF_CFG_GET_SLAVE_SUBSTSEP(LINIF_CFG_GET_CHANNEL[Channel].LinIfChIndex)->StartIdx; i < LINIF_CFG_GET_SLAVE_SUBSTSEP(LINIF_CFG_GET_CHANNEL[Channel].LinIfChIndex)->EndIdx; i++)
  {
