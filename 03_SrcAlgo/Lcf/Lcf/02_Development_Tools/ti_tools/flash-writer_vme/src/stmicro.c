...
#define STM_ID_M25P32_MICRON		0x16 /* Micron   chip*/
#define STM_ID_M25P32_MACRONIX		0x36 /* Macronix chip*/
...
void SPIWRITER_v_BulkeraseStartedHook()
{
  // Hook function for flashing script
  static volatile u8 pleaseDontOptimizeMeAway = 0;
  pleaseDontOptimizeMeAway++;
}

int stmicro_bulkerase(struct spi_flash *flash)
{
  struct stmicro_spi_flash *stm = to_stmicro_spi_flash(flash);
  int ret;
  u8 cmd[4];
  ...
  if (ret < 0) {
    printf("SF: STMicro bulk erase failed\n");
  }

  /* Up to 120 seconds */
  SPIWRITER_v_BulkeraseStartedHook();
  ret = stmicro_wait_ready(flash, 120 * CFG_HZ);
  if (ret < 0) {
    printf("SF: STMicro bulk erase timed out\n");
  }
  else
  {
    printf("SF: STMicro: Bulk erase successfully done.\n");
  }
...
int stmicro_erase(struct spi_flash *flash, u32 offset, size_t len)
{
	...
	// Use bulk erase in case whole flash is selected to be erased
	if ((offset == 0) && (len == flash->size))
	{
	  return stmicro_bulkerase(flash);
	}
