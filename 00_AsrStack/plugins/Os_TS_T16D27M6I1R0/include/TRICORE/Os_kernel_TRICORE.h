 ...
 *
 * $Id: Os_kernel_TRICORE.h 33382 2021-03-03 08:00:10Z arla271030 $
*/
...
#define OS_SuspendInterrupts(olp, lvl) \
	do {																	\
		...
		OS_DSYNC();															\
		__cx = OS_AddrFromCx(OS_MFCR(OS_PCXI));								\
		__old = (__cx->pcxi) >> OS_PCXI_PCPN_BIT;							\
																			\
		__new = (lvl) & (os_uint32_t) OS_ICR_CCPN;							\
																			\
		/* The level is never reduced by a Suspend operation! */			\
		if ( __new > __old )												\
		{																	\
			/* Disable interrupts in local context */						\
			OS_MTCR(OS_ICR, ((OS_MFCR(OS_ICR) & (os_uint32_t) ~((os_uint32_t)OS_ICR_CCPN)) | __new));	\
																			\
			/* Disable interrupts in calling context */						\
			/* Only works if called via system call */						\
			__cx->pcxi = (__cx->pcxi & ~OS_PCXI_PCPN) | (__new << OS_PCXI_PCPN_BIT);		\
		}																	\
		*(olp) = (os_uint8_t)__old;											\
	} while(0)
...
#define OS_ResumeInterrupts(olp) \
	do {																\
		...
		__cx = OS_AddrFromCx(OS_MFCR(OS_PCXI));							\
																		\
		__old = *(olp) & (os_uint32_t) 0xffu;							\
																		\
		/* Resume interrupts in calling context */						\
		/* Only works if called via system call */						\
		__cx->pcxi = (__cx->pcxi & ~OS_PCXI_PCPN) | (__old << OS_PCXI_PCPN_BIT);	\
																		\
		/* Resume interrupts in current context */						\
		OS_MTCR(OS_ICR, ((OS_MFCR(OS_ICR) & (os_uint32_t) ~((os_uint32_t)OS_ICR_CCPN)) | __old));	\
	} while(0)
...
#define OS_SetIsrResourceLevel(prio) \
	do {																				\
		...
		do {																			\
			OS_sirlcx = OS_AddrFromCx(OS_sirlpcxi);										\
			OS_sirlpcxi = OS_sirlcx->pcxi;												\
			OS_sirlpsw = OS_sirlcx->psw;												\
		} while ( (OS_sirlpsw & OS_PSW_CDC) != 0 );										\
...
#define OS_ArchMisalignedData(b,l)	( ((l) > 1) && ((((os_uint32_t)(b)) & 1u) != 0) )
#endif
...
#elif (OS_TRICOREARCH == OS_TRICOREARCH_16EP) || (OS_TRICOREARCH == OS_TRICOREARCH_18)
