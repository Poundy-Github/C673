 ...
 ** \param[in] RemoteAddrPtr IP address and port of the remote host that sent the data
 ...
 **
 ** The TCP/IP stack calls this function after the data was acknowledged by the peer
...
/** \brief Indicates a successful connection from client side
 **
 ...
 **
 ** The service gets called if the stack encounters a condition described by the
 ...
 ** (i.e. a new address is assigned or an assigned address becomes invalid).
 **
 ** \param[in] IpAddrId IP address Identifier, representing an IP address specified in the TcpIp
 **                     module configuration (e.g. static IPv4 address on EthIf controller 0).
