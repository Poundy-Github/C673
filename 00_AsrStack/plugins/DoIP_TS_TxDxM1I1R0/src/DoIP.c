 ...
 *
 * \param[in] TcpConIdx - Index of the used TCP connection.
 * \param[in] Abort - A parameter propagated to SoAd_CloseSoCon():
 *                    TRUE  - Closes TCP connection with RST flag, shall be used if no NACK is
 *                            being queued, i.e. in case of Inactivity or Alive check timeout.
 *                    FALSE - Closes TCP connection with FIN flag, shall be used when NACK shall
 *                            be transmitted before connection closure.
 *
 * \Reentrancy{Reentrant}
 * \Synchronicity{Synchronous}
*/
...
STATIC FUNC(void, DOIP_CODE) DoIP_ProcessTimeout(void)
{
  ...
  for(Idx = 0U; Idx < DOIP_MAX_TIMEOUT_VALUES; Idx++)
  {
    ...
    if(TimeoutValue == DoIP_GeneralTimer)
    {
      ...
      {
        DoIP_ResetTcpCon(Idx - DOIP_INACTIVITY_TIMER_INDEX_START, TRUE);
      }
      /* Alive check timeout handling */
      else if(Idx < DOIP_ANNOUNCEMENT_TIMER_INDEX_START)
      {
        DoIP_ResetTcpCon(Idx - DOIP_ALIVE_CHECK_TIMER_INDEX_START, TRUE);
      }
...
)
{
...
#endif /* DOIP_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    if(TcpConIdx != DOIP_INVALID_TCP_CON_IDX)
    {
      ...
      {
#if (DOIP_ENABLE_TCP_CLOSURE_FIN == STD_ON)
        /* NACK transmitted - close with FIN. */
        DoIP_ResetTcpCon(TcpConIdx, FALSE);
#else
        /* Close with RST. */
        DoIP_ResetTcpCon(TcpConIdx, TRUE);
#endif
      }
      else
      {
...
)
{
  ...
  DBG_DOIP_RESETTCPCON_ENTRY(TcpConIdx, Abort);
  ...
  /* Ignore return value: No error handling defined. */
  (void) SoAd_GetSoConId(DOIP_PBCFG_TCPCON(DoIPSoAdPduId).SoAdTxPduId, &SoConId);
  (void) SoAd_CloseSoCon(SoConId, Abort);
  (void) SoAd_OpenSoCon(SoConId);
  ...
  DBG_DOIP_RESETTCPCON_EXIT(TcpConIdx, Abort);
}
...
/* patch version check */
#if (DOIP_SW_PATCH_VERSION < 22U)
#error DOIP_SW_PATCH_VERSION wrong (< 22U)
#endif
