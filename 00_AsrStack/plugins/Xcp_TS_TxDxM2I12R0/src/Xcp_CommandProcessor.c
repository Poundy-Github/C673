...
{
  /* Local variable to hold daq list index with respect to other DAQ lists associated with the event */
  uint16 DaqListIndex = 0U;
  /* Local variable to hold address of the requested Event channel parameters */
  P2VAR(Xcp_EventType, AUTOMATIC, XCP_VAR) EventParamPtr = &Xcp_Event[EventChannelNumber];
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessSubCmdGetSlaveIdOnCAN(void)
{
  ...
  P2VAR(uint8, AUTOMATIC, XCP_VAR) ParamterPtr = &XCP_READ_CMDPACKET_BYTE(XCP_CMD_BYTE2_INDEX);
  /* Local variable to hold requested mode */
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessSubCmdGetDaqID(void)
{
  ...
  {
    /* If XCP_RESOURCE_STIM_MASK == XCP_RESOURCE_STIM then we have both DAQ and STIM so we need
     * to check the mode of the DAQ list.
     * If not, then we the DAQ list's direction is always DAQ and we don't need to check anymore.
     */
#if (XCP_RESOURCE_STIM == XCP_RESOURCE_STIM_MASK)
...
#endif
    {
      CanId = Xcp_TxLPduIdConf[Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].DaqDtoPduId].PduCanId;
    }
#if (XCP_RESOURCE_STIM == XCP_RESOURCE_STIM_MASK)
    else
    {
      CanId = Xcp_RxLPduIdConf[Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].StimDtoPduId].PduCanId;
    }
#endif
    ...
    else /* DAQ List number is valid */
    {
      ...
      else
      {
        ...
        else
        {
          ...
          /* Deviation MISRAC2012-2 <START> */
          P2VAR(Xcp_DaqType, AUTOMATIC, XCP_VAR) AddressOfTheFirstDynamicDaq =
          (Xcp_DaqType *)(&(Xcp_DaqLists.Xcp_DynamicArea[0U]));
...
STATIC FUNC(void, XCP_CODE) Xcp_ArrangeDynamicODTEntries(void)
{
         ...
         DaqListNumber++)
  {
    ...
    {
      /* Get the ODT */
      P2VAR(Xcp_OdtType, AUTOMATIC, XCP_VAR) DynamicOdt = &DynamicDaqListPtr->OdtList[ODTNumber];
