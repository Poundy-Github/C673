/*! \file **********************************************************************
  ...
  VERSION:                   $Revision: 1.34.1.3 $

  ---*/ /*---
  CHANGES:                   $Log: cml_mtrx.c  $
  CHANGES:                   Revision 1.34.1.3 2020/01/08 11:31:58CET Thomas, Jyothy (uid22555) 
  CHANGES:                   corrected alignment
  CHANGES:                   Revision 1.34.1.2 2019/10/11 11:51:40CEST Thomas, Jyothy (uid22555) 
  CHANGES:                   Re written the code for  CML_v_invertMatrix, CML_b_invertMatrixCramer2 and CML_b_invertMatrixCramer3
  CHANGES:                   Revision 1.34.1.1 2019/06/24 10:39:23CEST Kotre-EXT, Jeevan (uidk9586) 
  CHANGES:                   QAF level 3 warnings have been fixed.
  CHANGES:                   Revision 1.34 2018/02/26 08:51:30CET Arulkumar-EXT, Nandu (uidr5428) 
  CHANGES:                   Updated the source code with respect to AL_CML_01.09.18_INT-1
  CHANGES:                   Revision 1.33 2017/11/15 17:39:09CET Arulkumar-EXT, Nandu (uidr5428) 
...
*****************************************************************************/
/* PRQA S 3242 10 */
/* Deactivate QAC warning 3242, Date: 2019-06-18; Reviewer: uidk9586;
   Reason: CML_INLINE is defined to handle this here */
...
void CML_v_addMatrices(CML_t_Matrix* p_MatrixRes, const CML_t_Matrix* p_MatrixA, const CML_t_Matrix* p_MatrixB)
{
  ...
  float32 * CML_RESTRICT p_DataA = p_MatrixA->pData;       /* get pointer to matrix data */
  float32* CML_RESTRICT p_DataB = p_MatrixB->pData;       /* get pointer to matrix data */
  float32* CML_RESTRICT p_DataRes = p_MatrixRes->pData;   /* get pointer to result matrix data */
...
void CML_v_multiplyMatricesToSymResult(CML_t_Matrix* CML_RESTRICT p_MatrixRes, const CML_t_Matrix* p_MatrixA, const CML_t_Matrix* p_MatrixB)
{
  ...
  float32* CML_RESTRICT p_DataA;
  float32* CML_RESTRICT p_DataB;
  float32* CML_RESTRICT pLineA;
  float32* CML_RESTRICT p_DataRes = p_MatrixRes->pData;       /* get pointer to matrix data */
...
/* Deactivate QAC warning 1257, Date: 2019-06-18; Reviewer: uidk9586;
   Reason: There is no risk of conversion from Long to unsigned int as UL is used here for initialization only. */
    /* multiply matrix elements */
...
void CML_v_invertMatrix(CML_t_Matrix* p_MatrixRes, CML_t_Matrix* p_MatrixA)
{
  ...
  float32 f_Temp, f_MaxElem, f_Temp1;
...
/* PRQA S 2982 10 */
/* Deactivate QAC warning 2982, Date: 2019-06-18; Reviewer: uidk9586;
   ...
   Reason: There is no risk of conversion from Long to unsigned int as UL is used here for initialization only. */
          ...
          for (u_Idx1 = 0UL; u_Idx1<(uint32)p_MatrixA->Desc.col; u_Idx1++)   /* loop other rows */
          {
            if (u_Idx1 != u_col)
            {
              ...
              {
                 /* Re-written the code for ECU-SIL bit exactness */
                 f_Temp1 = p_DataA[u_Idx2 + u_pos2] * f_Temp;
                 p_DataA[u_Idx2 + u_pos1] = p_DataA[u_Idx2 + u_pos1] - f_Temp1; 
              }
              for (u_Idx2 = 0UL; u_Idx2<(uint32)p_MatrixA->Desc.col; u_Idx2++)  /* all elements */
              {
                  /* Re-written the code for ECU-SIL bit exactness */
                  f_Temp1 = p_DataRes[u_Idx2 + u_pos2] * f_Temp;
                  p_DataRes[u_Idx2 + u_pos1] = p_DataRes[u_Idx2 + u_pos1] - f_Temp1;
              }
            }
          }
...
*****************************************************************************/
/* PRQA S 3240 5 */
/* Deactivate QAC warning 3240, Date: 2019-06-18; Reviewer: uidk9586;
   Reason: CML_INLINE is defined to handle this here */
CML_INLINE boolean CML_b_invertMatrixCramer2(float32 a_res[MTRX_2X2_NOF_ELEMENTS], float32 a_in[MTRX_2X2_NOF_ELEMENTS])
{
  ...
  /* Cramers Rule for matrix size == 2 */
  /* Re-written the code for ECU-SIL bit exactness */
  f_temp1 = a_in[0] * a_in[3];
  f_temp2 = a_in[1] * a_in[2];
...
CML_INLINE boolean CML_b_invertMatrixCramer3(float32 a_res[MTRX_3X3_NOF_ELEMENTS], float32 a_in[MTRX_3X3_NOF_ELEMENTS])
{
  float32 f_temp  = 0.0f;   /* temporary variable */
  boolean b_ret   = b_FALSE;  /* return value */
  float32 f_temp1, f_temp2, f_temp3, f_temp4, f_temp5, f_temp6; 

  /* Re-written the code for ECU-SIL bit exactness */
  /* Cramers Rule for matrix size == 3 */
  f_temp1 = a_in[0] * a_in[4];
  f_temp2 = a_in[3] * a_in[1];
  f_temp3 = f_temp1 - f_temp2;
  f_temp4 = f_temp3 * a_in[8];

  f_temp1 = a_in[3] * a_in[7];
  f_temp2 = a_in[6] * a_in[4];
  f_temp3 = f_temp1 - f_temp2;
  f_temp5 = f_temp3 * a_in[2];

  f_temp1 = a_in[6] * a_in[1];
  f_temp2 = a_in[0] * a_in[7];
  f_temp3 = f_temp1 - f_temp2;
  f_temp6 = f_temp3 * a_in[5];

  f_temp = f_temp4 + f_temp5;
  f_temp = f_temp + f_temp6;

  if(CML_f_IsNonZero(f_temp))
  {
    f_temp = 1.0F / f_temp;

    f_temp1 = a_in[4] * a_in[8];
    f_temp2 = a_in[5] * a_in[7];
    f_temp3 = f_temp1 - f_temp2;
    a_res[0] = f_temp3 * f_temp;

    f_temp1 = a_in[2] * a_in[7];
    f_temp2 = a_in[1] * a_in[8];
    f_temp3 =  f_temp1 - f_temp2;
    a_res[1] = f_temp3 * f_temp;

    f_temp1 = a_in[1] * a_in[5];
    f_temp2 = a_in[2] * a_in[4];
    f_temp3 =  f_temp1 - f_temp2;
    a_res[2] = f_temp3 * f_temp;
    
    f_temp1 = a_in[5] * a_in[6];
    f_temp2 = a_in[3] * a_in[8];
    f_temp3 =  f_temp1 - f_temp2;
    a_res[3] = f_temp3 * f_temp;

    f_temp1 = a_in[0] * a_in[8];
    f_temp2 = a_in[2] * a_in[6];
    f_temp3 =  f_temp1 - f_temp2;
    a_res[4] = f_temp3 * f_temp;

    f_temp1 = a_in[2] * a_in[3];
    f_temp2 = a_in[0] * a_in[5];
    f_temp3 =  f_temp1 - f_temp2;
    a_res[5] = f_temp3 * f_temp;
    
    f_temp1 = a_in[3] * a_in[7];
    f_temp2 = a_in[4] * a_in[6];
    f_temp3 =  f_temp1 - f_temp2;
    a_res[6] = f_temp3 * f_temp;

    f_temp1 = a_in[1] * a_in[6];
    f_temp2 = a_in[0] * a_in[7];
    f_temp3 =  f_temp1 - f_temp2;
    a_res[7] = f_temp3 * f_temp;

    f_temp1 = a_in[0] * a_in[4];
    f_temp2 = a_in[1] * a_in[3];
    f_temp3 =  f_temp1 - f_temp2;
    a_res[8] = f_temp3 * f_temp;
...
*****************************************************************************/
void CML_v_copyArrayToSymMatrix(CML_t_Matrix* CML_RESTRICT p_MatrixRes, uint32 u_RowNr, uint32 u_ColNr, const float32* CML_RESTRICT p_Data)
{
  uint32 u_Idx1, u_Idx2;
...
void CML_v_lowTriaInvMatrix(CML_t_Matrix* p_MatrixRes, const CML_t_Matrix* p_MatrixA)
{
    ...
    && (p_MatrixRes->Desc.maxsize >= (p_MatrixA->Desc.col * p_MatrixA->Desc.row)) )
  {
    ...
    u_ColRowNr = (uint32) p_MatrixA->Desc.col; // ColNr = RowNr
/* PRQA S 1257 25 */
/* Deactivate QAC warning 1257, Date: 2019-06-18; Reviewer: uid22555;
   Reason: There is no risk of conversion from Long to unsigned int as UL is used here for initialization only. */
    for (u_Idx1=0UL; u_Idx1<u_ColRowNr; u_Idx1++)
    {
