...
{
	my $self = shift;
	GenldCommon::printOut("section_layout :vtc:linear");
	GenldCommon::printOut("{");
}
...
sub beginMemoryBlockMappings
{
	...
	GenldCommon::printOut("\ngroup (contiguous, ordered, run_addr = mem:mpe:$block)");
	GenldCommon::printOut("{");
}
...
sub endMemoryBlockMappings
{
	...
	{
		GenldCommon::printOut("\ngroup (contiguous, ordered, load_addr = mem:mpe:$self->{memDefs}{initdata_memory})");
		GenldCommon::printOut("{");
