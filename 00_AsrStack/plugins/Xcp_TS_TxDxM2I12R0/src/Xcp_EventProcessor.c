...
/** \brief Function to directly process data for the local partition
**  This function is used when all ODTs of an event are located on the local partition,
**  and sends the response to the Master Xcp
*/
STATIC FUNC(void, XCP_CODE) Xcp_ProcessLocalRequest(uint16 EventId);

/** \brief Function to directly read/write data for the ODTs processed for the local partition
**  This function is used when all ODTs of an event are located on the local partition
*/
STATIC FUNC(void, XCP_CODE) Xcp_ProcessLocalMemoryProxy(Xcp_MemoryProxyDataType* data);

/** \brief Function to set the values for Xcp_MemoryProxyData for the MemProxy request
*/
STATIC FUNC(void, XCP_CODE) Xcp_CreateMemoryProxyRequestForEvent(uint16 EventId, Xcp_MemoryProxyDataType* Xcp_MemoryProxyData);

/** \brief Function to check if all ODT entries of an event are located on the local partition
*/
STATIC FUNC(boolean, XCP_CODE) Xcp_AllRequestLocalPartition(Atomic_value_t LocalPartition, uint16 EventId);
#endif
...
FUNC(void, XCP_CODE) Xcp_ProcessEvents(uint16 EventId)
{
  ...
  if (EventProcessingStatus == XCP_OK)
  {
    ...
    {
#if (XCP_DAQSTIM_MEMORY_ACCESS_AREAS == STD_ON)
      Atomic_value_t LocalPartition = Atomics_Load(&Xcp_PartitionOfActiveConnection);
                         ...
                         &(Xcp_Event[EventId].EventStatusFlag)) == TRUE)
    {
      ...
      /* If MemProxy is available we can send the request, otherwise the request for event
         will be queued */
      /* !LINKSTO Xcp.dsn.BSWDistribution.EventProcessing.OnlyLocalData,1 */
      if (Xcp_AllRequestLocalPartition(LocalPartition, EventId) == TRUE)
      {
        Xcp_ProcessLocalRequest(EventId);
      }
      ...
      else if ((Atomics_Load(&Xcp_MemoryProxyState) == XCP_MEMORY_PROXY_AVAILABLE) &&
      ...
      /* Deviation MISRAC2012-2 <STOP> */
      {
        ...
        Xcp_CreateMemoryProxyRequestForEvent(EventId, &Xcp_MemoryProxyData);
        /* first prepare the local data */
        if (Xcp_RequestForEvent[EventId].EntriesForAppIdAvailable[LocalPartition] == TRUE)
        {
          /* send request localy */
          Xcp_MemoryProxyHandler(&Xcp_MemoryProxyData);
          Xcp_RequestForEvent[EventId].ProcessingCtr++;
        }
        /* now send request to other partitions */
        for (AppIdx = 0U; AppIdx < XCP_NUM_OF_OS_APPS; AppIdx++)
        {
          if ((Xcp_RequestForEvent[EventId].EntriesForAppIdAvailable[AppIdx] == TRUE) &&
              (AppIdx != LocalPartition))
          {
            ...
            (void)Xcp_SendMemoryProxyData[LocalPartition][AppIdx](&Xcp_MemoryProxyData);
          }
        }
      }
      else
      {
        ...
        Xcp_RequestQueue.WriteIdx %= XCP_NUMBER_OF_QUEUED_EVENTS;
      }
      Xcp_EventIsProcessing = FALSE;
#endif
    }
...
#endif /* XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_ON */
  {
    ...
    ((XCP_ENABLE_DAQ_OPTIMIZATION == STD_ON) && (XCP_ENABLE_BIT_OFFSET == STD_ON)))
        ...
        if (bitOffset != XCP_BIT_OFFSET_IGNORE_VALUE)
        {
          ...
          Xcp_MemoryProxyEnqueueRequest(DaqId, OdtEntryLength, DTOMirrorWritePos, MemoryArea->ApplicationId, Type, XCP_APPL_GET_ADDRESS( 0U, odtEntryAddress), DummyBitMask);
        }
        else
#endif
        {
          Xcp_MemoryProxyEnqueueRequest(DaqId, OdtEntryLength, DTOMirrorWritePos, MemoryArea->ApplicationId, XCP_MEMORYPROXYDATA_SAMPLE, XCP_APPL_GET_ADDRESS( 0U, odtEntryAddress), DummyBitMask);
        }
        SchM_Exit_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
...
STATIC FUNC(void, XCP_CODE) Xcp_ProcessQueuedEvents(void)
{
  ...
  if (EventId != XCP_E_INVALID_EVENT)
  {
    ...
    Xcp_CreateMemoryProxyRequestForEvent(EventId, &Xcp_MemoryProxyData);
    if (Xcp_RequestForEvent[EventId].EntriesForAppIdAvailable[partition] == TRUE)
    {
      /* send request localy */
      Xcp_MemoryProxyHandler(&Xcp_MemoryProxyData);
      Xcp_RequestForEvent[EventId].ProcessingCtr++;
...
STATIC FUNC(void, XCP_CODE) Xcp_ProcessLocalRequest(uint16 EventId)
{
  ...
  SchM_Enter_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
  Xcp_CreateMemoryProxyRequestForEvent(EventId, &Xcp_MemoryProxyData);
  Xcp_ProcessLocalMemoryProxy(&Xcp_MemoryProxyData);
  /* Update event processing parameters */
  for (DTOIdx = 0U; DTOIdx < Xcp_DaqPrioQueueOfRequest.NoOfEntries[EventId]; DTOIdx++)
  {
    ...
    Xcp_DaqPrioQueueOfRequest.ReadIdx %= XCP_MAX_SIZE_OF_DAQ_PRIO_LIST;
  }
  Xcp_DaqPrioQueueOfRequest.NoOfEntries[EventId] = 0U;
  /* exit critical section */
  SchM_Exit_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
  Xcp_UpdateEvent(EventId, XCP_OK);
  Xcp_ProcessTransmit(XCP_SID_MAIN_FUNCTION);
}
...
STATIC FUNC(void, XCP_CODE) Xcp_ProcessLocalMemoryProxy(Xcp_MemoryProxyDataType* data)
{
  ...
  for (Idx = 0U; Idx < data->size; Idx++)
  {
    ...
    ReadIdx++;
    ReadIdx %= XCP_MAX_SIZE_OF_REQUEST_BUFFER;
  }
}
      ...
      if (Xcp_CyclicEventOverloaded == FALSE)
      {
        ...
        for (DTOIdx = 0U; DTOIdx < Xcp_RequestToMemProxy.NoOfEntries[*EventId]; DTOIdx++)
        {
          if ((Xcp_RequestBuffer[IdxInRequest].type == XCP_MEMORYPROXYDATA_SAMPLE) ||
              (Xcp_RequestBuffer[IdxInRequest].type == XCP_MEMORYPROXYDATA_SAMPLE_0) ||
              (Xcp_RequestBuffer[IdxInRequest].type == XCP_MEMORYPROXYDATA_SAMPLE_1)
               )
          {
