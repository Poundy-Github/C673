    ...
    )
{
  ...
  /* No Exclusive Area needed here, as FrIf_Transmit must not preempt itself for the same PDU */
  ...
  for(iPdu = 0U; iPdu < (uint16_least)FrIf_OperationPtr->PduPositionNum_OwnerIdx; iPdu++)
  {
    ...
    if(OwnerIdx >= FrIf_FirstASR43OwnerIdx)
    {
      ...
      if(PduIdx >= FrIf_RootConfigPtr->FrIf_TxPduNoConfNum)
      {
        uint32 BitMapByteOffset = iPdu / 8U;
        uint8 BitmapBitOffset = iPdu % 8U;

        /* Check if transmitted bit is set for a decoupled Tx Pdu, contained by frame */
        if((((P2VAR(uint8,AUTOMATIC,FRIF_VAR))(P2VAR(void,AUTOMATIC,FRIF_VAR)) FrIf_Mem)
                                [(FrIf_RootConfigPtr->FrIf_TxDecTransmittedBitmapRef + BitMapByteOffset)] &
                                    ((uint8)0x01U << BitmapBitOffset)) != 0U)
        {
...
/* if no single controller implementation */
    ...
    /* Calculate number of bytes reserved in FrIf_Mem for FrIf_TxDecTransmittedBitmap */
    const FrIf_PduIdxType TxDecPduMaxNumFrameByteCount =
        (FrIf_RootConfigPtr->FrIf_TxDecPduMaxNumFrame + 7U) / 8U;
...
/* using dynamic payload length support for FrIf ? */
    ...
    /* Reset all bytes in FrIf_Mem used for TxDecTransmittedBitmap */
    TS_MemBZero(
                 &((P2VAR(uint8,AUTOMATIC,FRIF_VAR))(P2VAR(void,AUTOMATIC,FRIF_VAR)) FrIf_Mem)
                                  [FrIf_RootConfigPtr->FrIf_TxDecTransmittedBitmapRef],
                 TxDecPduMaxNumFrameByteCount
               );
...
#endif /* FRIF_UL_ASR43_API_ENABLE == STD_ON */
                /* All tranmissions handled for ASR43 conform upper layer. Reset remaining Pdus. */
                /* Reset all bytes in FrIf_Mem used for TxDecTransmittedBitmap */
                TS_MemBZero(
                             &((P2VAR(uint8,AUTOMATIC,FRIF_VAR))(P2VAR(void,AUTOMATIC,FRIF_VAR)) FrIf_Mem)
                                              [FrIf_RootConfigPtr->FrIf_TxDecTransmittedBitmapRef],
