 ...
 *
 * MISRAC2012-1) Deviated Rule: 20.9 (required)
 * All identifiers used in the controlling expression of #if or #elif preprocessing directives
 * shall be #define'd before evaluation.
 *
 * Reason:
 * The critical section is not required when BSWM_QUEUED_ARBITRATION_ENABLED is STD_ON and
 * BSWM_QUEUE_DEFERRED_REQUESTS is STD_ON. In order to express this conveniently,
 * BSWM_QUEUED_ARBITRATION_ENABLED and BSWM_QUEUE_DEFERRED_REQUESTS need to be part
 * of the same negated logical expression.
 * As BSWM_QUEUE_DEFERRED_REQUESTS is only defined when BSWM_QUEUED_ARBITRATION_ENABLED is STD_ON,
 * on configurations where BSWM_QUEUED_ARBITRATION_ENABLED is STD_OFF BSWM_QUEUE_DEFERRED_REQUESTS
 * remains undefined. This is however safe because the expression uses the logical AND operator
 * thus short-circuiting applies and BSWM_QUEUE_DEFERRED_REQUESTS is not evaluated if
 * BSWM_QUEUED_ARBITRATION_ENABLED is not STD_ON.
 */
...
/* patch version check */
#if (BSWM_SW_PATCH_VERSION < 7U)
#error BSWM_SW_PATCH_VERSION wrong (< 7U)
#endif
...
#if !((BSWM_QUEUED_ARBITRATION_ENABLED == STD_ON) && (BSWM_QUEUE_DEFERRED_REQUESTS == STD_ON))
          inst->SchMEnter();
#endif
