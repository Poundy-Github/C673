...
#ifdef TS_POLYSPACE
#include <CanIf_PBcfg.h>      /* PBcfg structure, to allow pre-compile time like  access */
#endif /* TS_POLYSPACE */
...
#define CANIF_CFG_GET_TXLPDU_CFG(Index)   \
    (CANIF_PBCFG_ACCESS(CanIf_TxLPduConfigType, txLPduConfig)[(Index)])
...
#define CANIF_CFG_GET_RXLPDU_CFG(Index)   \
    (CANIF_PBCFG_ACCESS(CanIf_RxLPduConfigType, rxLPduConfig)[(Index)])
...
#define CANIF_CFG_GET_SORTEDRXLPDU_CFG(Index)   \
    (CANIF_PBCFG_ACCESS(CanIf_SortedRxLPduConfigType, sortedRxLPduConfig)[(Index)])
...
#define CANIF_CFG_GET_DRIVER_CFG(Index)   \
    (CANIF_PBCFG_ACCESS(CanIf_CanDriverConfigType, canDriverConfig)[(Index)])
...
#define CANIF_CFG_GET_TRCV_CFG(Index)   \
    (CANIF_PBCFG_ACCESS(CanIf_CanTrcvChnlConfigType, canTrcvChnlConfig)[(Index)])
...
#define CANIF_CFG_GET_CTRL_CFG(Index)   \
    (CanIf_ConfigLayout.canControllerConfig[(Index)])
#else
...
#define CANIF_CFG_GET_HOHID2CANIF_IDX(Index)   \
    (CanIf_ConfigLayout.hohIdToCanIfIdx[(Index)])
...
#define CANIF_CFG_GET_HTH_CFG(Index)   \
    (CanIf_ConfigLayout.driverHthConfig[(Index)])
...
#define CANIF_CFG_GET_HRH_CFG(Index)   \
    (CanIf_ConfigLayout.driverHrhConfig[(Index)])
 ...
 */
extern CONST( uint8, CANIF_APPL_CONST )
  CanIf_BitMaskByte[8U];
