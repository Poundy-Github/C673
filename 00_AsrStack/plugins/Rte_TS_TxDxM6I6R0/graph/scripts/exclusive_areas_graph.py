...
"""
    ...
    This diagram shows all exclusive areas and their accessing runnables up to the task context in which they are running.
    If a runnable is directly called by another runnable, the calling chain is included, too. Exclusive areas that are colored
...
from scripts.runnable_context_graph import *
def create_graph(graph_generator):
    ...
    for exclusivearea in exclusive_areas:
        ...
        cluster = graph_generator.get_or_create_cluster(clusterid, exclusivearea.getparent(
        ).getparent().findtext("name") + '-' + exclusivearea.findtext("name"))
...
Instance: {{../../name}}""", style=(
        ...
        for executable in graph_generator.find_and_resolve_references(exclusivearea, "runsInsideExecutableInstances/executableRef"):
            executable_node = get_or_create_runnable_and_callers(
