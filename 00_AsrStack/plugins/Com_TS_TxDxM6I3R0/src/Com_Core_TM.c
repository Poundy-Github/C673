...
#if(COM_TXMODEBEHAVIOUR == COM_CUSTOM1)
/** \brief Com_EB_Get_RemainingMDTticks
 * Function calculates the remaining ticks for the MDT to expire.
 * \param[in] IPduPtr - reference to the configuration of the I-Pdu
 * \param[out] RemainingMDTticksPtr - remaining ticks for the MDT to expire
 */
_COM_STATIC_ FUNC(void, COM_CODE) Com_EB_Get_RemainingMDTticks
(
    P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
    P2VAR(ComTxIPduMinimumDelayTimeFactorType, AUTOMATIC, COM_APPL_DATA) RemainingMDTticksPtr,
    P2VAR(ComGlobalTimeType, AUTOMATIC, COM_APPL_DATA) GlobalTimeAtMDTticksPtr
);
#endif /* (COM_TXMODEBEHAVIOUR == COM_CUSTOM1) */
...
#if ( (COM_TX_MODE_MIXED_N_TIMES_ENABLE == STD_ON) && (COM_TXMODEBEHAVIOUR == COM_CUSTOM1) )
           ComTxIPduMinimumDelayTimeFactorType RemainingMDTticks = 0U;
           ComGlobalTimeType GlobalTimeAtMDTticks = 0U;

           if (Com_IsBitSet_8(PduStatusFlagsPtr, COM_TXPDU_MDTQUEUED))
           {
             /* send request during MDT */

             /* get remaining ticks until MDT expire and
              * used it to set the next NPeriod after MDT expired */
             Com_EB_Get_RemainingMDTticks(
                 IPduPtr,
                 &RemainingMDTticks,
                 &GlobalTimeAtMDTticks);
           }
#endif /* ( (COM_TX_MODE_MIXED_N_TIMES_ENABLE == STD_ON) && (COM_TXMODEBEHAVIOUR == COM_CUSTOM1) ) */
...
)
{
  ...
  ComTxIPduMinimumDelayTimeFactorType MDTQueueVal;
  ...
  GlobalTime = COM_GET_Com_GlobalTxTime(COM_GET_Com_MainFunctionTxIdx(IPduPtr));
  /* get the time out */
  MDTQueueVal = *MDTQueueValPtr;

  {
    ComTxIPduMinimumDelayTimeFactorType delta = 0U;
    COM_GET_DELTA_GlobalTxTime((ComTxIPduMinimumDelayTimeFactorType)GlobalTime,MDTQueueVal,delta,
        ComTxIPduMinimumDelayTimeFactorType);

    *RemainingMDTticksPtr = delta;
  }

  /* Leave critical section */
  Com_EB_SchM_Exit_SCHM_COM_EXCLUSIVE_AREA_0();

  /* assign used global time for remaining mdt ticks calculation */
  *GlobalTimeAtMDTticksPtr = GlobalTime;
...
#if(COM_TXMODEBEHAVIOUR == COM_CUSTOM1)
   ,ComTxModeRepetitionPeriodFactorType RemainingMDTticks
   ,ComGlobalTimeType GlobalTimeAtMDTticks
#endif /* (COM_TXMODEBEHAVIOUR == COM_CUSTOM1) */
)
{
...
#if(COM_TXMODEBEHAVIOUR == COM_CUSTOM1)
   /* in case of remaining ticks calculate GlobalTimeDelta */
   if(RemainingMDTticks != 0U)
   {
     ComGlobalTimeType GlobalTimeDelta;
     /* get global time value delta between newest and the one
      * which was used for remaining MDT ticks calculation */
     COM_GET_DELTA_GlobalTxTime(GlobalTimeAtMDTticks,globalTime,GlobalTimeDelta,ComGlobalTimeType);

     Offset = Offset + (ComTxModeRepetitionPeriodFactorType)(RemainingMDTticks - GlobalTimeDelta);
   }
#endif /* (COM_TXMODEBEHAVIOUR == COM_CUSTOM1) */

   TimeOut = (ComTxModeRepetitionPeriodFactorType) ((ComTxModeRepetitionPeriodFactorType)globalTime + Offset);
