...
/** \brief Copies meta data type
 *
 * Copies meta data (meta data type) from an internal buffer to the PDU's meta data memory
 *
 * Note: Length of a meta data type is calculated by the generator via the custom XPath
 * API asc_ecuc:getMetaDataTypeLength
 *
 ...
 */
extern FUNC(Std_ReturnType, ECUC_CODE) EcuC_CopyMetaDataTypeToPdu
(
...
/** \brief Copies the meta data type
 *
 ...
 * \retval E_NOT_OK             the PDU ID does not exist
 */
extern FUNC(Std_ReturnType, ECUC_CODE) EcuC_CopyMetaDataTypeFromPdu
(
  EcuC_PduIdType PduId,
  P2CONST(PduInfoType, AUTOMATIC, ECUC_APPL_DATA) PduInfoPtr,
