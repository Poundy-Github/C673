...
class GraphGenerator:
    ...
    def new_cluster(self, id, label, color=None, style="filled"):
        """
        ...
        """
        return Cluster(id, label, color, style)

    def get_or_create_cluster(self, id, label, color=None, parent_cluster=None, style="filled"):
        """
        ...
        if parent_cluster is not None:
            ...
            self.__subclusters[key] = self.new_cluster(id, label, color, style)
            parent_cluster.add_cluster(self.__subclusters[key])
            return self.__subclusters[key]
        else:
            if id in self.__clusters.keys():
                return self.__clusters[id]
            self.__clusters[id] = self.new_cluster(id, label, color, style)
            return self.__clusters[id]
    ...
    def get_or_create_node(self, cluster, xmlelement, text, style):
        """
        ...
        return Node.get_or_create_node(self.__nodes, cluster, xmlelement, text, style)
    ...
    def get_or_create_node_with_customized_id(self, customizedid, cluster, xmlelement=None, text="", style=None):
        """
        ...
        """
        self.add_legend(style=style)
        return Node.get_or_create_node_with_customized_id(customizedid, self.__nodes, cluster, xmlelement, text, style)
    ...
    def generate_graph(self, filename, compound='true', rankdir="LR", ranksep="0.2", bgcolor="#F0F0FF", fontcolor="#0000A0", fontsize="13", source_file_only=False):
        ...
        try:
            output_path = os.path.join(self.__args.output, filename+".gv")
            if source_file_only:
                maingraph.save(output_path)
            else:
                maingraph.render(output_path, view=False)
        except:
            ...
            if cluster is not self.__legend_cluster:
                ...
                try:
                    output_path = os.path.join(os.path.join(
                        self.__args.output, folder_name), suffix)+".gv"
                    if source_file_only:
                        maingraph.save(output_path)
                    else:
                        maingraph.render(output_path, view=False)
