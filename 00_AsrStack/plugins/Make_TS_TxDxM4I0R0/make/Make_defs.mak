...
# 1st stage: filter for existing directories, normalize, deduplicate, call depth-traversal worker
# parameter 1: path list
# parameter 2: wildcard expression list
rwildcard  = $(call _rwildcard,$(sort $(subst /.$(SPACE),$(SPACE),$(wildcard  $(addsuffix /.,$1))$(SPACE))),$2)
_rwildcard = $(if $1,$(wildcard $(foreach p,$2, $(addsuffix /$p,$1))) $(call _rwildcard,$(strip $(subst /.$(SPACE),$(SPACE),$(wildcard $(addsuffix /*/.,$1))$(SPACE))),$2))
...
CUSTOMER_OUTPUT_DIR			:= $(CUSTOMER_OUTPUT_PATH)
CUSTOMER_BIN_OUTPUT_PATH	:= $(CUSTOMER_OUTPUT_PATH)/bin
CUSTOMER_DEPEND_DIR			:= $(CUSTOMER_OUTPUT_PATH)/depend
CUSTOMER_LIB_DIR			:= $(CUSTOMER_OUTPUT_PATH)/lib
CUSTOMER_MK_TMP_DIR			:= $(CUSTOMER_OUTPUT_PATH)/make
CUSTOMER_OBJ_DIR			:= $(CUSTOMER_OUTPUT_PATH)/obj
CUSTOMER_LIST_DIR			:= $(CUSTOMER_OUTPUT_PATH)/list
CUSTOMER_LOG_DIR			:= $(CUSTOMER_OUTPUT_PATH)/temp

# now define absolute paths
OUTPUT_DIR					:= $(PROJECT_ROOT)/$(REL_OUT_DIR)
BIN_OUTPUT_PATH				:= $(CUSTOMER_BIN_OUTPUT_PATH)
DEPEND_DIR					:= $(CUSTOMER_DEPEND_DIR)
LIB_DIR						:= $(CUSTOMER_LIB_DIR)
MK_TMP_DIR					:= $(CUSTOMER_MK_TMP_DIR)
OBJ_DIR						:= $(CUSTOMER_OBJ_DIR)
LIST_DIR					:= $(CUSTOMER_LIST_DIR)
