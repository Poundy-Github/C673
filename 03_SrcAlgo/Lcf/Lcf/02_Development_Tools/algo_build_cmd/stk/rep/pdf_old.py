...
class PdfBase(BaseDocTemplate):  # pylint: disable=R0904
    ...
    def onLaterPages(self, canv, doc):
        ...
        pl = Image(StringIO(CONTI_CORP_LOGO), 4 * cm, 4 * cm * CONTI_LOGO_SIZE[1] / float(CONTI_LOGO_SIZE[0]))
...
class Pdf:  # pylint: disable=R0902
    """Base class for report generation in all validation projects"""
    def __init__(self, outfileName, reportTitle, reportSubject=None, date=asctime(),  # pylint: disable=R0913
                 confidentialLevel=CONF_LEVEL_STRICTLY, draft=False,
                 makeTitle=True, makeTableOfContent=True, makeTableOfFigures=False, makeTableOfTables=False,
        ...
        self._figNumber = 1
        # the report settings
        self._headerInfo = [reportTitle, environ["USERNAME"], reportSubject, date, "draft" if draft else "released"]

        self._confidentialLevel = confidentialLevel
        self._additionalTitleInfo = {} if additional_info is None else additional_info
        ...
        if makeTitle is True:
            self._makeTitle()

        if makeTableOfContent is True:
            self.makeTableOfContent()

        self._doTableOfFigures = makeTableOfFigures
        self._doTableOfTables = makeTableOfTables

        self._reportBuild = False

    def __del__(self):
        """ensure we build the report finally"""
    ...
    def _GotoNextSection(self, sectionType):
        """Goto next section type. Handles all counters
        :param sectionType: The started section type.
        """
        self._currentSectionType = sectionType
        # increment section counter
        for idx in xrange(sectionType, SECTION_TYPE_LAST_TYPE + 1):
            if idx == sectionType:
    ...
    def _BuildSectionString(self, sectionName):
        """Builds the section string
        :return: section string for current section
        """
        return "%s %s" % (self._BuildSectionNumberString(), sectionName)
        ...
        if self._currentSectionType > SECTION_TYPE_NONE:
            for idx in xrange(self._currentSectionType + 1):
                if self._appendix and (idx == 0):
                    number_str = number_str + ("%s" % ASCII_UPPER_LETTERS[self._sectionNumbers[idx] - 1])
                else:
                    number_str = number_str + ("%d" % self._sectionNumbers[idx])
    ...
    def addSection(self, sectionName, pageBreak=True, level=0):
        ...
        self._report.append(Paragraph(self._BuildSectionString(sectionName), self.header[level]))
    ...
    def insertTable(self, tableName, columnNameList, valueList, **kwargs):
        ...
        if tableName is not None:
            tp = Paragraph("<b>%s %s.%d</b>: %s" % (TABLE_CAPTION, self._BuildSectionNumberString(),
                                                    self._tableNumber, self.replaceHTMLChars(tableName)),
                           self.tableTS)
            tp.keepWithNext = True
            self._report.append(tp)
    ...
    def insertImage(self, imageDesc, image):
        ...
        """
        if type(image) != Image and type(image) == str and oPath.isfile(image):
            image = Image(image)

        flowables = [Spacer(1, 1 * cm), image]
        if imageDesc is not None:
            flowables.append(Paragraph("<b>%s %s.%d</b>: %s" %
                                       (FIGURE_CAPTION, self._BuildSectionNumberString(),
                                        self._figNumber, self.replaceHTMLChars(imageDesc)), self.figureTS))
    ...
    def insertParagraph(self, text):
        ...
        """
        self._report.append(Paragraph(text, NORMAL_STYLE))
