        ...
        if (BUFREQ_OK == bufReqRetVal)
        {
...
#endif
          {
            ...
            if (
                ...
                ||
                (pduType == SECOC_SECURED_PDU)
                ||
                (
                  (pduType == SECOC_CRYPTO_PDU) &&
                  (reqBufferSizeUpperLayer != 0U)
                )
               )
            {
  ...
  if (SECOC_INIT == SecOC_InitStatus)
  {
       ...
       )
    {
      ...
      if (result == E_OK)
      {
        ...
        {
          bufReqRetVal = SecOC_RxCheckIncomingPdu(SecOC_RxData[pduIdx].ReceivedTpSecuredDataLength, pduIdx, pduType);
          if (bufReqRetVal == BUFREQ_OK)
          {
        ...
        else
        {
...
#endif
          {
            if (pduType == SECOC_AUTH_PDU)
            {
              bufReqRetVal = SecOC_RxCheckIncomingPdu(SecOC_RxData[pduIdx].ReceivedTpAuthDataLength,
                                                      pduIdx,
                                                      pduType
                                                     );
              if (bufReqRetVal == BUFREQ_OK)
            ...
            {
              bufReqRetVal = SecOC_RxCheckIncomingPdu(SecOC_RxData[pduIdx].ReceivedTpCryptoDataLength,
                                                      pduIdx,
                                                      pduType
                                                     );
              if (bufReqRetVal == BUFREQ_OK)
