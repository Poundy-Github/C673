...
#include <SoAd_BufferPoolingStatistics.h> /* Macros for Buffer Pooling Statistics */
 ...
 **/
#define SoAd_GetSoConTxBufferState(TxBufferId, SoAdSoConTxBufferStatePtr ) \
do \
{ \
  SOAD_PRECONDITION_ASSERT(TxBufferId != SOAD_INVALID_SOCON_ID, DET_INTERNAL_API_ID); \
  (SoAdSoConTxBufferStatePtr) = &SOAD_PBRAM(SoConTxBufferState, TxBufferId);\
}while(0)
 ...
 **            of the socket connection.
 **/
#define SoAd_GetSoConTxBufferCfg(TxBufferId, SoAdSoConTxBufferCfgPtr ) \
do \
{  \
  SOAD_PRECONDITION_ASSERT(TxBufferId != SOAD_INVALID_SOCON_ID, DET_INTERNAL_API_ID); \
  (SoAdSoConTxBufferCfgPtr) = &SOAD_CFG(SoConTxBufferCfg, TxBufferId);\
}while(0)
...
/** \brief SoAd_GetSoConTxUdpData() returns pointer to SoConTxUdpData
 **
 ** SoAd_GetSoConTxUdpData() returns pointer to the SoConTxTcpData struct of the corresponding socket
 ** connection. An invalid memory access will be detected by defensive programming mechanism.
 **
 ** \param[in] SoConPtr - Pointer to socket connection configuration
 **
 ** \param[out] SoConTxUdpDataPtr - Pointer to the SoConTxUdpData struct of the socket connection.
 **/
#define SoAd_GetSoConTxUdpData(SoConPtr, SoConTxUdpDataPtr ) \
do \
{  \
  SOAD_PRECONDITION_ASSERT(SoConPtr->SoConTxUdpDataId != SOAD_INVALID_SOCON_ID, DET_INTERNAL_API_ID); \
  (SoConTxUdpDataPtr)  = &SOAD_PBRAM(SoConTxUdpData, SoConPtr->SoConTxUdpDataId);\
}while(0)
...
TS_MOD_PRIV_DECL FUNC(SoAd_PduTxTcpDataIdType, SOAD_CODE) SoAd_GetUdpTxBuffer
(
  P2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr
);

TS_MOD_PRIV_DECL FUNC(SoAd_PduTxTcpDataIdType, SOAD_CODE) SoAd_CheckUdpTxBuffer
(
  P2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr
);

TS_MOD_PRIV_DECL FUNC(void, SOAD_CODE) SoAd_CheckedReleaseUdpTxBuffer
(
  P2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr
);
