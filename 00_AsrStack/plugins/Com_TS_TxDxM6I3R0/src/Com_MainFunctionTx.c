...
#if (COM_TXMODEBEHAVIOUR == COM_CUSTOM1)
        /* do not send out cyclic transmission when n-times is ongoing */
        if (FALSE == n_times_running)
#endif /* (COM_TXMODEBEHAVIOUR == COM_CUSTOM1) */
        {
...
)
{
   ...
   if (Com_InitStatus == COM_EB_INIT)
   {
      ...
      /* call CbkTxPrep callback */
      Com_CbkTxPrep_Array[ComMainFunctionsTx_Idx] ();
