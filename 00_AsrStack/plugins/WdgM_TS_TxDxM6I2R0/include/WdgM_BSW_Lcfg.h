...
typedef struct
{
  CONSTP2VAR(WdgM_LocalStatusType, AUTOMATIC, AUTOMATIC) WdgM_EB_SELocalStatus; /* !LINKSTO WDGM.EB.DesignDecision.InternalVariable1,4 */
  /**< local statuses of SE from a specific core */
#if (WDGM_EB_GET_ALL_EXPIRED_SE == STD_ON)
  P2VAR(uint8, AUTOMATIC, AUTOMATIC) WdgM_EB_ExpiredSEIDs; /* !LINKSTO WDGM.EB.DesignDecision.InternalVariable27,1 */
  /**< holds the all the expired SEs */
  CONST(uint8, WDGM_CONST) WdgM_EB_ExpiredSEIDArraySize;
  /**< holds the maximum size of expired SEs */
  P2VAR(uint8, AUTOMATIC, AUTOMATIC) WdgM_EB_ExpiredSEIDsCounter; /* !LINKSTO WDGM.EB.DesignDecision.InternalVariable28,1 */
...
#if (WDGM_EB_MULTICORE_ENABLED == STD_ON)
  CONSTP2VAR(WdgM_ModeType, AUTOMATIC, AUTOMATIC) WdgM_EB_CoreSwitchMode; /* !LINKSTO WDGM.EB.DesignDecision.InternalVariable14,1 */
