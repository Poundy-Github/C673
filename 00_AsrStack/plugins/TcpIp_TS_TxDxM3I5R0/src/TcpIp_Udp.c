...
/** \brief Reads and resets dropped datagrams caused by invalid destination UDP-Port
 *
 ...
 */
STATIC FUNC(void, TCPIP_CODE) TcpIp_Udp_GetAndResetInvalidSocketCount
(
  boolean MeasurementResetNeeded,
  P2VAR(uint32, AUTOMATIC, TCPIP_APPL_DATA) MeasurementDataPtr
);

/** \brief Reads and resets dropped datagrams caused by invalid checksum for UDP packets
 *
 * This function reads and resets dropped datagrams caused by invalid checksum for UDP packets.
 *
 * \param[in] MeasurementResetNeeded   Flag to trigger a reset of the measurement
 * \param[out] MeasurementDataPtr      Reference to data buffer
 */
STATIC FUNC(void, TCPIP_CODE) TcpIp_Udp_GetAndResetInvalidChecksumCount
(
  boolean MeasurementResetNeeded,
  P2VAR(uint32, AUTOMATIC, TCPIP_APPL_DATA) MeasurementDataPtr
);

/** \brief Reads and resets dropped datagrams caused by invalid packet format for UDP packets
 *
 * This function reads and resets dropped datagrams caused by invalid packet format for UDP packets.
 *
 * \param[in] MeasurementResetNeeded   Flag to trigger a reset of the measurement
 * \param[out] MeasurementDataPtr      Reference to data buffer
 */
STATIC FUNC(void, TCPIP_CODE) TcpIp_Udp_GetAndResetInvalidPacketFormatCount
(
  boolean MeasurementResetNeeded,
  P2VAR(uint32, AUTOMATIC, TCPIP_APPL_DATA) MeasurementDataPtr
);

/** \brief Increment TcpIp_Udp_InvalidChecksumCounter value
 *
 * This function will increment TcpIp_Udp_InvalidChecksumCounter if max value is not reached.
 */
STATIC FUNC(void, TCPIP_CODE) TcpIp_Udp_IncrementInvalidChecksumCount
(
  void
);
...
STATIC VAR(uint32, TCPIP_VAR) TcpIp_Udp_InvalidChecksumCounter = 0U;
STATIC VAR(uint32, TCPIP_VAR) TcpIp_Udp_InvalidPacketFormatCounter = 0U;
#endif /* TCPIP_GETANDRESETMEASUREMENTDATA_API == STD_ON */
...
)
{
  ...
  if(TcpIp_IpAddrM_getIfState(localAddrId) == TCPIP_STATE_ONLINE)
  {
    if(lenByte >= TCPIP_UDP_HEADER_LENGTH)
    {
        ...
        if((rxChecksum == 0x0U) || (checksum == 0U))
        {
          ...
          if(numSocketsFound > 0U)
          {
            ...
            for(i = 0U; i < (uint8_least)numSocketsFound; i++)
            {
              ...
              {
                TcpIp_Udp_IncrementInvalidChecksumCount();
              }
#endif /* TCPIP_GETANDRESETMEASUREMENTDATA_API == STD_ON */
      ...
      {
        retVal = TCPIP_IP_RX_INVALID_HEADER;
      }
    }
    else
    {
...
#if (TCPIP_GETANDRESETMEASUREMENTDATA_API == STD_ON)
  if(retVal == TCPIP_IP_RX_INVALID_HEADER)
  {
    /* invalid Udp packet format */
    /* increment counter of dropped PDUs caused by invalid packet format */
    /* !LINKSTO TcpIp.Design.Udp.EA6.rxIndication,1 */
    SchM_Enter_TcpIp_SCHM_TCPIP_EXCLUSIVE_AREA_6();
    if(TcpIp_Udp_InvalidPacketFormatCounter < 0xFFFFFFFFU)
    {
      /* !LINKSTO TcpIp.Design.Udp.Atomic.GetAndResetInvalidUdpPacketFormatCount,1 */
      TS_AtomicAssign32(TcpIp_Udp_InvalidPacketFormatCounter, (TcpIp_Udp_InvalidPacketFormatCounter+1U));
    }
    /* !LINKSTO TcpIp.Design.Udp.EA6.rxIndication,1 */
    SchM_Exit_TcpIp_SCHM_TCPIP_EXCLUSIVE_AREA_6();
  }
...
#if (TCPIP_GETANDRESETMEASUREMENTDATA_API == STD_ON)
TS_MOD_PRIV_DEFN FUNC(void, TCPIP_CODE) TcpIp_Udp_GetAndResetCounter
(
...
{
  DBG_TCPIP_UDP_GETANDRESETCOUNTER_ENTRY(MeasurementIdx, MeasurementResetNeeded, MeasurementDataPtr);
  ...
  switch(MeasurementIdx)
  {
    case TCPIP_MEAS_DROP_UDP:
      ...
      break;
    case TCPIP_MEAS_DROP_INVALID_PACKET_FORMAT_UDP:
      TcpIp_Udp_GetAndResetInvalidPacketFormatCount(MeasurementResetNeeded, MeasurementDataPtr);
      break;
    case TCPIP_MEAS_DROP_INVALID_CHECKSUM_UDP:
      TcpIp_Udp_GetAndResetInvalidChecksumCount(MeasurementResetNeeded, MeasurementDataPtr);
      break;
    case TCPIP_MEAS_ALL:
      TcpIp_Udp_GetAndResetInvalidSocketCount(TRUE, NULL_PTR);
      TcpIp_Udp_GetAndResetInvalidPacketFormatCount(TRUE, NULL_PTR);
      TcpIp_Udp_GetAndResetInvalidChecksumCount(TRUE, NULL_PTR);
      break;
    /* CHECK: NOPARSE */
    default:
  ...
  DBG_TCPIP_UDP_GETANDRESETCOUNTER_EXIT(MeasurementIdx, MeasurementResetNeeded, MeasurementDataPtr);
}
#endif /* TCPIP_GETANDRESETMEASUREMENTDATA_API == STD_ON */
...
)
{
  ...
  {
    /* !LINKSTO TcpIp.Design.Udp.Atomic.GetAndResetInvalidUdpPacketFormatCount,1 */
    TS_AtomicAssign32(*MeasurementDataPtr, TcpIp_Udp_InvalidPacketFormatCounter);
  }
  if(MeasurementResetNeeded)
  {
    /* !LINKSTO TcpIp.Design.Udp.Atomic.GetAndResetInvalidUdpPacketFormatCount,1 */
    TS_AtomicAssign32(TcpIp_Udp_InvalidPacketFormatCounter, 0U);
  }
}
...
)
{
  if(MeasurementDataPtr != NULL_PTR)
  {
    /* !LINKSTO TcpIp.Design.Udp.Atomic.GetAndResetInvalidUdpChecksum,1 */
    TS_AtomicAssign32(*MeasurementDataPtr, TcpIp_Udp_InvalidChecksumCounter);
  }
  if(MeasurementResetNeeded)
  {
    /* !LINKSTO TcpIp.Design.Udp.Atomic.GetAndResetInvalidUdpChecksum,1 */
    TS_AtomicAssign32(TcpIp_Udp_InvalidChecksumCounter, 0U);
  }
}
...
)
{
  ...
  /* increment counter of dropped PDUs caused by invalid checksum */
  /* !LINKSTO TcpIp.Design.Udp.EA6.rxIndication,1 */
  SchM_Enter_TcpIp_SCHM_TCPIP_EXCLUSIVE_AREA_6();
  if(TcpIp_Udp_InvalidChecksumCounter < 0xFFFFFFFFU)
  {
    /* !LINKSTO TcpIp.Design.Udp.Atomic.GetAndResetInvalidUdpChecksum,1 */
    TS_AtomicAssign32(TcpIp_Udp_InvalidChecksumCounter, (TcpIp_Udp_InvalidChecksumCounter+1U));
  }
  /* !LINKSTO TcpIp.Design.Udp.EA6.rxIndication,1 */
  SchM_Exit_TcpIp_SCHM_TCPIP_EXCLUSIVE_AREA_6();
}
