...
/* CONTI_CHANGE
 * The base of this file before the CONTI CHANGE is the staterware 01_02_05_08
 */
...
#include "stdio.h"
#include "qspi.h"
/* CONTI_CHANGE Start, rename include file */
#include "qspiflash_driver.h"
/* CONTI_CHANGE End, rename include file */
#include "wd_timer.h"
#include "soc.h"
...
volatile uint32_t SPIWRITER_uSdfOffset = 0;
volatile uint32_t SPIWRITER_uFileSize = 0;
volatile uint32_t SPIWRITER_uFullErase = 0;

int32_t SPIWRITER_v_PreFlashHook(void){
	uint32_t i = 0xAA;
	return i;
}
...
int main(void) {
    qspi_DeviceType_e DeviceType = DEVICE_TYPE_WINBOND_QSPI4;
...
#endif
/* CONTI_CHANGE Start, no manuell user input */
//    while (1)
//    {
//        printf(
...
//            break;
//        }
//        else if (usrInputFlashType == 2)
//        {
//            DeviceType = DEVICE_TYPE_SPANSION_QSPI4;
//            break;
//        }
//        else if (usrInputFlashType == 3)
//        {
//            DeviceType = DEVICE_TYPE_MICRON_QSPI1;
//            break;
//        }
//        else if (usrInputFlashType == 4)
//        {
...
int32_t qspiwriter(void)
{
    uint32_t offset = SPIWRITER_uSdfOffset;
    uint32_t fileSize = SPIWRITER_uFileSize;
...
//    uint32_t  offset, fileSize, numBytesRead, totalBytesRead, input;
    ...
    uint32_t eraseWhole = SPIWRITER_uFullErase;
    uint8_t *tmp, *srcAddr;
    uint32_t *dstAddr;
...
//            scanf("%u", &input);
    srcAddr = tmp = (uint8_t *) (0x82000000);
