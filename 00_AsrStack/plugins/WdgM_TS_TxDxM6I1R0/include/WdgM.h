...
#if (defined WDGM_SID_GET_ALL_EXPIRED_SEID)
#error WDGM_SID_GET_ALL_EXPIRED_SEID already defined
#endif
/** \brief Service id of WdgM_GetAllExpiredSEID() */
#define WDGM_SID_GET_ALL_EXPIRED_SEID   0x1CU
...
/** \brief Get all supervised entities that have expired
 **
 ** Returns all the supervised entities that have expired and their total number.
 **
 ** \pre The WdgMGetAllExpiredSEIDs configuration parameter is enabled
 ** \pre Parameter \a ExpiredSEID is valid pointer
 ** \pre Parameter \a NoOfExpiredSEID is valid pointer
 **
 ** \param[out] ExpiredSEID List of expired supervised entities
 ** \param[out] NoOfExpiredSEID The number of expired supervised entities
 ** \return Success of operation
 ** \retval E_OK     Operation successful
 ** \retval E_NOT_OK Operation failed
 **
 ...
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous} */
extern FUNC(Std_ReturnType, WDGM_CODE) WdgM_GetAllExpiredSEID
(
