...
/* Prototype of function Dem_GetEventFreezeFrameData() can be
   generated automatically by RTE, based on the parameter
   DEM_DEFAULT_ASR_SERVICE_API or DEM_ENABLE_ASR40_SERVICE_API,
   in RTE_Dem.h which ends up included by Dem.c. When RTE is not active
   prototype shall be visible in Dem.h to ensure Dem.c is still compiling.
   These 2 setups, with respect to function prototype visibility, shall be
   mutual exclusive. To ensure this we have the construction above. */
...
/* Prototype of function Dem_ASR42_GetEventFreezeFrameData() can be
   generated automatically by RTE, based on the parameter
   DEM_DEFAULT_ASR_SERVICE_API or DEM_ENABLE_ASR42_SERVICE_API,
   in RTE_Dem.h which ends up included by Dem.c. When RTE is not active
   prototype shall be visible in Dem.h to ensure Dem.c is still compiling.
   These 2 setups, with respect to function prototype visibility, shall be
   mutual exclusive. To ensure this we have the construction above. */
 ...
 ** \retval DEM_WRONG_FILTER    filter is not accepted (Filter settings
 **                             that are not representing a diagnostic message, i.e.
 **                             "DEM_J1939DTC_PERMANENT + DEM_DTC_KIND_ALL_DTCS" and
 **                             "DEM_J1939DTC_CURRENTLY_ACTIVE + DEM_DTC_KIND_EMISSION_REL_DTCS"
 **                             are not accepted and return DEM_WRONG_FILTER.)
 ...
 **                                 The value is limited to 250, which is in accordance
 **                                 with the definition of diagnostic messages.
 **
 ...
 **
 ** \return SPN available or not
 ** \retval DEM_FILTERED_OK                  SPN  is found
 ** \retval DEM_FILTERED_NO_MATCHING_ELEMENT SPN is not available
 **
 ** \ServiceID{::DEM_SID_J1939DcmGetNextSPNInFreezeFrame}
 ** \Reentrancy{Non Reentrant}
 ** \Synchronicity{Asynchronous}
 */
 ...
 **                         contain valid data.
 **
 ** \return Indicates the availability status of the ratio
...
/** \brief Service sets the filter to the first applicable DTC for the DM31
 **        response for a specific node.
 **
 ** Note: Functionality not implemented, only stub available.
 **
 ** \param[in] node Nm node Id of requesting client.
 **
 ** \ServiceID{::DEM_SID_J1939DcmFirstDTCwithLampStatus}
 ** \Reentrancy{Non Reentrant}
 ** \Synchronicity{Synchronous}
 */
extern FUNC(void, DEM_CODE) Dem_J1939DcmFirstDTCwithLampStatus(uint8 node);

/** \brief Service to report the next filtered J1939 DTC for DM31 including current LampStatus..
 **
 ** Note: Functionality not implemented, only stub available.
 **
 ** \param[out] LampStatus       Receives the lamp status returned by this function.
 **                              If the return value of the function is other than
 **                              DEM_FILTERED_OK this parameter does not contain valid data.
 ** \param[out] J1939DTC         Receives the J1939DTC value. If the return value of
 ...
 ** \return DTC available or not
 ** \retval DEM_FILTERED_OK                   DTC is found
 ** \retval DEM_FILTERED_NO_MATCHING_ELEMENT  No further element (matching the filter criteria) found
 **
 ** \ServiceID{::DEM_SID_J1939DcmGetNextDTCwithLampStatus}
 ** \Reentrancy{Non Reentrant}
 ** \Synchronicity{Synchronous}
 */
extern FUNC(Dem_ReturnGetNextFilteredElementType, DEM_CODE) Dem_J1939DcmGetNextDTCwithLampStatus(
  ...
  P2VAR(uint32, AUTOMATIC, AUTOMATIC)                     J1939DTC,
  P2VAR(uint8, AUTOMATIC, AUTOMATIC)                      OccurenceCounter);
