 ...
 **
 ** \return Std_ReturnType value indicating whether the event entry was
 **         processed successfully or a DET error occurred while reading ED or
 **         FF data
 ** \retval E_OK                   Event entry successfully processed
 ** \retval DEM_E_NODATAAVAILABLE  At least one external data element could not
 **                                be read
 **
 ** \Reentrancy{Non reentrant}
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_ProcessEventEntry(
 ...
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_ProcessEventEntryFFData(
  Dem_EventIdType             EventId,
 ...
 **                           to store the DevFFs
 **
 ** \return Result of development FF storage/update
 ** \retval DEM_EVENTENTRY_UPDATED             development FF update was done
 ** \retval DEM_EVENTENTRY_NO_UPDATE_REQUIRED  development FF update was not required
 ** \retval DEM_E_NODATAAVAILABLE              At least one external data element
 **                                            could not be read
 **
 ** \Reentrancy{Non reentrant}
...
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_StoreDevFFs(
  Dem_EventIdType             EventId,
  Dem_EventMemoryEntryPtrType EvMemEntryPtr);
#endif
#endif /* (DEM_NUM_FFSEGS > 0U) */
 ...
 ** \retval DEM_EVENTENTRY_UPDATED             ED update was done
 ** \retval DEM_EVENTENTRY_NO_UPDATE_REQUIRED  ED update was not required
 ** \retval DEM_E_NODATAAVAILABLE              At least one external data element
 **                                            could not be read
 **
 ** \Reentrancy{Non reentrant}
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_ProcessEventEntryEData(
  Dem_EventIdType                  EventId,
 ...
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_StoreED(
...
#if (DEM_DEV_ERROR_DETECT == STD_ON)
    if (DEM_E_NODATAAVAILABLE == Dem_StoreFF(
                                   DEM_REGULAR_FF,
                                   &Dem_FFClass[Dem_GbiFFClassIdx(EventId)],
                                   PFFEntryData,
                                   EventId))
    {
      /* report the DET error that occured on reading FF data */
      DEM_REPORT_ERROR(DEM_SID_PrestoreFreezeFrame, DEM_E_NODATAAVAILABLE);
    }
#else
    (void)Dem_StoreFF(
      DEM_REGULAR_FF,
    ...
    do
    {
      ...
      (DEM_FREEZE_FRAME_CAPTURE != DEM_TRIGGER_TESTFAILED) )
        ...
        Dem_ProcessEvent(DEM_SID_MainFunction, EventInfo, EventStatus,
                         FirstFailed, ProcessEventJobs, &Notification);
  ...
  Dem_EventNotificationDataPtrType Notification)
{
  const Dem_EventIdType EventId = EventInfo.EventId;
#if (DEM_DEV_ERROR_DETECT == STD_ON)
  Std_ReturnType ProcessEntryResult = E_OK;
#endif
  ...
  DBG_DEM_PROCESSEVENT_ENTRY(ServiceId, EventInfo, EventStatus, FirstFailed,
                             Jobs, Notification);
  ...
  if ((Jobs & DEM_PROCESSEVENT_ENTRYUPDATE) != 0x00U)
  {
    ...
    {
#if (DEM_DEV_ERROR_DETECT == STD_ON)
      ProcessEntryResult = Dem_ProcessEventEntry(
                             EventInfo, EventStatus, Notification);
#else
      (void) Dem_ProcessEventEntry(EventInfo, EventStatus, Notification);
#endif
    }
  }
...
#if (DEM_DEV_ERROR_DETECT == STD_ON)
  if (ProcessEntryResult == DEM_E_NODATAAVAILABLE)
  {
    /* report outside exclusive area the DET error that occured on reading ED
     * or FF data; ServiceId indicates the calling context which may be
     * Dem_MainFunction() or Dem_SetEventStatus() */
  ...
  DBG_DEM_PROCESSEVENT_EXIT(ServiceId, EventInfo, EventStatus, FirstFailed,
                            Jobs, Notification);
}
  ...
  Dem_EventNotificationDataPtrType Notification)
{
  ...
  {
#if ( ( (DEM_NUM_EDSEGS > 0U) || (DEM_NUM_FFSEGS > 0U) || \
        ( (DEM_NUM_J1939FFSPNS > 0U) && (DEM_J1939_FREEZEFRAME_SUPPORT == STD_ON) ) ) && \
      ( (DEM_USE_CB_TRIG_ON_EVDAT == STD_ON) || (DEM_DEV_ERROR_DETECT == STD_ON) ) )
...
#endif
    {
#if ( (DEM_USE_CB_TRIG_ON_EVDAT == STD_ON) || (DEM_DEV_ERROR_DETECT == STD_ON) )
      /* process extended data for event */
      ProcessEDAndFFResult = Dem_ProcessEventEntryEData(
                               EventId, EvMemEntryPtr, EventEntryAllocResult);
#if (DEM_USE_CB_TRIG_ON_EVDAT == STD_ON)
      if (ProcessEDAndFFResult != DEM_EVENTENTRY_NO_UPDATE_REQUIRED)
      {
        Notification->TriggerOnEventData = EventId;
      }
#endif /* DEM_USE_CB_TRIG_ON_EVDAT */
#if (DEM_DEV_ERROR_DETECT == STD_ON)
      /* if value DEM_E_NODATAAVAILABLE is returned, it is used later
       * in Dem_ProcessEvent(), outside the exclusive area, to report
       * a DET error that occurred while reading ED */
      if (ProcessEDAndFFResult == DEM_E_NODATAAVAILABLE)
      {
      ...
      (void) Dem_ProcessEventEntryEData(
               EventId, EvMemEntryPtr, EventEntryAllocResult);
#endif /* (DEM_USE_CB_TRIG_ON_EVDAT == STD_ON) || (DEM_DEV_ERROR_DETECT == STD_ON) */
    }
#endif /* (DEM_NUM_EXT_DATAELEMENTS > 0U) */
#endif /* (DEM_NUM_EDSEGS > 0U) */
...
#endif /* DEM_FREEZE_FRAME_CAPTURE == DEM_TRIGGER_ON_FDC_THRESHOLD */
    {
      ...
      ProcessEDAndFFResult = Dem_ProcessEventEntryFFData(EventId, EvMemEntryPtr);
#if (DEM_USE_CB_TRIG_ON_EVDAT == STD_ON)
      if (ProcessEDAndFFResult != DEM_EVENTENTRY_NO_UPDATE_REQUIRED)
      {
        Notification->TriggerOnEventData = EventId;
      ...
      /* if value DEM_E_NODATAAVAILABLE is returned, it is used later
       * in Dem_ProcessEvent(), outside the exclusive area, to report
       * a DET error that occurred while reading FF data */
      if (ProcessEDAndFFResult == DEM_E_NODATAAVAILABLE)
      {
        Result = DEM_E_NODATAAVAILABLE;
      }
...
#if ( (DEM_USE_CB_TRIG_ON_EVDAT == STD_ON) || (DEM_DEV_ERROR_DETECT == STD_ON) )
      ProcessEDAndFFResult = Dem_ProcessEventEntryJ1939FFData(EventId, EvMemEntryPtr);
#if (DEM_USE_CB_TRIG_ON_EVDAT == STD_ON)
      if (ProcessEDAndFFResult != DEM_EVENTENTRY_NO_UPDATE_REQUIRED)
      {
        Notification->TriggerOnEventData = EventId;
      ...
      /* if value DEM_E_NODATAAVAILABLE is returned, it is used later
       * in Dem_ProcessEvent(), outside the exclusive area, to report
       * a DET error that occurred while reading J1939 FF data */
      if (ProcessEDAndFFResult == DEM_E_NODATAAVAILABLE)
      {
        Result = DEM_E_NODATAAVAILABLE;
      }
  ...
  DBG_DEM_PROCESSEVENTENTRY_EXIT(Result, EventInfo, EventStatus, Notification);
  return Result;
}
...
#if (DEM_DEV_ERROR_DETECT == STD_ON)
    if (DEM_E_NODATAAVAILABLE == Dem_StoreDevFFs(EventId, EvMemEntryPtr))
    {
      /* if value DEM_E_NODATAAVAILABLE is returned, it is used later
       * in Dem_ProcessEvent(), outside the exclusive area, to report
       * a DET error that occurred while reading FF data;
       * Note: a development freeze frame shall not trigger an event data
       * changed callback and therefore intentionally does not change the
       * Result to DEM_EVENTENTRY_UPDATED. */
      Result = DEM_E_NODATAAVAILABLE;
    }
#else
    (void)Dem_StoreDevFFs(EventId, EvMemEntryPtr);
#endif /* (DEM_DEV_ERROR_DETECT == STD_ON) */
  }
#endif /* (DEM_DEV_FREEZE_FRAME_USED == STD_ON) */

  DBG_DEM_PROCESSEVENTENTRYFFDATA_EXIT(Result, EventId, EvMemEntryPtr);
  return Result;
}
#endif /* (DEM_NUM_FFSEGS > 0U) */
  ...
  Dem_EventEntryAllocResultType    EventEntryAllocResult)
{
  ...
  DBG_DEM_PROCESSEVENTENTRYEDATA_EXIT(
    Result, EventId, EvMemEntryPtr, EventEntryAllocResult);
  return Result;
}
#endif /* DEM_NUM_EXT_DATAELEMENTS > 0U */
#endif /* (DEM_NUM_EDSEGS > 0U) */
  ...
  Dem_EventIdType                                     EventId)
{
  ...
  DBG_DEM_STOREFF_ENTRY(FFKind, IndividPartFFClass, FFEntryData, EventId);
  ...
  DBG_DEM_STOREFF_EXIT(
    Result, FFKind, IndividPartFFClass, FFEntryData, EventId);
  return Result;
}
  ...
  Dem_EventMemoryEntryPtrType EvMemEntryPtr)
{
       ...
       ++IdxDevFFKind)
  {
    ...
    if (ProcessFFStorage == TRUE)
    {
      ...
      if (IdxDevFF < SizeDevFFMem)
      {
        ...
        /* the result of FF storage is returned, in case a DET error has to be
         * reported outside exclusive area opened in Dem_ProcessEvent */
        Result = Dem_StoreFF(
          IdxDevFFKind,
  ...
  DBG_DEM_STOREDEVFFS_EXIT(Result, EventId, EvMemEntryPtr);
  return Result;
}
#endif /* (DEM_DEV_FREEZE_FRAME_USED == STD_ON) */
#endif /* (DEM_NUM_FFSEGS > 0U) */
