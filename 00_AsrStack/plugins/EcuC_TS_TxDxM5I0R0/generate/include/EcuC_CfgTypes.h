...
typedef struct
{
  ...
  uint16 metaDataItemListNum; /* number of configured meta data items for this PDU */
  uint8  metaDataTypeLength ; /* length of the meta data type */
} EcuC_PduConfigType;
...
typedef struct
{
  EcuC_PduConfigType pduConfig[[!"num:i($EcuC_Number_Of_PduIds)"!]U]; /* list of configured PDUs, from PduId 0 to maximal configured PduId */
  EcuC_MetaDataItemConfigType metaDataItemConfig[[!"num:i($EcuC_Meta_Data_Item_List_Size)"!]U]; /* list of configured meta data items, sorted according to PDU Id */
