...
)
{
    ...
    if (LINIF_UNINIT_ACCESS)
    {
        ...
        (void)Det_ReportError(LINIF_MODULE_ID, LINIF_INSTANCE_ID,
         ...
         ||  (LINIF_CH_IS_SLAVE == (LINIF_CFG_GET_CHANNEL[Channel].CfgFlags & LINIF_NODETYPE_MASK)))
    {
        (void)Det_ReportError(LINIF_MODULE_ID, LINIF_INSTANCE_ID,
...
#endif /* (LINIF_DEV_ERROR_DETECT == STD_ON) */
    {
...
#endif /* (LINIF_DEV_ERROR_DETECT == STD_ON) */
        {
            ...
            TableIndex = LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[Channel].LinIfChIndex)->SchTableInd +
            ...
            if(Channel != (NumChannels - 1U))
            {
                ...
                if (TableIndex >= LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[Channel].LinIfChIndex + 1U)->SchTableInd)
                {
                    ReportDet = TRUE;
                }
            }
...
)
{
    ...
    if ( LINIF_CFG_GET_SCHTABLE[TableIndex].RunMode != LINIF_RUN_ONCE )
    {
        ...
        if ( TableIndex ==  LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[internalChannel].LinIfChIndex)->SchTableInd )
        {
