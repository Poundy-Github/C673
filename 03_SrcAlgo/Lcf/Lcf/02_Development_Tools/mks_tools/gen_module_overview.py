...
def writeXMLFile(fileDirectory, resultFileName, finalDict, checkpoint_name, logger, ReleaseInfoDict, hoCommentFile):
    ...
    try:
        ...
        # Stripping not needed characters from the string:
        sheetName = strip_checkpointname(checkpoint_name)
        ...
        if (rb is not None) and (rb.get_sheet_names() is not None) :
            if (rb is not None)  :
                ...
                if sheetName in rb.get_sheet_names():
                    ...
                    if (strlen < 31) :
                        logger.info("Creating Worksheet: '%s'" % sheetName)
                ...
                if(successFlag and (not (worksheet is None))):
                    ...
                    for key, value in finalDict.iteritems():
                        ...
                        for key1, value1 in value.iteritems():
                            ...
                            previousValue = worksheetPrev.cell(row=row, column=col + ColCnt).value

                            # Removing of non-ASCII characters is necessary
                            if previousValue is not None:
                                logger.info("Non-ASCII characters may have to be removed from '%s'", previousValue)
                                previous_val = ''.join([i if ord(i) < 128 else '' for i in previousValue])
                                logger.info("Checkpoint value after removal: '%s'", previous_val)
...
def check_checkpointname(checkpoint_name):
    logger = logging.getLogger("check_checkpointname")
    logger.info("Checkpoint: '%s' type: '%s'", checkpoint_name, type(checkpoint_name))
    if isinstance(checkpoint_name, list):
        for name in checkpoint_name:
            if not check_checkpointname(name):
                logger.info("name: '%s' type: '%s'", (checkpoint_name, type(checkpoint_name)))
                return False
        logger.info("Checkpoint: '%s' type: '%s'", checkpoint_name, type(checkpoint_name))
        return True
    if not isinstance(checkpoint_name, (str, unicode)):
        logger.info("Checkpoint is not type of str or unicode: '%s'", type(checkpoint_name))
        return False
    regex = "^[\.A-Za-z0-9_-]*$"
    if not re.match(regex, checkpoint_name):
        logger.info("Checkpoint does not match regex '%s'", regex)
        return False
    return True

def checker_checkpoints(checkpoint_name):
    logger = logging.getLogger("checker_checkpoints")
    if isinstance(checkpoint_name, (str, unicode)):
        if check_checkpointname(checkpoint_name):
            return True
    elif isinstance(checkpoint_name, list):
        for name in checkpoint_name:
            if not check_checkpointname(name):
                return False
        return True
    else:
        logger.info("checkpoint_name: %s is type of: %s", checkpoint_name, type(checkpoint_name))
    return False

def strip_checkpointname(checkpoint_name):
    ...
    stripped_checkpoint_name = checkpoint_name
    if checkpoint_name.startswith('AL_'):
        stripped_checkpoint_name = checkpoint_name[len('AL_'):]
        stripped_checkpoint_name = re.sub('[.-]', '', stripped_checkpoint_name)
    return stripped_checkpoint_name
