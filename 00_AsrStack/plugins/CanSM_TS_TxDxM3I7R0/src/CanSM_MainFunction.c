...
/*==================[macros]================================================*/
#define CANSM_GENERIC_MAINFUNCTION 0u
...
)
{
  ...
  if (CanSM_GlobalInit == CANSM_GLOBAL_INIT)
  {
    ...
    for ( nw=0U; nw<CANSM_NETWORKS_NUM; nw++ )
    {
      ...
      /* !LINKSTO CanSM.EB.CanSMDistributedChannels_MainFunctions_Processing_Generic,1 */
      if (CANSM_NW_INFO_PARTITIONID(nw) == CANSM_GENERIC_MAINFUNCTION)
      {
        CanSM_MainFunctionInternal(nw);
      }
      else
      {
          /* Nothing to do, this nw will be handled in another MainFunction at another partition */
      }
#else
      CanSM_MainFunctionInternal(nw);
#endif
