  ...
  [!IF "not(var:defined('mfRxAlloc'))"!]
    ...
    [!IF "CanIfPublicCfg/CanIfMultiCoreSupport = 'true'"!]
      [!LOOP "as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfRxProcessing/*"!]
        [!VAR "mfRxAlloc" = "concat($mfRxAlloc, ';', node:pos(.), '#', node:name(.), '#', node:name(as:ref(CanIfPartitionRef)), '_MainFunctionRx_', node:name(.), '#', text:join(./CanIfRxPduProcessingRef/*,'|'))"!]
      [!ENDLOOP!]
    [!ELSE!]
      [!LOOP "as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfRxProcessing/*"!]
  ...
  [!IF "not(var:defined('mfTxAlloc'))"!]
    ...
    [!IF "CanIfPublicCfg/CanIfMultiCoreSupport = 'true'"!]
      [!LOOP "as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfTxProcessing/*"!]
        [!VAR "pos" = "node:pos(.)"!]
        [!VAR "mfTxAlloc" = "concat($mfTxAlloc, ';', node:pos(.), '#', node:name(.), '#', node:name(as:ref(CanIfPartitionRef)), '_MainFunctionTx_', node:name(.), '#', text:join(./CanIfTxPduProcessingRef/*,'|'))"!]
      [!ENDLOOP!]
    [!ELSE!]
      [!LOOP "as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfTxProcessing/*"!]
        [!VAR "pos" = "node:pos(.)"!]
