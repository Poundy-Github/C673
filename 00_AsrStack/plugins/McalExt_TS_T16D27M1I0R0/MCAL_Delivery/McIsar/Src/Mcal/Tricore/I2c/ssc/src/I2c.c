...
#if (I2C_SW_MAJOR_VERSION != 20U)
#error "I2C_SW_MINOR_VERSION does not match. "
#endif

#if (I2C_SW_MINOR_VERSION !=0U)
#error "I2C_SW_MINOR_VERSION does not match. "
#endif

#if (I2C_SW_PATCH_VERSION != 2U)
...
)
{
  ...
  {
    /* Report to  DET */
    (void)Det_ReportError(I2C_MODULE_ID, I2C_INSTANCE_ID, I2C_SID_INIT, \
                    I2C_E_INIT_FAILED );
...
)
{
  ...
  {
    /* Report to DET */
    (void)Det_ReportError(I2C_MODULE_ID, I2C_INSTANCE_ID, I2C_SID_DEINIT, \
                    I2C_E_UNINIT);
    ReturnValue = E_NOT_OK;
  }
  if (ReturnValue == E_OK)
...
)
{
  ...
  {
    /* Report to DET */
    (void)Det_ReportError(I2C_MODULE_ID, I2C_INSTANCE_ID, \
  ...
  while(FIFOCounter < FilledFIFOStage)
  {
    ...
    if((FIFOCounter == 0U) && (TxdCnt == 0U))
    {
       ...
       else
       {
          ...
          *interfacing with the HW is needed. Also, final value being assigned
  ...
  if(ChannelInfo->RxTransferState == I2C_RX_INITIATED)
  {
    ...
    else
    {
      /* Calculate the upper slave byte */
        ...
        HwModulePtr->TXD.U = ((uint32)UpperSlaveAddr |
                              (uint32)I2C_READ_OPERATION);
    }
