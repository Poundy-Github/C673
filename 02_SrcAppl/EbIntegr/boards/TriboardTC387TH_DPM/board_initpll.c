/* board_initpll.c
 *
 * This file contains the function boardInitPLL(), which
 ...
 *
 * $Id: board_initpll.c 33382 2021-03-03 08:00:10Z arla271030 $
*/
...
#include "board_pll_tc3xx.h"

#define TMPL_MODBASE_SCU 0xf0036000u

static void Tmpl_BoardWait(os_uint32_t);
static void Tmpl_BoardWaitValsEqual(os_reg32_t*, os_uint32_t);
static void Tmpl_BoardWaitForCCUCONIsUnlocked(os_reg32_t*);
static void Tmpl_BoardWaitBitSet(os_reg32_t*, os_uint32_t);


static void Tmpl_BoardWait(os_uint32_t delay)
{
	os_reg32_t vltCnt;
	/*
	...
	*/
	...
	OS_WriteSafetyEndinit(0);
}

static void Tmpl_BoardWaitValsEqual(os_reg32_t* vLeft, os_uint32_t vRight)
{
	os_reg32_t vltCounter = 0;
	os_uint32_t iterStep = TMPL_FREQ_STBL_DELAY >> 3;
...
static void Tmpl_BoardWaitForCCUCONIsUnlocked(os_reg32_t* lockReg)
{
	os_reg32_t vltCounter = 0;
	os_uint32_t iterStep = TMPL_FREQ_STBL_DELAY >> 3;
...
static void Tmpl_BoardWaitBitSet(os_reg32_t* val, os_uint32_t bitMask)
{
	os_reg32_t vltCounter = 0;
	os_uint32_t iterStep = TMPL_FREQ_STBL_DELAY >> 3;
...
*/
void boardInitPLL(void)
{
	os_reg32_t tmpVal_ccucon0, tmpVal_ccucon1, tmpVal_ccucon2;
	os_tricore_scu_t * const scu = (os_tricore_scu_t *)TMPL_MODBASE_SCU;
	...
	scu->osccon = (scu->osccon & ~TMPL_OSCCON_MODE_MASK) | TMPL_CCUCON_OSCVAL;
