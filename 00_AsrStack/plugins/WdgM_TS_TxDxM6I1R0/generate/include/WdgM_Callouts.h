...
[!IF "$ASR32ServiceAPIEnabled = 'true'"!][!//
...
/** \brief Indicates a violation of main function schedule time.
 */
extern FUNC(void, WDGM_CODE) [!"$MainFunctionViolationCalloutName"!]
(
  void
);
 ...
 *  \return    void
 */
extern FUNC(void, WDGM_CODE) [!"$RequestPartitionResetCalloutName"!]
(
...
/** \brief The function returns the current state of an OS-Application.
 ...
 *  \retval E_OS_ID:  Application is not valid (only in EXTENDED status).
 */
extern FUNC(StatusType, WDGM_CODE) [!"$GetApplicationStateCalloutName"!]
(
  ApplicationType Application,
