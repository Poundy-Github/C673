...
def __add_core_producer_consumer_cluster(graph_generator, cluster, ip_channel, ip_channel_node, task_to_groups, tasks, dc, producer_consumer_cores):
    ...
    if tasks_per_core:
        for task in tasks_per_core:
            producer_consumer_node = graph_generator.get_or_create_node(producer_consumer_core_cluster, task, """{{name}}
priority: {{priority}}
schedule: {{taskSchedule}}
cooperative groups: """ + str(task_to_groups[task] if task_to_groups.get(task) is not None else "-"), style=TASK_STYLE)
            if is_producer:
...
def create_graph(graph_generator):
    ...
    for ip_channel in ip_channels:
        ...
        cluster = graph_generator.get_or_create_cluster(
            ip_channel_id, ip_channel.findtext("name"))
