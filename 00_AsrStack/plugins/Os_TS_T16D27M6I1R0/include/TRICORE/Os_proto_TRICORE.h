 ...
 *
 * $Id: Os_proto_TRICORE.h 30867 2019-09-26 10:39:33Z sash270423 $
*/
...
OS_QUAL_INLINE os_intstatus_t OS_IntDisableLevel(os_intstatus_t dislevel) OS_ATTR_INLINE;
OS_QUAL_INLINE os_intstatus_t OS_IntDisableLevel(os_intstatus_t dislevel)
{
	register os_uint32_t oldIcr = OS_MFCR(OS_ICR);
...
*/
#define OS_IntDisableAll() OS_IntDisableLevel(OS_intDisableLevelAll)

/* OS_IntDisableMax() sets the ICR to the "disable" state (from the
 * constant OS_intDisableLevelMax) and returns the previous state.
*/
#define OS_IntDisableMax() OS_IntDisableLevel(OS_intDisableLevelMax)
...
typedef struct os_int_ir_config_driver_s
{
	os_setup_interrupt_fn_t setupInterrupt;
	os_enable_interrupt_fn_t enableInterrupt;
	os_disable_interrupt_fn_t disableInterrupt;
} os_int_ir_config_driver_t;
#define OS_InitIntIrConfigDriver(setup, enable, disable) { (setup), (enable), (disable) }
...
{
	OS_intIrConfigDriver.setupInterrupt(isr->arch.srn, OS_SRN_TOS_CPU | isr->arch.arbPrio);
}
