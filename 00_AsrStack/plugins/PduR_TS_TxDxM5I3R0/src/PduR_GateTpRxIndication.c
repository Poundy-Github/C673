...
)
{
   ...
   if(CallLoTpTransmit)
   {
      ...
      /* store old head index since we might need to restore this one in the case
      ...
      pSessionInfos[SessionIndex].PendTxConfCount = pGTabTpRx[GMPduId].RemainingEntries + 1U;
      ...
      do{
         ...
         /* call internal handler function of PduR_GateTpTransmit */
         Std_ReturnType RetVal = PduR_GateTpTransmit(GTabPos, SessionIndex, TargetAPIRef, &PduR_PduInfo, &RemainingEntries);
      ...
      /* release TP gateway session when no further TpTxConfirmation is pending what can be caused by:
         ...
         - TpTxConfirmation preemptied by other TpTxConfirmation of same routing path */
      if(pSessionInfos[SessionIndex].PendTxConfCount == 0U)
      {
         /* release assigned TP gateway session */
         PduR_ReleaseGateTpSession(&pSessionInfos[SessionIndex]);
