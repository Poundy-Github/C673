...
)
{
  ...
  else
  {
    for (id = 0U; id < LINSM_NUM_CHANNELS; id++)
    {
      ...
      {
        /* !LINKSTO LinSM.ASR20-11.SWS_LinSM_00234_EB,1 */
...
#if (LINSM_DEV_ERROR_DETECT == STD_ON)
                  /* !LINKSTO LinSM.ASR20-11.SWS_LinSM_00307,1 */
                  TimerIsZero = TRUE;
#endif
...
)
{
  ...
  if (SilenceAfterWakeupTimer != LINSM_INVALID_SILENCE_AFTER_WAKEUP)
  {
    ...
    {
      /* !LINKSTO LinSM.ASR20-11.SWS_LinSM_00236,1 */
...
)
{
  ...
  {
    /* !LINKSTO LinSM.ASR20-11.SWS_LinSM_00235,1 */
