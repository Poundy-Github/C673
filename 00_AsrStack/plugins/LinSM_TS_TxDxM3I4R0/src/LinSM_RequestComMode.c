...
/** \brief process slave channel in GOTOSLEEP state and
 *         call indications to upper layer if needed.
 *
 * checks if channel is in GOTOSLEEP state, if confirmation is expected or not and
 * calls BswM and ComM if LinSM should report indications.
 *
 * \param[in] callIndications   boolean that decides if indications are called or not
 * \param[in] network           ComM network for indications
 * \param[in] mode              mode to report to ComM in case transition is successful
 * \param[in] id                LinSM channel id of channel to operate on
 */
STATIC FUNC(void, LINSM_CODE) LinSM_ProcessSlaveGotoSleep
(
  boolean callIndications,
...
)
{
  ...
  boolean callIndications = FALSE;
    ...
    GotoSleepInProgress = LINSM_CHANNEL_SLAVE_GOTOSLEEPINPROGRESS(id);
#endif
    ...
    switch (mode)
    {
      case COMM_FULL_COMMUNICATION:
      {
        ...
        {
          /* !LINKSTO LinSM.ASR20-11.SWS_LinSM_00237,1 */
#if (LINSM_SLAVE_NODES_ENABLED == STD_ON)
          ...
          if ((SilenceAfterWakeupTimer == LINSM_INVALID_SILENCE_AFTER_WAKEUP) && (GotoSleepInProgress == FALSE))
#endif
          {
#if (LINSM_SLAVE_NODES_ENABLED == STD_ON)
            LINSM_ENTER_CRITICAL_SECTION(); /* protect channel flags */
...
#if (LINSM_SLEEPSUPPORT_TYPE == LINSM_ALL)
            /* !LINKSTO LinSM.ASR20-11.SWS_LinSM_00047,1 */
            LinSM_WakeUp(id);
            ...
            TS_AtomicAssign8(LINSM_CHANNEL_REPETITION_TIME(id),LINSM_CHANNEL_MODEREQUQSTREPETITIONTIME(id));
          }
        }
        else
        {
          LINSM_ENTER_CRITICAL_SECTION(); /* protect channel data */
      ...
      case COMM_NO_COMMUNICATION:
      {
             ...
             )
          {
            ...
            {
              /* !LINKSTO LinSM.ASR20-11.SWS_LinSM_00230_EB,1 */
              TS_AtomicAssign8(LINSM_CHANNEL_STORED_COM(id),NOTHING_STORED);
            }
            else
#endif
            {
          ...
          else
          {
            ...
            {
              LinSM_ProcessSlaveGotoSleep(callIndications, network, mode, id);
            }
            else
#endif
            {
              LINSM_ENTER_CRITICAL_SECTION(); /* protect channel timer and stored mode */
...
)
{
  ...
  /* save current state */
  currentState = LINSM_CHANNEL_STATE(id);
  /* !LINKSTO LinSM.ASR20-11.SWS_LinSM_00302,1 */
  ...
  if (LINSM_CHANNEL_IS_SLAVE(id) == TRUE)
  {
    if (retval == E_OK)
    {
      /* !LINKSTO LinSM.ASR20-11.SWS_LinSM_00233_EB,1 */
...
{
  LINSM_ENTER_CRITICAL_SECTION(); /* protect channel timer, state and flags */
