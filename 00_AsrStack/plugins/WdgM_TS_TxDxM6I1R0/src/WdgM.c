...
/* !LINKSTO WDGM.EB.Design.IncludeFileStructure,2 */
...
/* patch version check */
#if (WDGM_SW_PATCH_VERSION < 40U)
#error WDGM_SW_PATCH_VERSION wrong (< 40U)
#endif
...
)
{
  ...
  if (WdgM_EB_SECfg[InternalSEId].OsPartitionRefEnabled == TRUE)
  {
    ...
    WDGM_EB_REQUEST_PARTITION_RESET(FailedOsApp);
...
)
{
  ...
  for (CorePartitionIdx = 0U; CorePartitionIdx < CoreInstance->NumOsPartitions; CorePartitionIdx++)
  {
    if (WdgM_EB_GetPartitionStatus(CoreInstance->CoreID, CorePartitionIdx) == WDGM_EB_PARTITION_RESTARTING)
    {
      ...
      RetVal = WDGM_EB_GET_APPLICATION_STATE(WdgM_EB_GetPartitionId(CoreInstance->CoreID, CorePartitionIdx), &OsPartitionState);

      if ((RetVal == E_OK) && (OsPartitionState == APPLICATION_ACCESSIBLE))
      {
        uint8 LocalSize;
