 ...
 * \param[in,out] dataLengthPtr     in:  Number of bytes to copy
 *                               out: Number of actual copied bytes
 *
 * \retval BUFREQ_OK        copying successful
 * \retval BUFREQ_E_NOT_OK  <Up>_CopyTxData returned error
 * \retval BUFREQ_E_BUSY    <Up>_CopyTxData returned busy
 */

STATIC FUNC(BufReq_ReturnType, TCPIP_CODE) TcpIp_Tcp_txData_copyDataFromSoAd
(
  TcpIp_SocketIdType SocketId,
  uint8 socketOwnerId,
  P2VAR(uint8, AUTOMATIC, TCPIP_APPL_DATA) memoryPtr,
  P2VAR(uint32, AUTOMATIC, TCPIP_APPL_DATA) dataLengthPtr
);
 ...
 * \param[in,out]  lengthPtr     in:    Number of bytes to copy
 *                            out:   Number of actual copied bytes
 *
 * \retval BUFREQ_OK        copying successful
 * \retval BUFREQ_E_NOT_OK  <Up>_CopyTxData returned error
...
STATIC FUNC(BufReq_ReturnType, TCPIP_CODE) TcpIp_Tcp_txData_bufferData
(
  TcpIp_SocketIdType SocketId,
  uint8 socketOwnerId,
  ...
  uint32 index,
  P2VAR(uint32, AUTOMATIC, TCPIP_APPL_DATA) lengthPtr
);
      ...
      if(availBuff != 0U)
      {
        ...
        if((FreeBuffSize >= copyLength) || ((DataPtr == NULL_PTR) && (!ForceRetrieve)))
        {
          ...
          if(copyLength != 0U)
          {
            ...
            retValTemp = TcpIp_Tcp_txData_bufferData
                     (
                      socketId,
            ...
            if((retValTemp == BUFREQ_OK) || ((!ForceRetrieve) && (retValTemp == BUFREQ_E_BUSY)))
            {
              ctrlBlockPtr->used_buff += copyLength;
              retVal = TCPIP_OK;
            }
            else
            {
...
)
{
  ...
  do
  {
    ...
    } while((currlength > 0U) && (retVal == BUFREQ_OK));
  *lengthPtr -= currlength;
...
)
{
  ...
  uint32 currDatLength = *dataLengthPtr;
  ...
  do
  {
    ...
    retVal = TcpIp_CopyTxDataAPI[socketOwnerId](SocketId, memoryPtr, Length);
    if(retVal == BUFREQ_OK )
    {
      currDatLength -= Length;
    }

  } while((currDatLength > 0U) && (retVal == BUFREQ_OK));

  /* return actual number of copied bytes */
  *dataLengthPtr -= currDatLength;
