...
#if (LINIF_DEM_EVENTS_CONFIGURED)
/** \brief This function processes DEM reporting
 *
 *  \param currentChannel   Channel which is currently processed
 *  \param pIfChannel       Pointer to channel runtime data
 *  \param status           Current status from Lin_GetStatus
 *  \param pPduInfo         Received data
 */
STATIC FUNC(void, LINIF_CODE) LinIf_DemReporting
(
    uint8 currentChannel,
    LinIf_ChannelInfoType *pIfChannel,
    Lin_StatusType status,
...
/** \brief Global state of the LIN channels regarding ongoing sleep handling */
VAR(Atomic_t, LINIF_VAR) LinIf_SleepHandlingInProgress[LINIF_MAX_CHANNELS];
...
)
{
    ...
    if ( LinIf_InitState == LINIF_INIT)
    {
        ...
        {
#if (LINIF_MASTER_SUPPORTED == STD_ON)
          if (LINIF_CH_IS_MASTER == (LINIF_CFG_GET_CHANNEL[channelIdx].CfgFlags & LINIF_NODETYPE_MASK))
          {
...
#if (LINIF_SLAVE_SUPPORTED == STD_ON)
          if (LINIF_CH_IS_SLAVE == (LINIF_CFG_GET_CHANNEL[channelIdx].CfgFlags & LINIF_NODETYPE_MASK))
          {
            CONSTP2VAR(LinIf_ChannelInfoType, AUTOMATIC, LINIF_VAR) pIfChannel =
              &LinIf_ChannelInfo[channelIdx];
            boolean timerBusIdleExpired = FALSE;
...
#if (LINIF_EOS_NOTIF_CALLOUT_SUPPORTED == STD_ON)
    else if (FALSE != pIfChannel->FailedFrame)
    {
      P2FUNC(void, LINIF_NOTIF_CALLOUT_CODE, EOSNotifFuncPtr)(void) = LINIF_CFG_GET_EOS_NFPTR(pIfChannelMaster->SchCurrentTable);
      pIfChannelMaster->CallEOSNotif = FALSE;
...
)
{
    ...
    CONSTP2CONST(LinIfMasterChannelType, AUTOMATIC, LINIF_VAR) pIfChannelConfigMaster =
            LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[currentChannel].LinIfChIndex);
...
#if (LINIF_DEM_EVENTS_CONFIGURED)
        LinIf_DemReporting(currentChannel, pIfChannel, status, pPduInfo);
#endif /* LINIF_DEM_EVENTS_CONFIGURED */
  ...
  if(status == LIN_CH_SLEEP)
  {
      ...
      if(Result == E_NOT_OK)
      {
        ...
        const uint16 NullSchedule =
            LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[currentChannel].LinIfChIndex)->SchTableInd;
...
)
{
  ...
  CONSTP2CONST(LinIfSlaveChannelType, AUTOMATIC, LINIF_VAR) pIfChConfSlave =
      LINIF_CFG_GET_CHANNEL_SLAVE(LINIF_CFG_GET_CHANNEL[currentChannel].LinIfChIndex);
    CONSTP2VAR(LinIf_SlaveChannelInfoType, AUTOMATIC, LINIF_VAR) pIfChannelSlave =
      LINIF_GET_CHANNEL_SLAVE(currentChannel);
  uint8 sleepFlags;
  Std_ReturnType Result = E_OK;
  boolean lockSleepHandling = FALSE;

  const NetworkHandleType ComMChannel =
     LINIF_GET_COMM_CHANNEL(currentChannel);

  lockSleepHandling = (boolean)Atomics_TestAndSetFlag(&LinIf_SleepHandlingInProgress[currentChannel], (Atomic_value_t)0x1U);

  if (lockSleepHandling == FALSE)
  {
    ...
    Atomics_ClearFlag(&LinIf_SleepHandlingInProgress[currentChannel], (Atomic_value_t)0x1U);
  }
}
#endif /* LINIF_SLAVE_SUPPORTED == STD_ON */
...
)
{
    ...
    if(ret == E_OK)
    {
        ...
        if(dataLength != 0U)
        {
            ...
            StatusDelay =
                LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[currentChannel].LinIfChIndex)->DelayTime[dataLength - 1U];
        }
...
#if (LINIF_EOS_NOTIF_CALLOUT_SUPPORTED == STD_ON)
    else if (FALSE != pIfChannelMaster->CallEOSNotif)
    {
      pIfChannel->FailedFrame = TRUE;
    }
    else
    {
      /* do nothing for other frames */
    }
#endif
