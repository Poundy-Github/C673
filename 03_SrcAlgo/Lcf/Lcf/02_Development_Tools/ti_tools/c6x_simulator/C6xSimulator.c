...
// as this is TI code, we will not mess around with it and simply disable all warnings
#pragma warning( push, 0) // store the current warning level and switch to warning level 0 (i.e. no warnings)
...
#ifdef TMS320C64X
static volatile uint32 GFPGFR;  /* Galois field polynomial generator */
                                /* function register                 */
#endif

#ifdef TMS320C64PX
// 2020-6-3, need to link to registers from outside, why were the reg's static anyway?
...
/*
 * Searches in the bit-representation of 'value' for a leftmost 1 or 0 (depends on 'b').
 * Returns the number of bits up to the bit change.
 * Example:
 * 1, 0001000000... => returns 3
 * 1, 0111000000... => returns 1
 * 0, 0111000000... => returns 0
 */
uint32 _lmbd(uint32 b, uint32 value)
{
  uint32 bitPos = 0;
  uint32 bitmask = MIN_INT32; // bit-mask: 10000000000000000000000000000000
  ...
  if (b == 1)
      while((value & bitmask) == 0)
      {
          bitmask >>= 1;
          bitPos++;
          if (bitPos == 32) return 32; // no match found at all
      }
  else
      while((value & bitmask) != 0)
      {
...
int32 _sshvl(int32 a,int32 b)
{
  int32 tmp;
  int32 y32;
  if(b < -31) b = -31;      // fix by SB ! (shift right towards 0/-1 works as intended)
 ...
 *  Changed: $Date: 2021/12/13 19:18:07CET $
