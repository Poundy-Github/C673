 ...
 *
 * $Id: Os_kernel_task.h 34931 2021-08-16 09:54:42Z beoe851 $
 */
...
/*!
 *OS_IsValidCat2Isr
 * Checks if the ISR is valid category 2 ISR.
 */
#define OS_IsValidCat2Isr(isr) (((isr->flags)&(OS_ISRF_CAT)) == (OS_ISRF_CAT2))

#if (OS_N_CORES == 1)
#define OS_DoDisableInterruptSource(i, isr)		OS_LocalDisableInterruptSource(isr)
#define OS_DoEnableInterruptSource(i, isr, f)	OS_LocalEnableInterruptSource((isr), (f))
#define OS_DoClearPendingInterrupt(i, isr)		OS_LocalClearPendingInterrupt(isr)
#else /* OS_N_CORES */
#define OS_DoDisableInterruptSource(i, isr)		OS_MultiDisableInterruptSource((i), (isr))
#define OS_DoEnableInterruptSource(i, isr, f)	OS_MultiEnableInterruptSource((i), (isr), (f))
#define OS_DoClearPendingInterrupt(i, isr)		OS_MultiClearPendingInterrupt((i), (isr))
#endif
...
#define OS_INTRUSTED		OS_U(5)		/* In trusted function */
#define OS_INSHUTDOWN		OS_U(6)		/* Shut down */
#define OS_ININTERNAL		OS_U(7)		/* In some internal kernel function */

#define OS_INHOOKMIN		OS_U(8)		/* Hook functions lower limit */
#define OS_INSTARTUPHOOK	OS_U(8)		/* In startup hook */
#define OS_INSHUTDOWNHOOK	OS_U(9)		/* In shutdown hook */
#define OS_INERRORHOOK		OS_U(10)		/* In error hook */
#define OS_INPRETASKHOOK	OS_U(11)	/* In pre-task hook */
#define OS_INPOSTTASKHOOK	OS_U(12)	/* In post-task hook */
#define OS_INPREISRHOOK		OS_U(13)	/* In pre-isr hook */
#define OS_INPOSTISRHOOK	OS_U(14)	/* In post-isr hook */
#define OS_INPROTECTIONHOOK	OS_U(15)	/* In protection hook */
#define OS_INHOOKMAX		OS_U(15)	/* Hook functions upper limit */
...
#define OS_INTRUSTEDMASK		OS_INFUNCMASK(OS_INTRUSTED)
...
#define OS_CALLHOOKVOID_DIRECT(hf,h) \
	do {														\
		os_kerneldata_t * const OS_kd = OS_GetKernelData();		\
		os_uint8_t Os_CallHookTmp = OS_kd->inFunction;			\
		OS_kd->inFunction = (h);								\
		OS_TRACE_HOOK_ENTRY(h);									\
		(hf)();													\
		OS_TRACE_HOOK_EXIT_P(h);								\
		OS_kd->inFunction = Os_CallHookTmp;						\
	} while (0)
...
#define OS_CALLAPPSTARTUPHOOKS() \
	do {													\
		...
		{													\
			...
			{												\
				OS_kd->inFunction = OS_INSTARTUPHOOK;		\
				OS_CallStartupHook(casuhApp);				\
				OS_kd->inFunction = OS_ININTERNAL;			\
			}												\
			casuhApp++;										\
		}													\
	} while (0)
...
#define OS_CALLAPPSHUTDOWNHOOKS(ec) \
	do {													\
		...
		{													\
			...
			{												\
				OS_kd->inFunction = OS_INSHUTDOWNHOOK;		\
 ...
 *
 * !LINKSTO Kernel.Autosar.OsApplication.ApplicationHooks.ErrorHook.Call.OnlyIfErrorCodeReturned, 2
 */
...
os_boolean_t OS_CheckRateMonitor(os_ratemonitorid_t);
...
extern void OS_DisableAllInterruptSources(os_uint8_t mask);
...
extern os_errorresult_t OS_LocalDisableInterruptSource(const os_isr_t *);
extern os_errorresult_t OS_LocalEnableInterruptSource(const os_isr_t *, os_boolean_t);
extern os_errorresult_t OS_LocalClearPendingInterrupt(const os_isr_t *);
#if (OS_N_CORES != 1)
...
#if (OS_N_CORES == 1)
#define OS_DoActivateTask(ts)      	OS_LocalActivateTask((ts))
#define OS_DoActivateTaskAsyn(ts)	OS_LocalActivateTask((ts))
#define OS_DoSetEventAsyn(t,ts,ev)	OS_LocalSetEvent((ts),(ev))
#define OS_DoSetEvent(t,ts,ev)		OS_LocalSetEvent((ts),(ev))
#define OS_DoGetEvent(t,ts,ep)		OS_LocalGetEvent((ts),(ep))
#else /* OS_N_CORES */
#define OS_DoActivateTask(ts)		OS_MultiActivateTask((ts))
#define OS_DoActivateTaskAsyn(ts)	OS_MultiActivateTaskAsyn((ts))
#define OS_DoSetEvent(t,ts,ev)		OS_MultiSetEvent((t),(ts),(ev))
#define OS_DoSetEventAsyn(t,ts,ev)	OS_MultiSetEventAsyn((t),(ts),(ev))
#define OS_DoGetEvent(t,ts,ep)		OS_MultiGetEvent((t),(ts),(ep))
extern os_errorresult_t OS_MultiActivateTask(const os_task_t *);
extern os_errorresult_t OS_MultiActivateTaskAsyn(const os_task_t *);
extern os_errorresult_t OS_MultiSetEvent(os_taskid_t, const os_task_t *, os_eventmask_t);
extern os_errorresult_t OS_MultiSetEventAsyn(os_taskid_t, const os_task_t *, os_eventmask_t);
extern os_errorresult_t OS_MultiGetEvent(os_taskid_t, const os_task_t *, os_eventmask_t *);
extern os_errorresult_t OS_MultiDisableInterruptSource(os_isrid_t, const os_isr_t *);
extern os_errorresult_t OS_MultiEnableInterruptSource(os_isrid_t, const os_isr_t *, os_boolean_t);
extern os_errorresult_t OS_MultiClearPendingInterrupt(os_isrid_t, const os_isr_t *);
#endif /* OS_N_CORES */
