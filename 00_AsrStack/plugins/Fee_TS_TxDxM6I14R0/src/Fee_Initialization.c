...
/** \brief Function to the section index based on working section index if the section is consistent and newer. */
STATIC FUNC(void, FEE_CODE) Fee_ManageActiveSectionIndex(void);
...
)
{
  ...
  if (E_OK == Fls_BlankCheck(FEE_SECTION_START_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) + FEE_SECTION_MANAGEMENT_END_PATTERN_OFFSET,
  ...
  if (E_OK == Fls_BlankCheck(FEE_SECTION_START_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) + FEE_SECTION_MARKER_SIZE_AL,
...
)
{
  ...
  Fls_LengthType BlankCheckSize = (Fee_Gv.Fee_BufferIndex == 0U) ? FEE_CONSISTENCY_PATTERN_SIZE : FEE_SECTION_MANAGEMENT_OBJ_SIZE_AL;
  Fls_AddressType BlankCheckAddress = FEE_FOOTER_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) + FEE_SECTION_MANAGEMENT_END_PATTERN_OFFSET - Fee_Gv.Fee_BufferIndex;
#else /* #if (FEE_CONSISTENCY_PATTERN_ENABLED == STD_ON) */
  Fls_LengthType BlankCheckSize = FEE_SECTION_MANAGEMENT_OBJ_SIZE_AL;
  Fls_AddressType BlankCheckAddress = FEE_FOOTER_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) +  FEE_SECTION_MARKER_SIZE_AL;
#endif /* #if (FEE_CONSISTENCY_PATTERN_ENABLED == STD_ON) */

  /* !LINKSTO Fee.Dsn.BlankCheck.SectionManagement,1 */
  /* Blank check the footer's management object */
  /* In case of consistency pattern, it will be blank checked either the end pattern or the footer's management */
  if (E_OK == Fls_BlankCheck(BlankCheckAddress,
                             BlankCheckSize))
  {
    /* Reset retry counter */
    Fee_Gv.FeeRetryStateCounter = FEE_STATE_MAX_RETRIES;
...
)
{
  ...
  if (E_OK == Fls_BlankCheck(FEE_SECTION_START_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) + FEE_HEADER_ACTIVE_MARKER_OFFSET,
...
)
{
  /* !LINKSTO Fee.Dsn.BlankCheck.SectionMarkers,1 */
  /* Blank check the active marker in header */
  if (E_OK == Fls_BlankCheck(FEE_FOOTER_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) - FEE_FOOTER_BUFFER_IDX_FOR_CRASH,
...
{
  /* !LINKSTO Fee.Dsn.BlankCheck.SectionMarkers,1 */
  if (E_OK == Fls_BlankCheck(FEE_SECTION_START_ADDRESS(Fee_Gv.FeeWorkingSectionIndex),
                             FEE_SECTION_MARKER_SIZE_AL))
  {
    /* Reset retry counter */
    Fee_Gv.FeeRetryStateCounter = FEE_STATE_MAX_RETRIES;
...
)
{
  ...
  if (E_OK == Fls_BlankCheck(FEE_FOOTER_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) + FEE_HEADER_ACTIVE_MARKER_OFFSET,
                             FEE_SECTION_MARKER_SIZE_AL))
  {
    /* Reset retry counter */
    Fee_Gv.FeeRetryStateCounter = FEE_STATE_MAX_RETRIES;
...
)
{
  ...
  if (E_OK == Fls_Read(FEE_SECTION_START_ADDRESS(Fee_Gv.FeeWorkingSectionIndex),
                       Fee_Gv.FeeBuffPtr,
                       FEE_HEADER_SIZE))
#else /* #if (FEE_USE_BLANKCHECK == STD_ON) */
  /* Call flash read to read the header 0 fields and their corresponding markers */
  if (E_OK == Fls_Read(FEE_SECTION_START_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) + FEE_SECTION_MARKER_SIZE_AL,
...
)
{
  ...
  if (E_OK == Fls_Read(FEE_FOOTER_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) - FEE_FOOTER_BUFFER_IDX_FOR_CRASH,
                       Fee_Gv.FeeBuffPtr,
                       FEE_HEADER_SIZE + FEE_FOOTER_BUFFER_IDX_FOR_CRASH))
#else /* #if (FEE_USE_BLANKCHECK == STD_OFF) */
  /* Call flash read to read the footer */
  if (E_OK == Fls_Read(FEE_FOOTER_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) + FEE_SECTION_MARKER_SIZE_AL,
...
)
{
  ...
  if (E_OK == Fls_Read(Fee_Gv.FeeWorkingInfoAddress - (Fls_AddressType)((uint32)Fee_Gv.Fee_NumberOfMarginBlockInfos * (uint32)FEE_BLOCKINFO_SIZE),
...
)
{
  ...
  else
  {
    ...
    Fee_Gv.FeeWorkingInfoAddress -= (Fls_AddressType)((uint32)((uint32)Fee_Gv.Fee_NumberOfMarginBlockInfos + (uint32)1U) * (uint32)FEE_BLOCKINFO_SIZE);
...
)
{
  ...
  if (E_OK == Fls_BlankCheck(FEE_FOOTER_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) - FEE_VIRTUAL_PAGE_SIZE, FEE_VIRTUAL_PAGE_SIZE))
  {
    /* Reset retry counter */
    Fee_Gv.FeeRetryStateCounter = FEE_STATE_MAX_RETRIES;
    Fee_Gv.FeeFlashJobResult = MEMIF_JOB_PENDING;
...
)
{
  ...
  Fls_AddressType BlankCheckAddress;

  BlankCheckAddress = (Fee_Gv.FeeWorkingInfoAddress
                       - (Fls_AddressType)((uint32)Fee_Gv.Fee_NumberOfMarginBlockInfos * (uint32)FEE_BLOCKINFO_SIZE));
  ...
  if (E_OK == Fls_BlankCheck(BlankCheckAddress, FEE_BLOCKSTAMP_SIZE_AL))
  {
    /* Reset retry counter */
    Fee_Gv.FeeRetryStateCounter = FEE_STATE_MAX_RETRIES;
    Fee_Gv.FeeFlashJobResult = MEMIF_JOB_PENDING;
...
)
{
  ...
  else if ((MEMIF_JOB_OK == Fee_Gv.FeeFlashJobResult) && (!FEE_CHECK_FLAG(FEE_FLASH_FINAL_REFUSAL_MASK)))
  {
    ...
    Fee_Gv.FeeWorkingInfoAddress -= (Fls_AddressType)((uint32)Fee_Gv.Fee_NumberOfMarginBlockInfos * (uint32)FEE_BLOCKINFO_SIZE);
...
)
{
  ...
  if (Fee_Gv.FeeWorkingInfoAddress <
      (Fee_Gv.FeeWorkingDataAddress
       + (Fls_AddressType)((uint32)((uint32)Fee_Gv.Fee_NumberOfMarginBlockInfos
...
{
  Fls_AddressType RemainingInfoSize = ((Fee_Gv.FeeWorkingInfoAddress + (Fls_AddressType)FEE_BLOCKINFO_SIZE)
                                      - Fee_Gv.FeeWorkingDataAddress)
                                      - (Fls_AddressType)((uint32)((uint32)Fee_Gv.Fee_NumberOfMarginBlockInfos + (uint32)FEE_DATA_INFO_MARGIN) * (uint32)FEE_BLOCKINFO_SIZE);
  ...
  ReadAddress = (((Fee_Gv.FeeWorkingInfoAddress + (Fls_AddressType)FEE_BLOCKINFO_SIZE)
      - (Fls_AddressType)((uint32)Fee_Gv.Fee_NumberOfMarginBlockInfos * (uint32)FEE_BLOCKINFO_SIZE))
...
)
{
  ...
  {
    ReadAddress = (Fee_Gv.FeeWorkingInfoAddress - (Fls_AddressType)((uint32)Fee_Gv.Fee_NumberOfMarginBlockInfos * (uint32)FEE_BLOCKINFO_SIZE));
    /* If something was found with the last read the FeeInitializationInfoAddress
     * has been already updated to the next one (and Fee_NumberOfMarginBlockInfos is 0).
     * Else, if the area was found empty we must skip the empty area and read
     * the next possible block info. */
...
)
{
   ...
   * the next possible block info. */
  /* Read the next block info */
  if (E_OK == Fls_BlankCheck(BlankCheckAddress, FEE_VIRTUAL_PAGE_SIZE))
  {
    Fee_Gv.FeeFlashJobResult = MEMIF_JOB_PENDING;
  }
  /* flash job was refused */
...
)
{
  ...
  for (i=0U;i<7;i++)
  {
    if (Fee_Gv.FeeBuffPtr[FEE_FOOTER_BUFFER_IDX_FOR_CRASH + FEE_SECTION_MANAGEMENT_OFFSET + i] != FEE_FLASH_ERASE_VALUE)
    {
      SectionErased = FALSE;
      break;
    }
  ...
  {
    ComputedCrc_SMgt = Crc_CalculateCRC8(&Fee_Gv.FeeBuffPtr[FEE_FOOTER_BUFFER_IDX_FOR_CRASH + FEE_CIDFIELD_OFFSET], FEE_CID_CRC_COMPUTE_SIZE, FEE_CRC_START_VALUE, TRUE);
    ...
    if((ReadCidCrc == ComputedCrc_SMgt) && ((ReadConfigId == FEE_CONFIG_ID) || (ReadConfigId == FEE_LEGACY_CONFIG_ID)))
    {
      ...
      ComputedCrc_ECnt = Crc_CalculateCRC8(&Fee_Gv.FeeBuffPtr[FEE_FOOTER_BUFFER_IDX_FOR_CRASH + FEE_SECTION_MANAGEMENT_OFFSET], FEE_ECOUNTER_CRC_COMPUTE_SIZE, FEE_CRC_START_VALUE, TRUE);
...
STATIC FUNC(void, FEE_CODE) Fee_ManageActiveSectionIndex
(
    void
)
{
