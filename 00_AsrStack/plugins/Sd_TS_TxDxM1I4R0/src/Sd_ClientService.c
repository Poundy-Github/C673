        ...
        { /* TTL of service expired */
          ...
          if(ClientServiceStatePtr->RequestedState == SD_CLIENT_SERVICE_REQUESTED)
          {
...
#if(SD_SERVER_NOT_AVAILABLE_CALLBACK == STD_ON)
            {
              ...
              {
                (void)SoAd_GetRemoteAddr(ClientServiceStatePtr->SoConIdTcp, (P2VAR(TcpIp_SockAddrType, TYPEDEF, SD_APPL_DATA))(void *)&TcpRemoteAddr);
          ...
          Sd_ClientService_TTL_Run_Out(ClientServiceCfgPtr, ClientServiceStatePtr, (uint16)ServiceIndex);
          {
            ...
            ClientServiceStatePtr->SdOfferEvents = 0U;
...
#if (SD_CALLOUT_EVENT_MAPPING == STD_ON)
   /* !LINKSTO Sd.EB.CallOut.EventMapping_AVAILABLE,1 */
  {
    TcpIp_SockAddrInet6Type RemoteAddr;
    const uint8 InstanceIndex = ServiceCfgPtr->SdInstanceIndex;

    RemoteAddr.domain = Sd_InstanceStates[InstanceIndex].TxState.Address.domain;

    if(UdpSoConId != SOAD_INVALID_SOCON_ID)
    { /* Deviation MISRAC2012-1 */
      (void)SoAd_GetRemoteAddr(UdpSoConId, (P2VAR(TcpIp_SockAddrType, TYPEDEF, SD_APPL_DATA))(void*)&RemoteAddr);
    }
    else
    { /* Deviation MISRAC2012-1 */
      (void)SoAd_GetRemoteAddr(TcpSoConId, (P2VAR(TcpIp_SockAddrType, TYPEDEF, SD_APPL_DATA))(void*)&RemoteAddr);
    }

    Sd_Callout_EventMapping(ServiceCfgPtr->SdClientServiceID,
                            ServiceCfgPtr->SdClientServiceInstanceId,
                            /* Deviation MISRAC2012-1 */
                            (TcpIp_SockAddrType*)(void*)&RemoteAddr);
  }
#endif /* SD_CALLOUT_EVENT_MAPPING == STD_ON */
...
#if (SD_CALLOUT_EVENT_MAPPING == STD_ON)
  /* !LINKSTO Sd.EB.CallOut.EventMapping_DOWN,1 */
  Sd_Callout_EventMapping(ServiceCfgPtr->SdClientServiceID,
                          ServiceCfgPtr->SdClientServiceInstanceId,
                          NULL_PTR);
#endif /* SD_CALLOUT_EVENT_MAPPING == STD_ON */
