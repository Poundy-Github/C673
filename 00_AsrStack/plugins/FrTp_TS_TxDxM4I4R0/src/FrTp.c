...
/* patch version check */
#if (FRTP_SW_PATCH_VERSION < 27U)
#error FRTP_SW_PATCH_VERSION wrong (< 27U)
#endif
...

        {
            ...
            else
            {
                ...
                if(pConnection->ConnectionLimitConfigIdx != FRTP_NUMBER_CONNECTION_LIMITS)
                {
                    CONSTP2CONST(FrTp_ConnectionLimitConfigType, AUTOMATIC, FRTP_APPL_CONST)
                        pConnectionLimitConfig =
                        FRTP_CFG_GET_PTR_TO_CONNECTION_LIMIT_CONFIG(pConnection->ConnectionLimitConfigIdx);
                    connectionLimit = pConnectionLimitConfig->Limit;
