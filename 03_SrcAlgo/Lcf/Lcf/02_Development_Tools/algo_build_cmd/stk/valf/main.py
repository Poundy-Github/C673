...
# Local Imports
#===============================================================================
import stk.db.db_sql as db_sql
...
def main():
    ...
    optparser.add_option("-e", "--error-tolerance", dest="errortolerance",
                         ...
                         help="The error tolerance level from (%d..%d) [default=%d]"
                              % (db_common.ERROR_TOLERANCE_NONE,
                                 ...
                                 db_common.ERROR_TOLERANCE_NONE))
    optparser.add_option("-r", "--collection", dest="collection",
                         default=None,
                         help="The name of the rec file catalog collection")
    optparser.add_option("-s", "--report-config", dest="reportconfig",
                         default=None,
                         help="The configuration data for the reporter.")
    optparser.add_option("-w", "--write-option", dest="writeoption",
                         default=None,
                         help="Choose SKIP or REPLACE for database import.")
    optparser.add_option("-y", "--import_by", dest="importby",
                         default=None,
                         help="Import by Login Name - required option")
    optparser.add_option("-a", "--assign_port", dest="assignport", nargs=2, action="append",
                         default=None,
    ...
    if not logging_level in log.LEVEL_INFO_MAP:
    ...
    try:
        ...
        for p in assignport:
            process_mgr_obj.set_data_port(p[0], p[1])
            logger.debug("setting input data. [PortName='{0}', PortValue='{1}'] ".format(p[0], p[1]))
    except StandardError:
        pass
    ...
    if mts_batch_pathname is not None:
        if mts_batch_pathname and os.path.isfile(mts_batch_pathname):
            ...
            logger.error("Invalid mts batch play list.")
            optparser.print_help()
            sys.exit(RET_VAL_ERROR)
...
Path changes as validation_main is in a different location than the old start_em_validation.
...
- extended start_em_validation by additional options and changed some existing option tags
...
Initial revision
Member added to project /nfs/projekte1/REPOSITORY/Base_Development/05_Algorithm/
EM_EnvironmentModel/05_Testing/05_Test_Environment/algo/em_req_test/valf_tests/em/common/project.pj
Revision 1.1 2011/03/08 15:57:13CET Castell Christoph (uidt6394) (uidt6394)
Initial revision
Member added to project /nfs/projekte1/REPOSITORY/Base_Development/05_Algorithm/
EM_EnvironmentModel/05_Testing/05_Test_Environment/algo/em_req_test/valf_tests/em/common/01_Source_Code/project.pj
...
Added comments and functionality for database_filename (-f) and config_detector_filename (-x).
