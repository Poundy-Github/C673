  ...
  [!SELECT "as:modconf('Uart')[1]"!][!//
    ...
    ******************************************************************************/!]
    [!MACRO "Uart_ConfigurationCheck"!][!//
        ...
        [/cover] */
          [!LOOP "UartConfigSet/UartChannel/*"!][!//
          [!VAR "AscLinHW" = "UartHwUnit"!]
          [!VAR "tmp" = "num:i(count(./../*[UartHwUnit = $AscLinHW]))"!][!//
            [!IF "num:i($tmp) > num:i(1)"!][!//
            [!ERROR!][!//
              [!"$AscLinHW"!] module shouldn't be configured for more than 1 Uart Channel
            [!ENDERROR!][!//
          [!ENDIF!]
            [!VAR "flag" = "num:i(0)"!][!//
            [!VAR "Count" = "num:i(0)"!][!//
            ...
            [!LOOP "./McuAscLinAllocationConf/*"!][!//
              ...
              [!VAR "McuAscLinChannelAllocationConf" = "McuAscLinChannelAllocationConf/McuAscLinChannelAllocationConf"!]
               [!IF "$AscLinHW = $McuAscLinKernel"!]
                 ...
                 [!VAR "flag" = "num:i(1)"!][!//]
                 [!IF "$McuAscLinChannelAllocationConf = 'ASCLIN_CH_USED_BY_UART_DRIVER' and $Count > num:i(1)"!]
                 [!ERROR!][!//
                    Error : More than one instance of [!"$AscLinHW"!] exists in Mcu container.
                [!ENDERROR!][!//
                [!ELSEIF "$AscLinHW = $McuAscLinKernel and $McuAscLinChannelAllocationConf != 'ASCLIN_CH_USED_BY_UART_DRIVER'and $Count = num:i(1)"!]
                     [!ERROR!]
                        Configure the [!"$AscLinHW"!] Channel for UART in Mcu Configuration container.
                     [!ENDERROR!]
                   [!ENDIF!]
                [!ENDIF!]
            [!SELECT "as:modconf('Uart')[1]"!][!//
                [!IF "$McuAscLinChannelAllocationConf = 'ASCLIN_CH_USED_BY_UART_DRIVER' and num:i(count(./UartConfigSet/UartChannel/*[UartHwUnit = $McuAscLinKernel])) = num:i(0)"!][!//
                    [!ERROR!]
                        ERROR : Configure the [!"$McuAscLinKernel"!] in Uart channel container for UartHwUnit.
                    [!ENDERROR!]
                [!ENDIF!]
            [!ENDSELECT!]
            [!ENDLOOP!]
