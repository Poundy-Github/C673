...
ifneq (,$(filter-out depend clean% $(MAKE_CLEAN_RULES),$(MAKECMDGOALS)))
  ...
  dirlist := $(addprefix $(OUTPUT_DIR)/,$(subst $(BLANK),/../,$(filter-out $(notdir $(wildcard $(OUTPUT_DIR)/*)),generated)))  ifneq (,$(GLOBAL_LIB_ENABLE))
 ...
 # all targets, except clean* & depend

# don't create directories for cleaning & depend, but consider multiple make targets like "make depend all"
ifneq (,$(filter-out depend clean% $(MAKE_CLEAN_RULES),$(MAKECMDGOALS)))
  # check which directories need creation
  # we use gnu make -p (even on DOS), so concatenate them to one path that's efficiently creating the whole tree
  dirlist := $(addprefix $(CUSTOMER_OUTPUT_DIR)/,$(subst $(BLANK),/../,$(filter-out $(notdir $(wildcard $(CUSTOMER_OUTPUT_DIR)/*)),bin depend lib list make obj temp)))
...
clean:
	-$(EB_VERBOSE)cd $(call fixCmdPaths,$(CUSTOMER_OUTPUT_DIR)) && $(RMTREE) $(clean_dirs)
...
clean_all:
  ...
  -$(EB_VERBOSE)$(RMTREE) $(GENERATED_DIR) $(CUSTOMER_OUTPUT_DIR)
