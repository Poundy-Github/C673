...
#define FEE_FOOTER_BUFFER_IDX_FOR_CRASH            ((Fee_State == FEE_INIT_READ_FOOTER) ? FEE_BLOCKINFO_SIZE : 0U)
#else /* #if (FEE_USE_BLANKCHECK == STD_ON) */
#define FEE_FOOTER_BUFFER_IDX_FOR_CRASH            (((((Fee_State == FEE_INIT_READ_FOOTER) || \
                                                     (Fee_State == FEE_BLANK_CHECK_FOOTER)) || \
                                                     (Fee_State == FEE_BLANK_CHECK_FOOTER_ACTIVE)) || \
                                                     (Fee_State == FEE_BLANK_CHECK_FOOTER_ERASABLE)) \
                                                     ? FEE_BLOCKINFO_SIZE : 0U)
#endif /* #if (FEE_USE_BLANKCHECK == STD_ON) */
#else /* (FEE_CRITICAL_BLOCK_NUMBER != 0U) */
...
#define FEE_CHECKSUM_NUMBER_OF_BYTES          (Fls_LengthType)((FEE_BLOCKSTAMP_SIZE) - (FEE_CHECKSUM_SIZE))
...
#if (defined FEE_BLKSCOUNTER_INDEX)
#error FEE_BLKSCOUNTER_INDEX already defined
#endif
/** \brief Index of block's section counter */
#define FEE_BLKSCOUNTER_INDEX                 9U

#if (defined FEE_CHECKSUM_INDEX)
#error FEE_CHECKSUM_INDEX already defined
#endif
/** \brief Index of checksum information in blockinfo */
#define FEE_CHECKSUM_INDEX                    10U

#if (defined FEE_BLOCKSTAMP_SIZE)
#error FEE_BLOCKSTAMP_SIZE already defined
#endif
/** \brief Number of bytes in block stamp */
#define FEE_BLOCKSTAMP_SIZE                    11U
...
#endif
/** \brief Number of bytes in block stamp */
#define FEE_BLOCKSTAMP_SIZE                    10U
...
#define FEE_FIRST_BLOCK_INFO_ADDRESS(Section) \
  (FEE_FOOTER_ADDRESS(Section) - FEE_BLOCKINFO_SIZE)
...
#if (FEE_CONSISTENCY_PATTERN_ENABLED == STD_ON)
#if (defined FEE_RETENTION_MASK)
#error FEE_RETENTION_MASK already defined
#endif
/** \brief Macro used to activate behavior towards data retention at first writing after reset
 **        If the "&" operation with this mask gives the following values it means:
 **        Value 0: No dummy block info will be written to ensure data retention;
 **        Value 1: A dummy block info will be written to ensure data retention.
 **/
#define FEE_RETENTION_MASK                                0x0100U
#endif /* #if (FEE_CONSISTENCY_PATTERN_ENABLED == STD_ON) */
