 ...
 *
 * $Id: kern-dotrytogetspinlock.c 34824 2021-08-05 13:57:39Z ache271795 $
*/
...
static os_autosarspinlockid_t OS_CheckNestedSpinlock(os_objectid_t, os_autosarspinlockid_t);
...
OS_TOOL_FUNCTION_ATTRIB_PRE
static os_autosarspinlockid_t OS_CheckNestedSpinlock(os_objectid_t lastSpinlock, os_autosarspinlockid_t lockId)
OS_TOOL_FUNCTION_ATTRIB_POST
{
	os_autosarspinlockid_t findSpinlock = lockId;
...
OS_TOOL_FUNCTION_ATTRIB_POST
{
	...
	if ( result != OS_E_OK )
	{
		/* Drop out: either failed the calling context check or "out" is null */
	}
	else
	...
	else
	{
		...
		else
		{
			...
			findSpinlock = OS_CheckNestedSpinlock(lastLock, lockId);
...
const os_uint8_t OS_dummy_kern_dotrytogetspinlock_c = 0xeb;
