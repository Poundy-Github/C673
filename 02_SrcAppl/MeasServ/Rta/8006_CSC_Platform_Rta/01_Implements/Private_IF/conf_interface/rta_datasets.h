...
/// \file
/// \brief This file declares the configuration interface regarding the normal dataSets used for the core feature of the RTA.

#ifndef RTA_DATA_SETS_H
#define RTA_DATA_SETS_H
...
/// This structure contains all constant parameters of the RTA for one core.
typedef struct
{
...
/// contains pointers to the parameters and the control variables of one particular core.
typedef struct
{
  /// pointer to all constant parameters for the particular core. \n
  /// Details see in description of  RTA_t_Params.
  const RTA_t_Params* p_params;
  ...
  uint32* const p_callbackCnt;
}RTA_t_DataSet;


typedef struct
{
  const RTA_t_DataSet**const p_dataSets;
  uint32                     u_numberOfCores;
}RTA_t_DataSetHandler;


extern const RTA_t_DataSetHandler rta_dataSetHandler;

void RTA_p_SetDataSet(const RTA_t_DataSet* dataSet);
