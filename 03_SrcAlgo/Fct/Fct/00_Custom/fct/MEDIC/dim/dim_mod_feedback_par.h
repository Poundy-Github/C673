...
PACKAGENAME:               dim_mod_feedback_par.h

DESCRIPTION:               header of the DIM driver feedback parameters

AUTHOR:                    $Author: Muddaveeraiah, Nalina (uidn6869) $

CREATION DATE:             $Date: 2020/06/14 02:08:52CEST $
...
@ingroup Medic_dim
@{ **/
#ifndef DIM_MOD_DRV_FDBCK_PAR_INCLUDED
#define DIM_MOD_DRV_FDBCK_PAR_INCLUDED
...
/*! @brief       DIM Feedback Default Driver Breaking Value
    ...
    @unit        SI-unit     
       ...
       */
#define DIM_FEEDBACK_DEFAULT_DrvBraking         (b_FALSE)
...
/*! @brief       DIM Feedback Default Driver Override Value
    @general     Default Value for Driver Override
    @conseq      @incp  
                 @decp  
    @attention   [None]
    @typical     eDriverOverride_Invalid
    @unit        [enum]    
    
       */
#define DIM_FEEDBACK_DEFAULT_DriverOverride       (eOverrideFdbk_Invalid)
...
/* ****************************************************************
    ...
    **************************************************************** */
/*! @brief DIM Pedal Feedback Parameter Structure, refer to @ref dim_mod_feedback_par_eba.c for values of this struct.
...
typedef struct  
{
  ...
  CML_t_Vector2D fWeakNegCurve[DIM_FEEDBACK_PAR_WeakNegCurve_POINTS]; /*!< Timer dependent weak feedback range @values: float32[DIM_FEEDBACK_PAR_WeakNegCurve_POINTS][2]*/
  #endif
  float32 fHighGasPedalPos;                   /*!< definition for high gas pedal position -> negative feedback*/
  float32 fHighGasPedalGrad;                  /*!< definition for high gas pedal gradient -> negative feedback*/
  float32 fHighGasPedalPosWeakNegH;           /*!< definition for high gas pedal position -> weak negative feedback (higher threshold)*/
  float32 fHighGasPedalPosWeakNegL;           /*!< definition for high gas pedal position -> weak negative feedback (lower threshold)*/
  float32 fHighGasPedalGradWeakNeg;           /*!< definition for high gas pedal gradient -> weak negative feedback*/
  float32 fAutoBrakeJerkLimit;                /*!< typical jerk for autobraking in m/s^3*/
  float32 fAutoBrkTimeNoPosFdbck;             /*!< time (s) where no feedback due to gas pedal falling is allowed after a high negative accel jerk (must be > 0.0!!!)*/
  float32 fVeryHighGasPedalPos;               /*!< definition for very high gas pedal position -> negative feedback without checking autobraking*/
  float32 fVeryHighGasPedalGrad;              /*!< definition for very high gas pedal gradient -> negative feedback without checking autobraking*/
...
#endif
#endif /*DIM_MOD_DRV_FDBCK_PAR_INCLUDED*/
/**@}*/
