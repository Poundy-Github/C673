...
(
  ...
  SoAd_PduTxTcpDataIdType TxBufferId
);
...
STATIC FUNC(Std_ReturnType, SOAD_CODE) SoAd_Handle_Tx_Mode_Indirect
(
  CONSTP2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr,
  ...
  P2VAR(SoAd_SoConTxDataType, AUTOMATIC, SOAD_APPL_DATA) SoConTxDataPtr
);
...
STATIC FUNC(void, SOAD_CODE) SoAd_ResetStateOfIncludedPdus
(
  CONSTP2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr
);

STATIC FUNC(void, SOAD_CODE) SoAd_CopyShadowBufferToTxBuffer
(
  CONSTP2CONST(SoAd_SoConGroupType, AUTOMATIC, SOAD_APPL_DATA) SoConGroupPtr,
  SoAd_PduTxTcpDataIdType TxBufferId,
  P2VAR(SoAd_SoConTxBufferStateType, AUTOMATIC, SOAD_APPL_DATA) TxShadowBufferStatePtr
);
...
STATIC FUNC(Std_ReturnType, SOAD_CODE) SoAd_TxModeSelection_With_HeaderMode
(
  CONSTP2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr,
  CONSTP2CONST(SoAd_PduRouteDestVirtualType, AUTOMATIC, SOAD_APPL_DATA) PduRouteDestVirtualPtr,
  uint16 TcpIpTxLen,
    ...
    /* CHECK: PARSE */
    {
      ...
      if(BusyCnt == 0U)
      {
...

        {
          /* UDP */
          uint8 TxConfFlag;
          /* If Buffer is available, always proceed with udp handling */
          const SoAd_PduTxTcpDataIdType TxBufferId = SoAd_CheckUdpTxBuffer(SoConPtr);
          if(TxBufferId != SOAD_INVALID_SOCON_ID)
          {
            SoAd_MainFunctionIfTxUdp(SoConPtr, SoConStatePtr, TxBufferId);
            SoAd_CheckedReleaseUdpTxBuffer(SoConPtr);
          }
...
TS_MOD_PRIV_DEFN FUNC(SoAd_PduTxTcpDataIdType, SOAD_CODE) SoAd_GetUdpTxBuffer
(
  P2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr
)
{
  SoAd_SoConIdType BufferId = SOAD_INVALID_SOCON_ID;
  if(SoConPtr->SoConTxUdpDataId != SOAD_INVALID_PDUTXUDPDATA_ID)
  {
    P2VAR(SoAd_SoConTxUdpDataType, AUTOMATIC, SOAD_APPL_DATA) SoConTxUdpDataPtr =
      &SOAD_PBRAM(SoConTxUdpData, SoConPtr->SoConTxUdpDataId);
    if(SoConTxUdpDataPtr->TxBufferId != SOAD_INVALID_SOCON_ID)
    {
      BufferId = SoConTxUdpDataPtr->TxBufferId;
    }
    else
    {
      uint32 NextFreeBufferPoolId;
      CONSTP2CONST(SoAd_SoConGroupType, AUTOMATIC, SOAD_APPL_DATA) SoConGroupPtr =
        &SOAD_CFG(SoConGroup, SoConPtr->SoConGroupId);

      CONSTP2VAR(uint32, AUTOMATIC, SOAD_APPL_DATA) BufferPoolInUseBitMap =
          &SOAD_PBRAM_REF_ID(uint32, SoAd_PBcfg->SoAdBufferPoolBitMapBase, 0U);

      const uint32 StartBit = (uint32)SoConGroupPtr->BufferPoolFirstId - 1U;
      const uint32 EndBit = ((uint32)SoConGroupPtr->BufferPoolFirstId)
        + ((uint32)SoConGroupPtr->BufferPoolCount);

      /*Cant use wordwrap here because coverage instrumentation does not work then.*/
      /* Deviation MISRAC2012-3 */
      for(SOAD_COMSTACK_BF_ITERATE(NextFreeBufferPoolId, &BufferPoolInUseBitMap[0], StartBit, EndBit))
      {
        /* Downcast to SoAd_SoConIdType is okay since the result is
           <= SoConGroupPtr->BufferPoolCount which is of same type. */
        const SoAd_SoConIdType BufferPoolIdOffset
          = (SoAd_SoConIdType)(NextFreeBufferPoolId - SoConGroupPtr->BufferPoolFirstId);
        SoConTxUdpDataPtr->TxBufferId = BufferPoolIdOffset + SoConGroupPtr->BufferFirstId;
        BufferId = SoConTxUdpDataPtr->TxBufferId;
        SoAd_ClearBitIn32BitMap
          (
            (NextFreeBufferPoolId),
            &SoAd_PBcfg->SoAdBufferPoolBitMapBase
          );
        break;
      }
      /* This is either a new BufferId or an Invalid one if no buffer is available */
      if(BufferId == SOAD_INVALID_SOCON_ID)
      {
        SOAD_BUFFERPOOLING_UDPPDUDROP(SoConPtr->SoConGroupId);
      }
    }
  }
  /* if we dont even have udp data id there is no chance to retrieve a buffer */
  return BufferId;
}

TS_MOD_PRIV_DEFN FUNC(SoAd_PduTxTcpDataIdType, SOAD_CODE) SoAd_CheckUdpTxBuffer
(
  P2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr
)
{
  SoAd_PduTxTcpDataIdType BufferId = SOAD_INVALID_SOCON_ID;
  if(SoConPtr->SoConTxUdpDataId != SOAD_INVALID_PDUTXUDPDATA_ID)
  {
    P2VAR(SoAd_SoConTxUdpDataType, AUTOMATIC, SOAD_APPL_DATA) SoConTxUdpDataPtr =
      &SOAD_PBRAM(SoConTxUdpData, SoConPtr->SoConTxUdpDataId);
...
TS_MOD_PRIV_DEFN FUNC(void, SOAD_CODE) SoAd_CheckedReleaseUdpTxBuffer
(
  P2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr
)
{

  SoAd_PduTxTcpDataIdType BufferId = SoAd_CheckUdpTxBuffer(SoConPtr);
  if(BufferId != SOAD_INVALID_SOCON_ID)
  {
    P2VAR(SoAd_SoConTxBufferStateType, AUTOMATIC, SOAD_APPL_DATA) TxBufferStatePtr = NULL_PTR;
    SoAd_GetSoConTxBufferState(BufferId, TxBufferStatePtr);
    if(TxBufferStatePtr->TxLen == 0U)
    {
      SoAd_ReleaseUdpTxBuffer(SoConPtr);
    }
  }

}

TS_MOD_PRIV_DEFN FUNC(void, SOAD_CODE) SoAd_ReleaseUdpTxBuffer
(
  P2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr
)
{
  ...
  if(BufferId != SOAD_INVALID_SOCON_ID)
  {
    ...
    const uint32 BufferPoolCount = SoConGroupPtr->BufferPoolCount;
    const uint32 BufferPoolFirstId = SoConGroupPtr->BufferPoolFirstId;
    const uint32 BufferIdOffSet = SoConGroupPtr->BufferFirstId;

    /* Check that BufferId is inside the buffer pool range of this socket connection group and
     * does not free fixed buffers. */
    if((BufferId >= BufferIdOffSet) && (BufferId < (BufferIdOffSet + BufferPoolCount)))
    {
      const uint32 BufferPoolId = BufferPoolFirstId + (((uint32)BufferId) - BufferIdOffSet);
      SoAd_SetBitIn32BitMap
      (
        BufferPoolId,
        &SoAd_PBcfg->SoAdBufferPoolBitMapBase
      );
      SoConTxUdpDataPtr->TxBufferId = SOAD_INVALID_SOCON_ID; /* Reset BufferId*/
    }

    SoAd_GetSoConTxBufferState(BufferId, TxBufferStatePtr);
    TxBufferStatePtr->BufferIdx = 0U; /* Reset read index */
    TxBufferStatePtr->TxLen = 0U; /* Reset write index */
    TxBufferStatePtr->TxTimer = 0U; /* Reset buffer timer */

  }
}

TS_MOD_PRIV_DEFN FUNC(void, SOAD_CODE) SoAd_CleanUpShadowBuffer
(
  P2CONST(SoAd_SoConType, AUTOMATIC, SOAD_APPL_DATA) SoConPtr
)
{
  CONSTP2CONST(SoAd_SoConGroupType, AUTOMATIC, SOAD_APPL_DATA) SoConGroupPtr =
    &SOAD_CFG(SoConGroup, SoConPtr->SoConGroupId);
  if(SoConGroupPtr->ShadowBufferId != SOAD_INVALID_SOCON_ID)
  {
    P2VAR(SoAd_SoConTxBufferStateType, AUTOMATIC, SOAD_APPL_DATA) TxBufferStatePtr = NULL_PTR;
    SoAd_GetSoConTxBufferState((SoConGroupPtr->ShadowBufferId), TxBufferStatePtr);
    TxBufferStatePtr->BufferIdx = 0U; /* Reset read index */
    TxBufferStatePtr->TxLen = 0U; /* Reset write index */
    TxBufferStatePtr->TxTimer = 0U; /* Reset buffer timer */
  }
...
)
{
  ...
  SoAd_GetSoConTxBufferState(TxBufferId, TxBufferStatePtr);
  SOAD_ENTER_CRITSEC();
...
)
{
  ...
  SoAd_PduTxTcpDataIdType TxBufferId = SoAd_CheckUdpTxBuffer(SoConPtr);
  if(TxBufferId != SOAD_INVALID_SOCON_ID)
  {
    P2VAR(SoAd_SoConTxBufferStateType, AUTOMATIC, SOAD_APPL_DATA) TxBufferStatePtr = NULL_PTR;
    SoAd_GetSoConTxBufferState(TxBufferId, TxBufferStatePtr);
   ...
   * SoAd_PrepareUdpIfTransmit
   */
  RetVal_TcpIp = TcpIp_UdpTransmit(SoConStatePtr->SocketId, NULL_PTR, RemoteAddrPtr,
...
)
{
  ...
  {
    SOAD_BUFFERPOOLING_UDPPDUSENT(SoConPtr->SoConGroupId);
  ...
  switch(TxMode)
  {
    ...
    {
      RetVal = SoAd_Handle_Tx_Mode_Indirect(
        SoConPtr,
        RetVal,
        SoAdSrcPduInfoPtr,
        PduRouteDestVirtualPtr,
  ...
  if((SOAD_TX_MODE_NONE != TxMode) || BufferSendingRequired)
  {
    ...
    if(SOAD_INVALID_SOCON_ID != SoConGroupPtr->ShadowBufferId)
    {
      SoAd_RestoreUdpIfShadowBuffer(SoConPtr);
    }
...
)
{
  ...
  switch(RetVal_TcpIp)
  {
    ...
    case TCPIP_E_PHYS_ADDR_MISS:
    {
      if(SOAD_FLAG(SoConPtr, SOAD_SOCON_FLAG_IS_UDP_RETRY))
      {
        /* !LINKSTO SoAd.dsn.Func0141.Tx.Udp.If.UDPRetry.BufferUnavailable, 1 */
        SoAd_PduTxTcpDataIdType TxBufferId = SoAd_CheckUdpTxBuffer(SoConPtr);
        if(TxBufferId != SOAD_INVALID_SOCON_ID)
        {
          ...
          {
            /* PDU needs to be stored. Shadow Buffer will be cleaned up.*/
            SoAd_CleanUpShadowBuffer(SoConPtr);
  ...
  *SendBufferPtr = FALSE;
  ...
  if(SOAD_FLAG_UP(SoConStatePtr,SOAD_SOCONSTATE_TXING_OK) && (TotalPduLength <= 0xFFFFU))
  {
    ...
    else if(SOAD_INVALID_DEST_VIRTUAL_ID != ActivePduRouteDestVirtualId)
    {
      ...
      {
        SoAd_PduTxTcpDataIdType TxBufferId = SoAd_GetUdpTxBuffer(SoConPtr);
        if(SOAD_INVALID_SOCON_ID != TxBufferId)
        {
          if(ActivePduRouteDestVirtualPtr->SoConId == PduRouteDestVirtualPtr->SoConId)
          {
            ...
            CONSTP2CONST(SoAd_SoConGroupType, AUTOMATIC, SOAD_APPL_DATA) SoConGroupPtr =
              &SOAD_CFG(SoConGroup, SoConPtr->SoConGroupId);
            /* !LINKSTO SoAd.dsn.Func0141.Tx.Udp.If.ShadowBuffer.Unavailable, 1 */
            if(SOAD_INVALID_SOCON_ID != SoConGroupPtr->ShadowBufferId)
            {
    ...
    else
    {
      ...
      if(SOAD_INVALID_SOCON_ID != TxBufferId)
      {
        if(SOAD_FLAG_UP(SoConStatePtr, SOAD_SOCONSTATE_FLAG_TX_BUFFER_LOCK))
        {
          ...
          /* In case there is no shadow buffer E_NOT_OK will be returned*/
        }
        else
        {
          RetVal = SoAd_IfTransmit_UdpStoreToBuffer
...
{
  Std_ReturnType RetVal = E_NOT_OK;
  /* !LINKSTO SoAd.dsn.Func0141.Tx.Udp.If.TriggerNever.BufferUnavailable, 1 */
  SoAd_PduTxTcpDataIdType TxBufferId = SoAd_GetUdpTxBuffer(SoConPtr);
  if(SOAD_INVALID_SOCON_ID != TxBufferId)
  {
    ...
    if(!SOAD_FLAG(PduRouteDestCommonPtr, SOAD_PDUROUTEDEST_FLAG_TRIGGER_ALWAYS))
    {
      ...
      {
        /* Store in buffer */
        RetVal = SoAd_IfTransmit_UdpStoreToBuffer(SoConPtr, /* SoConPtr */
        SoConStatePtr, SoAdSrcPduInfoPtr, /* SoAdSrcPduInfoPtr */
        PduRouteDestVirtualPtr);
      }
      else
      {
        ...
        else
        {
          CONSTP2CONST(SoAd_SoConGroupType, AUTOMATIC, SOAD_APPL_DATA) SoConGroupPtr =
          &SOAD_CFG(SoConGroup, SoConPtr->SoConGroupId);
          const SoAd_BufLengthType ShadowBufferId = SoConGroupPtr->ShadowBufferId;
          *TxModePtr = SOAD_TX_MODE_BUFFERING;
          if(ShadowBufferId != SOAD_INVALID_SOCON_ID)
          {
            P2CONST(SoAd_SoConTxBufferCfgType, AUTOMATIC, SOAD_APPL_DATA)
            TxShadowBufferCfgPtr = NULL_PTR;
            P2VAR(SoAd_SoConTxBufferStateType, AUTOMATIC, SOAD_APPL_DATA)
            TxShadowBufferStatePtr = NULL_PTR;
            const SoAd_BufLengthType FreeBufferSize = TxBufferCfgPtr->Capacity - TcpIpTxLen;
            SoAd_GetSoConTxBufferCfg(ShadowBufferId, TxShadowBufferCfgPtr);
            SoAd_GetSoConTxBufferState(ShadowBufferId, TxShadowBufferStatePtr);
            if(FreeBufferSize < TxShadowBufferCfgPtr->Capacity)
            {
              /* Buffering is done after transmission of buffer. Shadow buffer may need to be
               * reduced if remaining buffer after new PDU is stored is smaller than the Shadow
               * Buffer capacity. */
              TxShadowBufferStatePtr->BufferIdx = TxShadowBufferCfgPtr->Capacity - FreeBufferSize;
            }
          }
        }
      }
...
)
{
  ...
  {
    RetVal =
    SoAd_TxModeSelection_With_HeaderMode
    (
      SoConPtr,
      PduRouteDestVirtualPtr,
      TcpIpTxLen,
...
)
{
  ...
  if((TxBufferStatePtr->TxLen + SoAdSrcPduInfoPtr->SduLength + HeaderLength)
  ...
  {
    SoAd_CheckedReleaseUdpTxBuffer(SoConPtr);
    /* Clean ShadowBuffer because SoAd_RestoreUdpIfShadowBuffer relies on existing TxBuffer */
    SoAd_CleanUpShadowBuffer(SoConPtr);
  }
...
)
{
  Std_ReturnType RetVal = E_NOT_OK;
  P2CONST(SoAd_SoConTxBufferCfgType, AUTOMATIC, SOAD_APPL_DATA) TxBufferCfgPtr = NULL_PTR;
  P2VAR(SoAd_SoConTxBufferStateType, AUTOMATIC, SOAD_APPL_DATA) TxShadowBufferStatePtr = NULL_PTR;
  const uint8 HeaderLength
    = (uint8)(SOAD_FLAG(SoConPtr, SOAD_SOCON_FLAG_IS_PDU_HEADER_MODE) ? SOAD_PDU_HEADER_LEN : 0U);
  ...
  SoAd_GetSoConTxBufferCfg((SoConGroupPtr->ShadowBufferId), TxBufferCfgPtr);
  TxBufferPtr = &SOAD_PBRAM(SoAdBuffer, TxBufferCfgPtr->BufId);
  SoAd_GetSoConTxBufferState((SoConGroupPtr->ShadowBufferId), TxShadowBufferStatePtr);

  BufferSize = TxBufferCfgPtr->Capacity
    - (TxShadowBufferStatePtr->BufferIdx + TxShadowBufferStatePtr->TxLen);
  ...
  if(BufferSize >= (HeaderLength + (SoAd_BufLengthType)SoAdSrcPduInfoPtr->SduLength))
  {
    ...
    else
    {
      RetVal = SoAd_IfTransmit_UdpStoreToBufferCommon
      (
        &TxBufferPtr[TxShadowBufferStatePtr->BufferIdx], /* TxBufferPtr */
        &TxShadowBufferStatePtr->TxLen, /* BufferLenPtr */
        &TxShadowBufferStatePtr->TxTimer, /* TimerPtr */
        SoConPtr,
        SoAdSrcPduInfoPtr,
        PduRouteDestVirtualPtr
      );
...
{
  P2VAR(uint8, AUTOMATIC, SOAD_APPL_DATA) TxBufferPtr = NULL_PTR;
  P2CONST(SoAd_SoConTxBufferCfgType, AUTOMATIC, SOAD_APPL_DATA) TxShadowBufferCfgPtr;
  P2CONST(SoAd_SoConTxBufferCfgType, AUTOMATIC, SOAD_APPL_DATA) TxBufferCfgPtr;
  P2VAR(SoAd_SoConTxBufferStateType, AUTOMATIC, SOAD_APPL_DATA) TxBufferStatePtr;
  P2VAR(uint8, AUTOMATIC, SOAD_APPL_DATA) TxShadowBufferPtr;
  SoAd_GetSoConTxBufferCfg((SoConGroupPtr->ShadowBufferId), TxShadowBufferCfgPtr);
  TxShadowBufferPtr = &SOAD_PBRAM(SoAdBuffer, TxShadowBufferCfgPtr->BufId);
  SoAd_GetSoConTxBufferState(TxBufferId, TxBufferStatePtr);
  SoAd_GetSoConTxBufferCfg(TxBufferId, TxBufferCfgPtr);
  TxBufferPtr = &SOAD_PBRAM(SoAdBuffer, TxBufferCfgPtr->BufId);
  SOAD_PRECONDITION_ASSERT
  (
    (TxBufferStatePtr->TxLen + TxShadowBufferStatePtr->TxLen) <= TxBufferCfgPtr->Capacity,
    SOAD_IFTRANSMIT_SVCID
  );
  TS_MemCpy
    (
      &TxBufferPtr[TxBufferStatePtr->TxLen],
      &TxShadowBufferPtr[TxShadowBufferStatePtr->BufferIdx],
      TxShadowBufferStatePtr->TxLen
    );
  TxBufferStatePtr->TxLen += TxShadowBufferStatePtr->TxLen;
  TxBufferStatePtr->TxTimer = TxShadowBufferStatePtr->TxTimer;
}
...
)
{
  ...
  if(0U != TxShadowBufferStatePtr->TxLen)
  {

    /* Initialize with previous. Underflow is covered by ComStack_FindNextOne() */
    SoAd_PduTxTcpDataIdType TxBufferId = SoAd_CheckUdpTxBuffer(SoConPtr);
    /* TxBufferId is always valid, because ShadowBuffer is only filled if buffer is available and
     * cleaned if buffer is released*/
    SOAD_POSTCONDITION_ASSERT(SOAD_INVALID_SOCON_ID != TxBufferId, DET_INTERNAL_API_ID);

    SoAd_CopyShadowBufferToTxBuffer
    (
      SoConGroupPtr,
      TxBufferId,
      TxShadowBufferStatePtr
    );
    /* Reset transmitted buffer position. */
    SoAd_CleanUpShadowBuffer(SoConPtr);

    SoAd_ResetStateOfIncludedPdus
    (
      SoConPtr
    );
...
)
{
  ...
  if(!SOAD_FLAG(PduRoutePtr , SOAD_PDUROUTE_FLAG_LAST_IS_BEST))
  {
    ...
    TS_MemCpy(PayloadDestPtr, &TxBufPtr[TxBufferStatePtr->BufferIdx], CopyNow);
    /* !LINKSTO SoAd.ASR42.SWS_SoAd_00543_UDP_IF_Tx_nPDU_IPFrag, 1 */
    TxBufferStatePtr->BufferIdx += CopyNow;
    PayloadDestPtr = &PayloadDestPtr[CopyNow];
    /* Udp frame cannot be bigger than 64k, cast of TxLen is protected by check in
     * SoAd_PrepareUdpIfTransmit
     */
