...
/** \brief mask for extracting node type */
#if (defined LINIF_NODETYPE_MASK)
#error LINIF_NODETYPE_MASK already defined!
#endif /* #if (defined LINIF_NODETYPE_MASK)*/
#define LINIF_NODETYPE_MASK        0x20U

/** \brief mask for identifying a slave node */
#if (defined LINIF_CH_IS_SLAVE)
#error LINIF_CH_IS_SLAVE already defined!
#endif /* #if (defined LINIF_CH_IS_SLAVE)*/
#define LINIF_CH_IS_SLAVE          0x20U

/** \brief mask for identifying a master node */
#if (defined LINIF_CH_IS_MASTER)
#error LINIF_CH_IS_MASTER already defined!
#endif /* #if (defined LINIF_CH_IS_MASTER)*/
#define LINIF_CH_IS_MASTER          0U
...
#define LinIf_StopTimerBI(pIfData)     \
        TS_AtomicAssign16((pIfData)->TimerBusIdle, 0U) \
...
#define LinIf_StartTimerNas(pIfChConfig, pIfData)           \
        TS_AtomicAssign16((pIfData)->TimerNas, (pIfChConfig)->NCNasTimeout) \
...
#define LinIf_StopTimerNas(pIfData) \
        TS_AtomicAssign16((pIfData)->TimerNas, 0U) \
...
#define LINIF_CFG_GET_CHANNEL_MASTER(Channel) \
(&LINIF_PBCFG_ACCESS(LinIfMasterChannelType, LinIfMasterRef)[(Channel)])
...
(&LINIF_PBCFG_ACCESS(LinIfSlaveChannelType, LinIfSlaveRef)[(Channel)])
...
&LinIf_SlaveChannelInfo[(Channel)]
...
(&LINIF_PBCFG_ACCESS(LinIf_SlaveETFrmListSepType, LinIf_SlaveETFrmListSepRef)[(Channel)])
...
#endif /* LINIF_BUS_MIRRORING_SUPPORT_ENABLE == STD_ON */
#if (LINIF_EOS_NOTIF_CALLOUT_SUPPORTED == STD_ON)
    boolean FailedFrame;              /**< flag for failed frame setup */
#endif /* LINIF_EOS_NOTIF_CALLOUT_SUPPORTED == STD_ON */
...
extern VAR(Atomic_t, LINIF_VAR) LinIf_SleepHandlingInProgress[LINIF_MAX_CHANNELS];
