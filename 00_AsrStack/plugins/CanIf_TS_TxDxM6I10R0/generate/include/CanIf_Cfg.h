...
[!IF "CanIfPublicCfg/CanIfBusMirroringSupport = 'true'"!]
[!IF "CanIfMirroringSupport/CanIfMirrorToCDDReportingEnable = 'true'"!]
/* !LINKSTO CanIf.EB.SWS_CANIF_00905.Cdd,1 */
#include <[!"CanIfMirroringSupport/CanIfMirrorToCDDReportingHeader"!]>
[!ENDIF!]
[!ENDIF!]
...
#define CANIF_DEV_RUNTIME_ERROR_DETECT          [!WS "3"!][!IF "CanIfPublicCfg/CanIfDetReportRuntimeError = 'true'"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
#endif
#define CANIF_MULTICORE_SUPPORT         [!WS "3"!][!IF "CanIfPublicCfg/CanIfMultiCoreSupport = 'true'"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
#define CANIF_CTRL_SIZE [!"num:i(count(CanIfCtrlDrvCfg/*/CanIfCtrlCfg/*))"!]U
...
#if (defined CANIF_CDD_REPORTING_ENABLE)
#error CANIF_CDD_REPORTING_ENABLE already defined!
#endif /* #if( defined CANIF_CDD_REPORTING_ENABLE)*/
#define CANIF_CDD_REPORTING_ENABLE   [!WS "3"!][!//
[!IF "CanIfMirroringSupport/CanIfMirrorToCDDReportingEnable = 'true'"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!][!/*
*/!]

#if (defined CANIF_UL_REPORTING)
#error CANIF_UL_REPORTING already defined!
#endif /* #if( defined CANIF_UL_REPORTING)*/
[!IF "CanIfMirroringSupport/CanIfMirrorToCDDReportingEnable = 'true'"!]
#define CANIF_UL_REPORTING [!"CanIfMirroringSupport/CanIfMirrorToCDDReportingFunctionName"!]
[!ELSE!]
#define CANIF_UL_REPORTING Mirror_ReportCanFrame
[!ENDIF!]
...
#if (defined CANIF_MAX_TX_PDUS)
#error CANIF_MAX_TX_PDUS already defined!
#endif /* #if(defined CANIF_MAX_TX_PDUS) */
#define CANIF_MAX_TX_PDUS [!"$canIfMaxTxPdus"!]U
[!ENDIF!][!/* ($ulTxConfirmationSupport = 'true') and ($ulTxConfirmationResutSupported = 'true') */!]
