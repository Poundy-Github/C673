...
typedef struct
{
  CONSTP2VAR(WdgM_LocalStatusType, AUTOMATIC, AUTOMATIC) WdgM_EB_SELocalStatus;
#if (WDGM_EB_GET_ALL_EXPIRED_SE == STD_ON)
  P2VAR(uint8, AUTOMATIC, AUTOMATIC) WdgM_EB_ExpiredSEIDs;
  CONST(uint8, WDGM_CONST) WdgM_EB_ExpiredSEIDArraySize;
  P2VAR(uint8, AUTOMATIC, AUTOMATIC) WdgM_EB_ExpiredSEIDsCounter;
#endif
  ...
  CONSTP2VAR(WdgM_EB_InitStatusType, AUTOMATIC, AUTOMATIC) WdgM_EB_InitStatus;
  /**< initialization status for each core */
#if (WDGM_EB_MULTICORE_ENABLED == STD_ON)
  CONSTP2VAR(WdgM_ModeType, AUTOMATIC, AUTOMATIC) WdgM_EB_CoreSwitchMode;
  ...
  WdgM_ModeType WdgM_EB_CurrentModeId;
  ...
  CONSTP2VAR(uint32, AUTOMATIC, AUTOMATIC) WdgM_EB_PartitionSupervisionCycleCntPtr;
