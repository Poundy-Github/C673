...
/** \brief SID of service EcuC_SetPduMetaData() */
#define ECUC_SID_ECUC_SETMETADATATYPE                 0x13U
/** \brief SID of service EcuC_GetPduMetaData() */
#define ECUC_SID_ECUC_GETMETADATATYPE                 0x14U
...
/** \brief Internal handler function for meta data type copy
 *
 * Copies meta data (meta data type) from the PDU's meta data memory to an internal buffer
 *
 * \param[in] PduId                ID of the PDU the meta data type is belonging to
 * \param[in] PduInfoPtr           pointer to the PDU info (unused)
 * \param[out] MetaDataTypeDstPtr  pointer where the meta data type is copied to
 *
 * \return Result of operation
 * \retval E_OK                 value is copied successfully
 * \retval E_NOT_OK             the PDU ID does not exist
 */
STATIC FUNC(Std_ReturnType, ECUC_CODE) EcuC_CopyMetaDataTypeFromPduInt
(
  EcuC_PduIdType PduId,
  P2CONST(PduInfoType, AUTOMATIC, ECUC_APPL_DATA) PduInfoPtr,
  P2VAR(uint8, AUTOMATIC, ECUC_APPL_DATA) MetaDataTypeDstPtr
);
 ...
 * \param[out] PduInfoPtr     pointer to the PDU info (unused)
...
/** \brief Internal handler function for meta data type copy
 *
 * Copies meta data (meta data type) from an internal buffer to the PDU's meta data memory
 *
 * \param[in] PduId               ID of the PDU the meta data type is belonging to
 * \param[out] PduInfoPtr         pointer to the PDU info (unused)
 * \param[in] MetaDataTypeSrcPtr  pointer where the meta data type is copied from
 *
 * \return Result of operation
* \retval E_OK                value is copied successfully
 * \retval E_NOT_OK           the PDU ID does not exist
 */
STATIC FUNC(Std_ReturnType, ECUC_CODE) EcuC_CopyMetaDataTypeToPduInt
(
  EcuC_PduIdType PduId,
  P2VAR(PduInfoType, AUTOMATIC, ECUC_APPL_DATA) PduInfoPtr,
  P2CONST(uint8, AUTOMATIC, ECUC_APPL_DATA) MetaDataTypeSrcPtr
);
...
/* !LINKSTO EcuC.ArchDesign.MetaData.API.EcuC_CopyMetaDataTypeToPdu,1 */
FUNC(Std_ReturnType, ECUC_CODE) EcuC_CopyMetaDataTypeToPdu
(
  EcuC_PduIdType PduId,
  P2VAR(PduInfoType, AUTOMATIC, ECUC_APPL_DATA) PduInfoPtr,
  P2CONST(uint8, AUTOMATIC, ECUC_APPL_DATA) MetaDataTypeSrcPtr
...
#if (ECUC_DEV_ERROR_DETECT == STD_ON)
  if(MetaDataTypeSrcPtr == NULL_PTR)
  {
    ECUC_REPORT_ERROR(ECUC_SID_ECUC_SETMETADATATYPE, ECUC_E_PARAM_POINTER);
  }
  else
#endif /* (ECUC_DEV_ERROR_DETECT == STD_ON) */
  {
    retval = EcuC_CopyMetaDataTypeToPduInt(PduId, PduInfoPtr, MetaDataTypeSrcPtr);

#if (ECUC_DEV_ERROR_DETECT == STD_ON)
    if(retval == E_NOT_OK)
    {
      ECUC_REPORT_ERROR(ECUC_SID_ECUC_SETMETADATATYPE, ECUC_E_INV_ARG);
    }
#endif /* (ECUC_DEV_ERROR_DETECT == STD_ON) */
  }
  return retval;
...
/* !LINKSTO EcuC.ArchDesign.MetaData.API.EcuC_CopyMetaDataTypeFromPdu,1 */
FUNC(Std_ReturnType, ECUC_CODE) EcuC_CopyMetaDataTypeFromPdu
(
  EcuC_PduIdType PduId,
  P2CONST(PduInfoType, AUTOMATIC, ECUC_APPL_DATA) PduInfoPtr,
  P2VAR(uint8, AUTOMATIC, ECUC_APPL_DATA) MetaDataTypeDstPtr
...
#if (ECUC_DEV_ERROR_DETECT == STD_ON)
  if(MetaDataTypeDstPtr == NULL_PTR)
  {
    ECUC_REPORT_ERROR(ECUC_SID_ECUC_GETMETADATATYPE, ECUC_E_PARAM_POINTER);
  }
  else
#endif /* (ECUC_DEV_ERROR_DETECT == STD_ON) */
  {
    retval = EcuC_CopyMetaDataTypeFromPduInt(PduId, PduInfoPtr, MetaDataTypeDstPtr);

#if (ECUC_DEV_ERROR_DETECT == STD_ON)
    if(retval == E_NOT_OK)
    {
      ECUC_REPORT_ERROR(ECUC_SID_ECUC_GETMETADATATYPE, ECUC_E_INV_ARG);
    }
#endif /* (ECUC_DEV_ERROR_DETECT == STD_ON) */
  }
  return retval;
...
)
{
  ...
  if(PduId < EcuC_ConfigData.numConfigPdus)
  {
    ...
    (
      MetaDataTypeDstPtr,
      EcuC_ConfigData.metaDataItemConfig[dataOffset].metaDataItem.metaDataItemOffsetPtr,
      EcuC_ConfigData.pduConfig[PduId].metaDataTypeLength
    );
    retval = E_OK;
  }
