 ...
 *
 * Reason:
 * In the context of "for (TxSrcIterator = XCPR_NO_OF_RX_SRC_PDU_IDS;
 *                         TxSrcIterator < XCPR_NO_OF_SRC_PDU_IDS; TxSrcIterator++)"
 * when "XCPR_NO_OF_RX_SRC_PDU_IDS = SrcIterator" the message condition is always
 * false is expected;
 * Expression "TxSrcIterator < XCPR_NO_OF_SRC_PDU_IDS" is always evaluated as false
 * if "TxSrcIterator = XCPR_NO_OF_SRC_PDU_IDS".
 *
 * TASKING-2) Deviated Rule: W549
 * W549: condition is always true
 *
 * Reason:
 * In the context of "for(Iterator = 0U;
 *                        Iterator < XCPR_NO_OF_RX_SRC_PDU_IDS; Iterator++)"
 * when "XCPR_NO_OF_RX_SRC_PDU_IDS = 1" the message condition is always true is expected;
 * Expression "Iterator < XCPR_NO_OF_RX_SRC_PDU_IDS" is always evaluated as true
 * if "XCPR_NO_OF_RX_SRC_PDU_IDS = 1".
 * Expression "Iterator < XCPR_DEST_PDU_FLAG_SIZE" is always evaluated as true
 * if "XCPR_DEST_PDU_FLAG_SIZE = 1".
 * Expression "Iterator < (XCPR_NO_OF_FLEXRAY_DEST_WITH_SEQ_CORR +
 * XCPR_NO_OF_ETHERNET_DESTINATIONS)" is always evaluated as true if
 * "(XCPR_NO_OF_FLEXRAY_DEST_WITH_SEQ_CORR + XCPR_NO_OF_ETHERNET_DESTINATIONS)= 1".
 * Expression "Iterator < XCPR_NO_OF_DEST_MULTI_PDUS_IN_FRAME" is always evaluated as true
 * if "XCPR_NO_OF_DEST_MULTI_PDUS_IN_FRAME = 1".
 * Expression "Iterator < XCPR_NO_OF_SRC_PDU_IDS" is always evaluated as true
 * if "XCPR_NO_OF_SRC_PDU_IDS = 1".
 * Expression "Iterator < XCPR_NO_OF_TX_DEST_PDU_IDS" is always evaluated as true
 * if "XCPR_NO_OF_TX_DEST_PDU_IDS = 1".
 * Expression "Iterator < XCPR_NO_OF_CONNECTION_GROUPS" is always evaluated as true
 * if "XCPR_NO_OF_CONNECTION_GROUPS = 1".
 * Expression "Iterator < XCPR_NO_OF_TX_DESTINATION_PDUS" is always evaluated as true
 * if "XCPR_NO_OF_TX_DESTINATION_PDUS = 1".
 */
...
)
{
  ...
  /* !LINKSTO XcpR.Impl.ConcatMessagesFlexRayPackMultipleMessages,1 */
  /* !LINKSTO XcpR.Impl.ConcatMessagesEthernetPackMultipleMessages,1 */
...
{
#if ((XCPR_DET_RUNTIME_CHECKS == STD_ON) || (XCPR_DEV_ERROR_DETECT == STD_ON))
  uint8 DetIterator;
#endif
