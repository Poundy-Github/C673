...
{
  const Fls_AddressType TargetAddressInFlash = Fee_Gv.FeeWorkingInfoAddress;
  const Fls_AddressType TargetDataAddress = Fee_Gv.FeeDataAddress;
  Fls_LengthType LengthToWrite;
...
)
{
  ...
  /* Read the block data in one Fls_Read call */
...
FUNC(void, FEE_CODE) Fee_SfSSWriteWrittenMarker(void)
{
  ...
  else if ((MEMIF_JOB_OK == Fee_Gv.FeeFlashJobResult) && (!FEE_CHECK_FLAG(FEE_FLASH_FINAL_REFUSAL_MASK)))
  {
    ...
    if(FEE_CHECK_FLAG(FEE_HIGHEST_INTERNAL_PRIORITY_MASK))
    {
      Fee_Gv.FeeSwitchRequiredSpace -= ((Fls_LengthType)BlockSizeAlligned+
