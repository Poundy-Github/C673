...
#
TRESOS2_ADDITIONAL_RSC_MODULES ?= Resource
TRESOS2_CONF_MODULE_IDS   = $(sort $(PLUGINS_TO_GENERATE) $(foreach MOD,$(TRESOS2_ADDITIONAL_RSC_MODULES),$(if $($(MOD)_VARIANT),$(MOD)_$($(MOD)_VARIANT))))
TRESOS2_CONF_STUBS        = $(patsubst %Stub,%,$(patsubst %_Stub,%,$(filter-out $(TRESOS2_GEN_PLUGINS),$(basename $(notdir $(TRESOS2_CONF_FILES))))))
TRESOS2_CONF_STUBS_IDS    = $(foreach MOD,$(TRESOS2_CONF_STUBS),\
                             $(if $($(MOD)Stub_VARIANT),$(MOD)_$($(MOD)Stub_VARIANT),\
                              $(if $($(MOD)Stub_40_VARIANT),$(MOD)_$($(MOD)Stub_40_VARIANT),\
                               $(if $($(MOD)Stub_41_VARIANT),$(MOD)_$($(MOD)Stub_41_VARIANT),\
                                $(if $($(MOD)Stub_42_VARIANT),$(MOD)_$($(MOD)Stub_42_VARIANT),\
                                 $(if $($(MOD)Stub_43_VARIANT),$(MOD)_$($(MOD)Stub_43_VARIANT),\
                                  $(if $($(MOD)Stub_44_VARIANT),$(MOD)_$($(MOD)Stub_44_VARIANT))))))))

# create a semicolon-separated list. Value for -DEcuResourceModuleIds (generator call, legacy mode only)
TRESOS2_CONF_ALL_IDS = $(strip $(TRESOS2_CONF_MODULE_IDS) $(TRESOS2_CONF_STUBS_IDS))
TRESOS2_CONF_ECU_RESOURCEMODULE_IDS = $(subst $(BLANK),;,$(strip $(TRESOS2_CONF_ALL_IDS)))
