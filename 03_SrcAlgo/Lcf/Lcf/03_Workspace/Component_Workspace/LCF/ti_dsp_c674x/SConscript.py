...
#
# MODULE NAME: lib_env_ti_c674x\SConscript.py
#
# DESCRIPTION: SConscript to set up the build environment for the build of TI C674x Libraries. 
...
#
# LAST CHANGE:      $Date: 2021/12/14 01:43:39CET $
#                   $Author: Wang, David (Wangd3) $
#
# CURRENT VERSION:  $Revision: 1.1 $
#############################################################################
# CHANGES:                   $Log: SConscript.py  $
# CHANGES:                   Revision 1.1 2021/12/14 01:43:39CET Wang, David (Wangd3) 
# CHANGES:                   Initial revision
# CHANGES:                   Member added to project /ADAS/Projects/MFC5xx/MFC5J3/06_Algorithm/DPU/04_Engineering/03_Workspace/Component_Workspace/LCF/ti_dsp_c674x/project.pj
# CHANGES:                   Revision 1.2 2017/06/27 14:02:43CEST Asong, Wedndah Asaha (uidj2187) 
# CHANGES:                   Modified scons config scripts for Mainstream SIL-Build
# CHANGES:                   Revision 1.1 2017/06/13 16:13:24CEST Berendes, Ignacius (uid41806) 
# CHANGES:                   Initial revision
# CHANGES:                   Member added to project /ADAS/SW/Components/LCF_LateralControlFunctions/04_Engineering/03_Workspace/Component_Workspace/LCF/ti_dsp_c674x/project.pj
# CHANGES:                   Revision 1.0 2016/07/01 12:15:43CEST Prajescu, Claudia (uidl9533) 
# CHANGES:                   Initial revision
# CHANGES:                   Member added to project /nfs/projekte1/REPOSITORY/Base_Development/05_Algorithm/LCF_LateralControlFunctions/04_Engineering/03_Workspace/algo/lcf/ti_dsp_c674x/project.pj
# CHANGES:                   Revision 1.47 2015/09/23 16:11:21CEST Fischer-EXT, Andre (uidg5297) 
# CHANGES:                   add dependency between ccs project generation and ti compiler-directory
# CHANGES:                   - Added comments -  uidg5297 [Sep 23, 2015 4:11:22 PM CEST]
# CHANGES:                   Change Package : 379758:1 http://mks-psad:7002/im/viewissue?selection=379758
# CHANGES:                   Revision 1.46 2015/07/22 10:58:41CEST Fischer-EXT, Andre (uidg5297) 
# CHANGES:                   replace resolve_contradicting_ccflags by resolve_incompatible_ccflags due to renaming function
# CHANGES:                   --- Added comments ---  uidg5297 [Jul 22, 2015 10:58:41 AM CEST]
# CHANGES:                   Change Package : 359738:1 http://mks-psad:7002/im/viewissue?selection=359738
# CHANGES:                   Revision 1.45 2015/07/06 10:05:10CEST Fischer-EXT, Andre (uidg5297) 
# CHANGES:                   update handling of optimization flags
# CHANGES:                   --- Added comments ---  uidg5297 [Jul 6, 2015 10:05:11 AM CEST]
# CHANGES:                   Change Package : 353994:1 http://mks-psad:7002/im/viewissue?selection=353994
# CHANGES:                   Revision 1.44 2015/06/26 07:06:20CEST Palanisamy-EXT, Lenin (uidr0826) 
# CHANGES:                   Add revision label with module description for all GSCons files (Issue 320651)
# CHANGES:                   --- Added comments ---  uidr0826 [Jun 26, 2015 7:06:20 AM CEST]
# CHANGES:                   Change Package : 350940:1 http://mks-psad:7002/im/viewissue?selection=350940

#***************************************************************************************************

# SConscript to build TI C674x library and CCS 5.3 project
...
execfile(sconscript_dir + "/ti_c674x_lib_config.scfg")
...
dsp_env = Environment(# list of scons ADAS extensions used
                      ...
                      BUILD_TARGET = "TI_C674X",
                      # use Visual Studio 2005
                      MSVS_VERSION = "8.0",
                      MSVC_VERSION = "8.0")

if not verbose:
    dsp_env['CCCOMSTR'] = "\nCompiling ecu c674x $variant\n[$SOURCE]"
    dsp_env['CXXCOMSTR'] = "\nCompiling ecu c674x $variant\n[$SOURCE]"
    dsp_env['LINKCOMSTR'] = "\nLinking ecu c674x $variant\n[$TARGET]"
...
for e in dsp_env_list:
    copied_deliverables_dir = False
    env = e.Clone()
    # setup output folder
    dsp_algo_build_dir = (build_dir + "/algo/" + algo_name +
                         "/ti_c674x/" + env["variant"])
    dsp_algo_deliverables_dir = (deliverables_dir + "/lib/ti_c674x/algo/" +
                         algo_name + "/")
    ...
    env.VariantDir(dsp_algo_build_dir,
                   "../../../../",
                   duplicate = 0)
    result = SConscript(dsp_algo_build_dir + "/01_Source_Code/algo/" + component_name + "/SConscript.py",
                        exports = {
                            "env" : env,
    ...
    if install_to_deliverables_dir in env["variant"]:
        algo_ti_c674x_lib = env.Install([dsp_algo_build_dir, dsp_algo_deliverables_dir],
                                    result[0][0])

    else:
        algo_ti_c674x_lib = env.Install([dsp_algo_build_dir],
                                    result[0][0])

    # generate aliases containing the variant's name
    generate_alias(component_name + "_algo_ti_c674x_" + env["variant"], algo_ti_c674x_lib)
    generate_alias(component_name + "_algo_ecu_" + env["variant"], algo_ti_c674x_lib)
    generate_alias(component_name + "_algo_" + env["variant"], algo_ti_c674x_lib)
    if algo_name != component_name:
        generate_alias(algo_name + "_algo_ti_c674x_" + env["variant"], algo_ti_c674x_lib)
        generate_alias(algo_name + "_algo_ecu_" + env["variant"], algo_ti_c674x_lib)
        generate_alias(algo_name + "_algo_" + env["variant"], algo_ti_c674x_lib)
    generate_alias(env["variant"], algo_ti_c674x_lib)
    generate_alias("all_" + env["variant"], algo_ti_c674x_lib)
    # generate aliases NOT containing the variant's name
    generate_alias(component_name + "_algo_ti_c674x", algo_ti_c674x_lib, component_name + ':algo:ecu:ti_c674x', ['algo lib: C674x'])
    generate_alias(component_name + "_algo_ecu", algo_ti_c674x_lib, component_name + ':algo:ecu')
    generate_alias(component_name + "_algo", algo_ti_c674x_lib, component_name + ':algo')
    if algo_name != component_name:
        generate_alias(algo_name + "_algo_ti_c674x", algo_ti_c674x_lib, component_name + ':' + algo_name + '_algo:ecu:ti_c674x', ['algo lib: C674x'])
        generate_alias(algo_name + "_algo_ecu", algo_ti_c674x_lib, component_name + ':' + algo_name + '_algo:ecu')
        generate_alias(algo_name + "_algo", algo_ti_c674x_lib, component_name + ':' + algo_name + '_algo')
    generate_alias("all", algo_ti_c674x_lib, component_name)
    ...
    if COMMAND_LINE_TARGETS:
        for command in COMMAND_LINE_TARGETS:
            if((re.search(algo_name + "_sim", command)) or (re.search(component_name + "_sim", command))):    
                NoClean([algo_ti_c674x_lib, dirsToClean])
            else:
			    Clean(algo_ti_c674x_lib, dirsToClean)
    else:
        Clean(algo_ti_c674x_lib, dirsToClean)
...
if not skip_ide_generation:
    ...
    ide_ccs_algo_ti_c674x = dsp_env.EclipseProject(
                        # specify project name
                        target_name = algo_name.upper() + "_TI_C674x_Lib",
                        # specify folder for storing the project
                        target_dir = build_dir + "/algo/" + algo_name + "/ti_c674x/",
                        ...
                        scons_target = [algo_name + "_algo_ti_c674x_" + variant[0] for variant in variant_list],
    ...
    Requires(ide_ccs_algo_ti_c674x, Dir(c6000_code_gen_tool_dir).abspath)

    generate_alias(component_name + "_ide_ccs_algo_ti_c674x", ide_ccs_algo_ti_c674x, component_name + ':ide:ccs:ti_c674x', ['CCS 5: C674x algo lib'])
    generate_alias(component_name + "_ide_ccs", ide_ccs_algo_ti_c674x, component_name + ':ide:ccs')
    generate_alias(component_name + "_ide", ide_ccs_algo_ti_c674x, component_name + ':ide')
    if algo_name != component_name:
        generate_alias(algo_name + "_ide_ccs_algo_ti_c674x", ide_ccs_algo_ti_c674x, component_name + ':' + algo_name + '_ide:ccs:ti_c674x', ['CCS 5: C674x algo lib'])
        generate_alias(algo_name + "_ide_ccs", ide_ccs_algo_ti_c674x, component_name + ':' + algo_name + '_ide:ccs')
        generate_alias(algo_name + "_ide", ide_ccs_algo_ti_c674x, component_name + ':' + algo_name + '_ide')
    generate_alias("all", ide_ccs_algo_ti_c674x, component_name)
...
if not skip_ide_generation:
    ...
    for index, variant in enumerate(variant_list):
        map(lambda x : ecu_cppdefs[variant[0]].append(x), c674x_built_in_defines_list)
        if "--abi=eabi" in dsp_env_list[index].Clone(variant=variant[0])['CCFLAGS']:
            map(lambda x : ecu_cppdefs[variant[0]].append(x), c674x_built_in_eabi_defines_list)
    ...
    talias = [algo_name + "_algo_ti_c674x_" + variant[0] for variant in variant_list]
    talias += [algo_name + "_qac_ti_c674x"]
    tcppdefs.append([])
    if profile:
