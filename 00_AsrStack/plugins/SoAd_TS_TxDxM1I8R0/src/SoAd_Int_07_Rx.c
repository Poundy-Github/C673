...
)
{
  ...
  {
    /* Avoid to copy more data then available in the ring buffer. */
    BytesToCopy = SOAD_MIN(BufferSize, RingBufferAccessPtr->Used);
  }

  /* NOTE: The following loop is repeated until all buffered PDUs are processed, UL returns busy
   * or an error occurs. Several calls to <UL>_CopyRxData are possible,
   * to simplify the handling of their return values it is done mutually as part of the loop. */
  while(BytesToCopy > 0U)
  {
    ...
    if(RetVal_BufReq == BUFREQ_OK)
    {
      *TcpMoveWindowLenPtr += BytesToCopy;
    }
    else
    {
      BufferSize = 0U;
...
)
{
  ...
  if(SocketRoutePtr->SocketRouteDestCnt == 1U)
  {
    /* Only one Dest exists. */
    CONSTP2VAR(uint32, AUTOMATIC, SOAD_APPL_DATA) SocketRouteDestVirtualEnabledBitMapPtr
      = &SOAD_PBRAM_REF_ID(uint32, SoAd_PBcfg->SocketRouteDestVirtualEnabledBitMapBase, 0U);
    P2CONST(SoAd_SocketRouteDestType, AUTOMATIC, SOAD_APPL_DATA) SocketRouteDestPtr =
              &SOAD_CFG(SocketRouteDest, SocketRoutePtr->SocketRouteDestFirstId);
    uint32 Offset = SocketRouteDestPtr->VirtualDestinationStartId;
    if(SocketRouteDestPtr->VirtualDestinationCount > 1U)
    {
      /* Destination belong to a specific routing group and has individual bit for each
          socket connection of the group. */
      Offset += SoConGroupFirstIdOffset;
    }

    if(0U != SOAD_GET_BIT_FROM_32BIT_BITARRAY(SocketRouteDestVirtualEnabledBitMapPtr, Offset))
    {
      IsEnabled = TRUE;
    }
  }
  else
  {
    ...
    if(*SocketRouteDestIdPtr != SOAD_INVALID_SOCKET_ROUTE_DEST_ID)
    {
      IsEnabled = TRUE;
    }
  }
