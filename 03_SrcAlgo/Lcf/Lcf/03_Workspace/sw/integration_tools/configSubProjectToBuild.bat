...
rem This script configures all component sub-projects to head. 
...
for /F "tokens=2 delims==" %%s in ('set supProj[') do (
	...
	echo Reconfigure now ..\%%s >> %INITIAL_WD%\%tsnow%_configSubProject.txt
	cd ..\..\..\%%s	
	for /d %%a in ("!cd!\*") do (
        if %%~nxa neq common (
            if %%~nxa neq algo (
                ...
                cd ..
                if !ANSWER! neq Y (
                    echo Do you want to reconfigure %%a ^(!cpSource! to !cpHead!^) ? [!ANSWER!] 
                    SET /P ANSWER=
                )
                if !ANSWER!==y (
                    si configuresubproject --subprojectRevision=:head --changePackageId=!changePID! --type=build %%~nxa\project.pj
                    echo %%a ^(!cpSource! to !cpHead!^)
                    echo %%a ^(!cpSource! to !cpHead!^) >> %INITIAL_WD%\%tsnow%_configSubProject.txt
                )
                if !ANSWER!==Y (
                    si configuresubproject --subprojectRevision=:head --changePackageId=!changePID! --type=build %%~nxa\project.pj
                    echo %%a ^(!cpSource! to !cpHead!^)
        ...
        if %%~nxa equ common (
            ...
            for /d %%b in ("!cd!\*") do (
                ...
                if !ANSWER!==y (
                    si configuresubproject --subprojectRevision=:head --changePackageId=!changePID! --type=build %%~nxb\project.pj
                    echo %%b ^(!cpSource! to !cpHead!^)
                    echo %%b ^(!cpSource! to !cpHead!^) >> %INITIAL_WD%\%tsnow%_configSubProject.txt   
                )
                if !ANSWER!==Y (
                    si configuresubproject --subprojectRevision=:head --changePackageId=!changePID! --type=build %%~nxb\project.pj
                    echo %%b ^(!cpSource! to !cpHead!^)
