 ...
 *
 * $Id: TRICORE-startcore.c 34824 2021-08-05 13:57:39Z ache271795 $
*/
...
OS_TOOL_FUNCTION_ATTRIB_POST
{
	...
	bootreg = OS_MFCCR(cfsrBase, OS_BOOTREG);
	if ( (bootreg & OS_BOOTMASK) == OS_BOOTHALTED )
	{
		/* Initialise the stack pointer to interrupt stack */
		OS_MTCCR(cfsrBase, OS_A10, ((os_uint32_t)(OS_kernel_ptr[core]->iSpInitial)));

		/* Set PSW and tell the CPU that we're running on the interrupt stack.
		*/
		OS_MTCCR(cfsrBase, OS_PSW, OS_INITIAL_PSW | OS_PSW_IS);

		/* initialize global address register to kernel data struct */
		OS_MTCCR(cfsrBase, OS_A8, (os_uint32_t)OS_kernel_ptr[core]);



		/* Core startup procedure is architecture dependent */
#if (OS_TRICOREARCH==OS_TRICOREARCH_18)
		/* Aurix TC18 adds support for hardware assisted virtualisation.
		 * If this option is enabled, all the cores will boot up in hypervisor mode.
		 * Board code entry is OS_CoreStartupEntry. It disables the virtualisation,
		 * switch to normal mode, loads the address of OS_KickStartCoreAurixTC18
		 * to A11 and ends by executing OS_InitCsaList which exits through
		 * a simple jump to A11, resuming in OS_KickStartCoreAurixTC18.
		 */
		/* Deviation MISRAC2012-2 <2> */
		OS_MTCCR(cfsrBase, OS_PC, (os_uint32_t)&OS_CoreStartupEntry);
		 ...
		 */
		/* Deviation MISRAC2012-2 <2> */
		OS_MTCCR(cfsrBase, OS_PC, (os_uint32_t)&OS_InitCsaList);
		OS_MTCCR(cfsrBase, OS_A11, (os_uint32_t)&OS_KickStartCore);
#endif

		/* Reset Halt */
		bootreg = (bootreg & (~((os_uint32_t)OS_BOOTMASK))) | OS_BOOTRELEASE;
		OS_MTCCR(cfsrBase, OS_BOOTREG, bootreg);
	}
}
